...
to. A datasheet's spectral dye density chart records how strongly the dyes
absorb at every visible wavelength. Reversal and motion-picture sheets give
one curve per dye. Still-film colour negative sheets give only the combined
density of a mid-grey and of the unexposed base, and this project infers the
three dye curves from that pair. The overlap between the dye curves is
...
The secondary route for motion picture negative, omitted from the figure, decodes the same
...
derived from picture content. A reference target photographed on the roll
under a declared illuminant counts as a measurement, and
may set a per-roll parameter that is then recorded and held fixed across the
frames it governs. **No stage neutralises the illumination under which the
photograph was made.** A frame lit by tungsten remains warm, and a roll
exposed as the light changed returns with that change intact. A colour cast can be removed later in
the grade; a cast that an automatic estimator has removed cannot be restored.
What survives is the illumination as this film and this paper render it, a
photographic record of the light source; whether
that rendering matches the physical illuminant is a separate claim, unverified
on film (PROJECT.md, Known limitations).
...
Pro Laser.** The paper determines how much of the
...
The simulation targets the paper's colour treatment alone. The render is an
idealised print: the
...
lowering it at the adjustment node is ordinary use. The pairing carries no obligation: print emulation is a look,
...
orange colouration as its still-film relatives, and in current practice is
scanned far more often than printed, although Kodak documents both workflows.
Two routes read the scan, forking at one node.
...
assumption the stock does not meet, so its bound is scored against the
scene-referred route as a model reference; Resolve's built-in "LMT Kodak
2383 Print Film Emulation" may optionally follow the decode for a
print-through look.

The scene-referred route is the secondary path, per stock, for graded work:
it reads the scan back as a per-stock, model-based estimate of the relative
scene exposure the characteristic curves imply, under the stock's nominal
balance illuminant, as a digital intermediate does. Off the neutral series
that estimate rests on an assumption the published data cannot test, and no
frame has been checked against a known scene (PROJECT.md, Known
limitations).
...
image dyes, using that stock's own traced dye set, and reading the LEDs through the stock's own traced orange mask, as
...
neutral, so that a neutral of the manufacturer's series decodes to a
reflectance of 0.18, and a three-by-three matrix, fitted on some 3,300 measured
...
colour cast. The datasheet's two charts of that neutral disagree with each
other, though, and the table follows the characteristic series; the
spectrum decodes off neutral by an amount PROJECT.md registers. And the
film's behaviour is taken from published data: the tables
diverge between stocks through their characteristic curves and spectral
sensitivities, which the datasheets publish separately for each; the dyes
agree family-wide, as PROJECT.md documents.
...
2. **Render the chart and examine it.** Some facts appear on the chart as
   words only.
...
> enters a fit as if it had been measured.** Where a spectral curve ends, its
> shape is not extrapolated; extrapolating a plausible tail is the standard
> route to a confident wrong answer. Every engine must still say something
> about the band it cannot see, so that claim is made as a bracket of the two
> admissible extremes carried through the chain with their spread recorded,
> or as a single labelled prior whose alternatives have been measured end to
> end, and every such sample is marked in the data file so a reader can
> exclude it (PROJECT.md, hard constraint 1). Tone
> curves are the one deliberate exception, the engines continuing a
> characteristic curve's terminal slope beyond its digitised span, and the shipped print cubes engage that
> extension on none of the printable window (PROJECT.md, register entry 16).
> The Vision3 scene tables, which invert a characteristic curve, do not
> extend it at all: the inverse lookup clamps at both
