...
A second finding is that the original Academy specification S-2008-002
publishes Status M to APD conversion matrices for 31 negative and intermediate
stocks, Vision3 5207 and 5219 among them, with residual errors. That is a
published quantity relating two metrics this project computes from the same dye
model; compared on each stock's neutral series, the model reproduces it to
0.018 D worst per-channel mean absolute disagreement on each stock's own dye
basis, within the project's 0.02 D threshold, and to 0.024 D from the shipped
family basis, reported and not gated (§4).
...
| **A, verified primary** | `ACEScsc.ADX16_to_ACES.ctl` and `ACEScsc.ADX10_to_ACES.ctl`, `aces-dev` v1.0.3; `CSC.Academy.ADX16_to_ACES.ctl`, `aces-input-and-colorspaces` (ACES 2, transform `a2.v1`) | Fetched; every constant read from the code. The ACES 2 file carries the same constants as the 1.0.3 file; the comparison was made against the `main` branch, whose latest commit touching that file is `c7f5fc3` (2025-09-26, from the repository's commit history), and the claim of identical constants is pinned to that revision |
...
- **The receiver is described without derivation.** The responsivities "are based on
...
The calculation is stated differently by edition, and the two forms are
equivalent up to the discretisation the later text makes explicit. S-2008-002
Equation 6, carried into ST 2065-2:2012 as Equation 2, is the continuous form:
...
T(λ) the spectral transmittance of the negative "or 10^−density(λ)", and a note
that summations are used in practice. ST 2065-2:2020 §4.1.3 states the discrete
form directly, on the responsivities tabulated in its companion file
`st2065-2a-2020.csv`:

```
APD_c = −log10 ( Σ_λ Π_λ,c T_λ  /  Π_sum,c ),   Π_sum,c = Σ_λ Π_λ,c
```

The tables run 360–730 nm at 2 nm in both editions (numerically identical,
densitometry note §6); §4.1.3 requires the transmittance to be measured "at
intervals no larger than 5 nm". The geometry is ISO 5-2 diffuse transmission in
both. For sample conditions both SMPTE editions refer to ISO 5-3 and print
no figures; the 23 ± 2 °C and 50 ± 5 % relative humidity quoted in the
Academy text are S-2008-002's.

Two notes matter to a scanning project.

- First, the 2020 edition's Note 2 says only that "A densitometer with spectral
  responsivities equal to the spectral responsivities of Π_APD could measure APD
  values directly"; the clause "no such densitometer, however, exists at the
  time of this writing" is the older Academy wording and is not in the 2020
  text.
- Second, Note 3, already quoted in the densitometry note: conversions from
  other metrics "are product specific: a separate transformation needs to be
...
stocks." The factors therefore implement the Academy's average balancing
convention: a fleet-average gain under which the ideal neutral of the quoted
passage lands on equal code values. A fixed average cannot ensure that for
every stock, exposure and process, and a real stock's neutral exposure need
not yield equal encoded densities across its scale.

The v1.1 revision history records a "Modification to ADX encoding gain factors"
between September 2009 and October 2010, so the printed values are the second
set. The ADX16 engine applies them verbatim (densitometry note §4b); they are
not a project trim.
...
One qualification travels with that quotation. A linear combination of
responsivities is a relation between spectral response channels, before the
logarithm; if normalised transmittance signals satisfy `t' = A t`, their
densities satisfy `D' = −log10(A · 10^−D)`, which is not in general `A D`.
Elementary example: mixing equal parts of two signals at densities 0 and 1
gives `−log10((1 + 0.1)/2) = 0.2596 D`, where the mean density is 0.5 D. A matrix
applied directly between density triplets, which is what Annex C fits, is a
material- and range-dependent approximation unless further conditions
establish it, which is one more reason to evaluate its residuals; the
Academy's fitted matrices remain useful for what they are.
...
scanner-density-to-APD transform is required.

What the ECN-2 route ships is narrower than that: the ADX16 cube computes APD
spectrally, bypassing Status M, but from the FAMILY-AVERAGE Vision3 dye set and
mask (`Vision3_dye_density.json`, `shared_full_curves`), one cube for the four
stocks. Per-stock dye models exist in the scene route and in the validation
instruments; they are not the ADX16 deliverable. The family cube is therefore
an approximation of the stock-specific transform the standard calls for, and
citing the standard's guidance does not make it stock-specific; the own-basis
against family-basis rows of the table below are the measure of what the
approximation costs on the neutral series, and the build's own family-mask
bound covers the dye box. The same section says the residual of a 3×3 can be
reduced by a polynomial or "nearly eliminate[d]" by a stock-specific 3D LUT
with shaper, which is the form the route ships, with the family caveat.
...
The 31 stocks are Kodak 5201, 5205, 5207, 5212, 5217, 5218, 5219, 5229, 5242,
5245, 5246, 5248, 5260, 5274, 5277, 5279 and 5299, and Fujifilm 8502, 8511,
8522, 8532, 8543, 8552, 8553, 8562, 8563, 8572, 8573, 8582, 8583 and 8592.
Vision3 50D (5203) and 200T (5213) post-date the document and are absent; 5201,
5205, 5212, 5217 and 5218 are their Vision2 predecessors.
...
Two features are worth reading before any use.

- The diagonal is not unity: APD red runs about 9–10 % steeper than Status M red
  and APD blue about 4–5 % flatter, with a green-into-blue term of 0.028 in both
  stocks.
- And the offsets are not zero; the largest is 5219's −0.047 D in red. The
  intercept alone does not state the two metrics' difference at D-min, which is
  the complete expression `M · StatusM_Dmin + offset − StatusM_Dmin`. Nor is a
  density-domain intercept a printer-light quantity: a printer light changes log
  exposure at the receiving material, and the conversion runs through
  transmission and the receiver's characteristic curve, so the process note's
  0.025 log exposure per light applies only with its original apparatus and
  material scope, and is not applied here.
...
uses, which is the kind of check Rule 4 asks for. The pass criterion is a
project threshold of 0.02 D on the per-channel mean absolute disagreement
over the traced neutral series; the figure is the expected error the document
states for its fitted transforms over 97 patches, 64 of them colours, so the
threshold is borrowed from a different population and the check is a
consistency criterion; it reproduces no Academy residual.
...
| Stock, basis            | mean (APD_spectral − APD_AnnexC), R G B | mean abs, R G B   | max abs, R G B    | Annex C's own mean abs error (all) |
|:------------------------|:----------------------------------------|:------------------|:------------------|:-----------------------------------|
| 250D, own dyes and mask | −0.014 +0.010 +0.003                    | 0.014 0.010 0.003 | 0.028 0.035 0.004 | 0.004 0.010 0.002                  |
| 250D, family basis      | −0.010 +0.009 +0.001                    | 0.010 0.009 0.001 | 0.023 0.031 0.003 |                                    |
| 500T, own dyes and mask | +0.018 +0.008 −0.002                    | 0.018 0.008 0.002 | 0.022 0.032 0.005 | 0.003 0.006 0.002                  |
| 500T, family basis      | +0.024 +0.012 +0.001                    | 0.024 0.012 0.001 | 0.032 0.037 0.002 |                                    |

The mean-absolute column is the statistic the pass criterion is stated on; the
signed mean beside it is the bias and cannot itself substantiate a
mean-absolute pass. The checker prints the worst per-channel mean absolute
value; the per-channel values above were read from it on 2026-09-12 (own
basis worst 0.0184 D, family basis worst 0.0239 D, unchanged from the
2026-09-03 run).
...
- **Green runs 0.003–0.006 D mean absolute below 2.1 D**, with maxima of
  0.015–0.019 D (strictly below: the first point at or above 2.1 D is a
  shoulder point and is excluded), and diverges at the shoulder, reaching
  +0.03 to +0.04 D at the top of the traced axis. Annex C's own maximum green
  residual is 0.032 (5219) and 0.053 (5207) over its 97 patches, a different
  population whose error locations are not published, so the shoulder
  discrepancy here overlaps the Academy's published maximum without the
  comparison being able to say where the Academy's error occurs, or whether
  the shoulder discrepancy comes from the model or from the affine
  approximation; being below an external maximum does not validate a
...
- **Red carries a near-constant offset**, −0.01 to −0.03 D for 250D and +0.01
  ...
  D-min term is real in both data sets. The correlation with the mask trace
  is an explanation to test: this comparison
  does not separate mask error from matrix-fit error and dye-model error,
  which would need a controlled comparison holding the other two fixed.

Overall the dye model agrees with the Academy's measured Status M to APD
relation to 0.001–0.018 D per-channel mean absolute disagreement on the
stocks' own bases, within the project's 0.02 D threshold, and to 0.024 D on
the family basis the ADX16 cube actually reads through, which exceeds it and
is reported without gating; the largest single-point disagreement is
0.037 D, in green at the shoulder on the family basis (0.035 D own). No channel
shows the 0.1 D class of disagreement a wrong dye reading or a
one-curve-per-layer misreading would produce.

Two caveats bound the pass.

- The ISO Status M table in `data/standards/` enters the check twice, in the
  neutral solve that fixes the dye amounts and in the Status M remeasurement,
  while the Annex C matrix is external and fixed; an error in the table
  therefore does NOT cancel, it changes the solved amounts and so the spectral
  APD while leaving the prediction untouched. Measured: shifting the Status M red
  responsivity by +1 nm in memory and re-solving keeps the neutral closure below
  5e-16 D and moves 250D's red mean disagreement from −0.0138 to −0.0168 D, a
  per-point change of up to 0.0086 D (*measured here*, 2026-09-09; a sensitivity
  test; it does not estimate any observer displacement). The shared table
  induces dependence and prevents a clean attribution of the residual between
  table, dyes and mask; the table's explicit entries are verified against the
  1995 edition of ISO 5-3 (densitometry note §0).
- And the Annex C patches were laser exposures on 2010 emulsions, measured by
  spectrophotometer, while the ramp here is the datasheet's traced neutral; the
  two neutrals need not lie on the same locus in dye space.

The check is `engine/ecn2/annexc_check.py`; the 31 matrices are held as
`data/standards/StatusM_to_APD_S-2008-002_AnnexC.json`.
...
matrices sums to 1.000 to four places, so equal encoded channel densities
decode to equal ACES values. That is a conditional algebraic result of the
decoder; whether a given stock's photographed neutral arrives at equal encoded
densities depends on the stock, exposure and process, and the fleet-average k
factors of §3 do not guarantee it without calibration.

Other properties, computed from the constants:

- With the full reference constants the curve is continuous at the junction;
  the displayed LUT is rounded: the LUT's last node is −0.92655 at 0.6 and the
  linear branch gives the same value there.
- Its slope in log exposure per unit of channel independent density is
  constant at 2.00 only between 0.3 and 0.5 (gamma 0.50); below 0.3 the LUT
  is a deliberately varying toe, 11.11 over 0.010–0.028, 7.69 over
  0.028–0.054, 4.88 over 0.054–0.095, 4.00 over 0.095–0.145, 2.67 over
  0.145–0.220 and 2.50 over 0.220–0.300; then 1.95 between 0.5 and 0.6, and
  1/0.55 = 1.818 above 0.6 (slopes from the rounded nodes as printed; the
  reference code carries the full constants, and continuity should be checked
  on those, never on this transcription). The Cineon document's decode
  used 1/0.6 throughout (§1).
...
**What the decode assumes.** What the code establishes is this much: two fixed
matrices, one fixed curve, and no stock parameter. The physical reading of each
constant is an INFERENCE from their positions in the chain, stated in
no fetched document: on that reading the `CDD_TO_CID` matrix removes one set
of print-film cross-sensitivities, the LUT inverts one characteristic curve,
and `EXP_TO_ACES` maps one set of camera-film spectral sensitivities to the
RICD. Which film or print film any of them represents is unnamed in the code
and in every fetched document (§6).

Clark's 2010 report states the design consequence plainly: "It is important to
note that ACES will preserve the aesthetic characteristics of film and other
...
judged against". The ADX16 cube lands a Vision3 scan on the APD the
family-average dye set and mask produce, the quantity ADX is defined to carry
up to the family approximation of §4; the Academy decode then inverts a generic
curve and a generic matrix. Every difference between a stock's real curve and
that generic one, and between its sensitivities and the generic `EXP_TO_ACES`,
survives into ACES as image content, which is what the Academy intends and what
"printer-light trims are dialled on a known neutral" in the README addresses.

The trims act in the CDD domain, before the matrix, so they are density offsets
in printing density. They are analogous to printer-light adjustments in their
effect on the exposure the decode reconstructs, but converting one into
physical light points requires a stated sign, scale and receiver response model,
and none is asserted here (§4).

The scene route inverts the stock's modelled characteristic curves and
sensitivities; it is an internal stock-model reference, since no measured check
in this project establishes that its output is the scene with the film's
rendering removed (PROJECT.md, Known limitations). `adx_validate.py` scores
the full chain through this decode (PROJECT.md).
...
- **The Annex C check** (§4) passes a project criterion of 0.02 D borrowed
  from a different population, on each stock's own basis (worst channel mean
  absolute 0.018 D, pointwise maximum 0.035 D); on the shipped family basis it
  reads 0.024 D mean absolute with a pointwise maximum of 0.037 D, exceeding
  the criterion and reported without gating. It runs on demand and is not
  part of the ADX16 build's self-report, since it reads per-stock
  characteristic curves the stock-blind build does not load.
- **The green shoulder.** The model's green discrepancy grows at its
  shoulder, to a magnitude that overlaps the Academy's published maximum
  residual over a different patch set whose error locations are unavailable.
  The comparison does not isolate whether the shoulder discrepancy comes from
  the model or from the affine approximation; a patch-level comparison would
  need the Academy's 97 spectra, which are not published.
...
- **ISO 5-2 and ISO 5-3:2009** remain unobtained; the Status M table's
  explicit entries are verified against the public 1995 edition (densitometry
  note §0).
