...
- Anchoring operates on camera-linear scanner data, ahead of the pre-shaper's
  ...
  engines integrate each LED behind the stock's D-min spectrum rather than
...
to 2.089, so the mask differs in spectral shape as well as in magnitude.
...
within 0.114 ΔE2000 for every stock pair tested, while 4–16% of cube nodes
...
for the convention, not for the illuminant.
...
Seven of them pin it – Portra 400, Portra 800, Ektar 100, Ultra Max 400,
Pro 400H and both Fujifilm stocks – and ten of the twelve rest on a bound of one
kind or another, so a pinned cyan shift is the constraint speaking rather than
a per-stock measurement, and is no evidence about the mask. See
`fit_audit.bounds_pinned` and register entry 12 in PROJECT.md.
