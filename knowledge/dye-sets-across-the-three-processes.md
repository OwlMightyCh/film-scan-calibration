...
- **Colour negative.** Literature "about 690 nm"; this project's shipped C-41
  cyan model peaks 699.9–713.3 nm (§4c). **These do not agree on eleven stocks,
  ...
  designed for on every stock but Pro 400H, whose fit lands at
  699.9 nm. On eleven stocks the shipped cyan is still rising at the 700 nm edge
...
The fleet summaries in §4a and §4c were recomputed on 2026-09-23 from the
...
|---|---|---|---|
| Mean | +21.50 | +5.22 | +6.01 |
| Minimum | +15.13 | +2.46 | +4.41 |
...
| Statistic | Cyan | Magenta | Yellow |
|---|---|---|---|
| Mean | +22.89 | +4.63 | +6.41 |
| Minimum | +14.88 | +2.17 | +3.87 |
| Maximum | +28.27 | +8.17 | +8.62 |
| Negative values | none | none | none |
...
chart; Pro 400H rests free at +15.53 with no parameter on a bound (§4d-ter). Pro
Image 100 sits at +24.9996, free of the bound, and reaches 28.27 nm; Ektar 100,
Ultra Max 400, Portra 400 and Portra 800 exceed 25 nm through the width alone. Fujifilm 200
and 400 are the exception in the other direction, pinning `sC` while `wC` rests
on 0.85, so their peaks move only 21.25 nm.

The smallest displacement in the fleet is Pro 400H's at 14.88 nm, then Fujicolor
100's at 17.40 nm. Eight of the twelve records carry at least one pinned
parameter, a cyan shift or a cyan width in every case: the magenta and yellow
widths run on 0.5–2.0, a range no stock reaches. See PROJECT.md register entry 12.
...
699.9–713.3 nm, **eleven beyond the grid entirely** and Pro 400H's at
699.9 nm, so on eleven stocks the fitted cyan curve is still rising at 700 nm
...
The experiment was run on an earlier fit population, whose RMSE column
predates the shipped fits, and the result is unambiguous:
...
bounds, under which two of the twelve stocks pin `sC` at +25.00 and eight of
...
| Stock | Withheld RMSE | In-band RMSE | Cyan peak, 400–700 fit → with tail | sC | In-band cost | Tail RMSE after |
|---|---|---|---|---|---|---|
| Fujifilm 200 / 400 | 0.0043 | 0.0119 | 706.3 → 706.3 nm | +25.00 pinned, stays | none | 0.0034 |
| Superia Premium 400 | 0.0070 | 0.0196 | 707.9 → 707.6 nm | +19.90 → +19.67 | none | 0.0038 |
| Fujicolor 100 | 0.0491 | 0.0207 | 702.4 → 699.0 nm | +15.13 → +12.33 | 0.0207 → 0.0216 | 0.0217 |
| Pro 400H | 0.0017 | 0.0105 | 700.0 → 699.9 nm | free, stays free | none | 0.0009 (training) |
...
- Where the withheld error is below the in-band error, the aggregate
  ...
  identified by that. Pro 400H is the strongest case, at 0.0017 D on 16
  samples the fit never saw.
- Where it is not, Fujicolor 100, the model overshoots the band by 0.045 D
  mean, the cyan placed too far red.

The same test sets the width bounds, which are per dye and uniform across the
fleet. Magenta and yellow absorb inside the measured band, so their widths run
on 0.5–2.0, a range no stock reaches. Held to 0.85–1.15, the magenta width pins
Pro 400H at 1.15 and moves its 400–700 fit into a second cyan basin, `sC`
pinned at +25.00 and the peak at 713.7 nm, which misses the withheld tail by
0.0525 D. The cyan width keeps 0.85–1.15 as an extrapolation guard: released,
it makes the withheld tail predict worse on the two stocks where it moves the
fit, Superia Premium 400 0.0070 → 0.0518 D and Fujicolor 100 0.0491 → 0.0700 D.
Fujicolor 100's tail residual after refit (0.022) stays at the in-band level,
which suggests a mismatch of the warped form at the turnover; whether the form
can describe the true spectrum waits on the source-uncertainty analysis.
...
cyan peak within 0.5 nm on Fujifilm 200/400, 3–7 nm on seven further stocks and
12–16 nm on three (Portra 160, Portra 800, Pro Image 100) at 5% residual
tolerance; the same three move 10–16 nm across the four individual Vision3
stock bases and the family average, the other nine under 3 nm: ambiguity across
every chart–basis combination tested. Status M density of the reconstructed
midscale spreads at most 0.014 D (red) across any of those sets, which is
closure of the sum, a range across reconstructed midscales.
...
deviates Status M on unequal-amount inputs by up to 0.100 D in green (Portra
160), 0.05–0.06 D on Portra 800, Superia Premium 400 and Pro Image 100, and
0.004–0.04 D on the rest; equal-amount inputs (not a photographed neutral, the
curves being independently normalised) by 0.002–0.051 D. The two are reported
side by side.

The Pro 400H tail fit is the shipped fit (registry `fit_traced_tail`; the tail
enters as extra residual terms, the curves stay on 400–700 nm). Against the
400–700 fit it moves no dye curve by more than 0.0009 D and the reconstructed
midscale's Status M by 0.0001 D, so the admission is measured data confirming
the fit rather than steering it.
...
| Ektar 100 | 0.6711 | at or beyond 700 nm |
| Ektachrome E100 | 0.908 (of a 0.948 peak) | 658 nm |

(Read 2026-09-23 from the shipped peak-normalised `shared_full_curves` cyan
arrays, the multistart fits under the per-dye width bounds; the read of
2026-09-09 gave 0.6616 and 0.6713, that of 2026-09-02 0.649 and 0.650, and the single-start fits of
...
Across the fleet the fitted cyan peak displacement spans 13 nm (14.9–28.3 nm,
§4a).
...
| Quantity | Magnitude |
|---|---|
| Basis sensitivity of the dye decomposition | 0.027–0.049 D |
