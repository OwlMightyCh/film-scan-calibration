...
**Headline finding: the red-shift is not a cyan phenomenon.** All thirty fitted
displacements across the ten-stock fleet, three dyes each, are positive without
...
- **Colour negative.** Literature "about 690 nm"; this project's fitted C-41
  cyan peaks 702.4–713.8 nm. **These no longer agree, and the disagreement
  grew when the fit improved.** Under the earlier single-start solve the
  modelled peaks ran 691.8–708.2 nm and the agreement with Bergthaller's design
  target was cited here as corroboration. The multistart finds optima 8 to 13 nm
  further red, so the fitted peak now sits 12 to 24 nm past the wavelength the
  ...
  extrapolation in all eleven cases and the discrepancy is as likely to be an
...
seven of ten stocks that peak sits at the grid edge and is an extrapolation.
...
| Statistic | Cyan | Magenta | Yellow |
|---|---|---|---|
| Mean | +25.14 | +6.06 | +7.19 |
| Minimum | +17.40 | +3.74 | +3.69 |
| Maximum | +28.75 | +8.17 | +10.82 |
| Negative values | none | none | none |

**Most stocks now move their cyan peak FURTHER than the 25 nm the bound
nominally permits**, because the peak lands at `p + s·w` and `wC` rests on the
1.15 ceiling for several stocks: six of the eleven pin `sC` at +25.00 and four
...
**Thirty shifts out of thirty are positive.** Register entry 8 describes this for
...
For all eleven C-41 stocks the fitted cyan argmax now lands exactly on 700 nm,
the last point of the output grid. Adding `peak_shift_nm` to the basis peak of
685 nm places the modelled peaks at 702.4–713.8 nm, **all eleven beyond the grid
entirely**, and for **all eleven** the fitted cyan curve is still rising at
700 nm, so the peak is not represented in the shipped curve at any stock. This
became universal when the decomposition moved from a single start to a
multistart: under the single start the modelled peaks ran 691.8–708.2 nm with
three beyond the grid and seven still rising. The better-fitting optimum is the
one that sits further outside the measured data. For Ektar 100 the 700 nm grid edge already lies 12.1 nm beyond the
...
This produces a genuine mismatch. **Nine of the ten stocks record an `endpoints`
...
| Portra 400 / 160 / Ultra Max 400 / Gold 200 | 700.0 nm | 702.6–703.1 nm | +2.6 to +3.1 nm |
...
For the other nine the intersection with a 700 nm `GRID` is the only thing
...
`GRID` happens to stop at 700 nm. Anyone who raises that ceiling, which the
earlier draft of this note proposed doing, silently fits fabricated data and the
fit degrades by roughly an order of magnitude. The safe form of the change is to
...
**The two stocks that pin the cyan shift bound are completely insensitive to
it.** Three further stocks pin a width bound instead – Fujicolor 100 and
Pro 400H at `wM` = 1.15, Superia Premium 400 at `wC` = 1.15 – so five of the ten
fit against a bound of one kind or another. See `fit_audit.bounds_pinned` and
PROJECT.md register entry 12.
Fujifilm 400 and 200 sit at +25.00 at every cut, so the missing 17.5 nm would
...
stock's digitised support rather than the range fitted. Nine of the ten stocks
...
|---|---|---|
| Portra 400 | 0.645 | at or beyond 700 nm |
| Ektar 100 | 0.671 | at or beyond 700 nm |
| Ektachrome E100 | 0.908 (of a 0.949 peak) | 658 nm |

For the negative stocks the red probe sits on the steep rising flank of the cyan
absorption, where the local slope is approximately **+0.0094 per nm**. A 10 nm
error in the fitted cyan peak position therefore moves the modelled cyan density
at the red LED by about 0.094 in peak-normalised units, close to a tenth of
peak. Across the fleet the fitted cyan shift spans 18 nm.
...
Measured from each stock's own `_datasheet_curves.json` on 2026-08-16, using
...
merely the scale: the ratio (B−R)/(G−R) runs from 1.414 on Portra 160 to 2.049
on Ultra Max 400, so the mask's spectral profile is itself a per-stock property.

The ordering is coherent with Bergthaller's account. The professional Kodak
stocks carry the weakest masks, Portra 400 at 0.623, Portra 160 at 0.638 and
Ektar 100 at 0.652, while the consumer and older formulations carry the
strongest, Gold 200 at 0.738, Ultra Max 400 at 0.753 and Fujifilm 200/400 at
0.947. That is the pattern expected if premium films have moved to
...
`PrintEmulationEngine`, presented as `endura_print_engine.py` for the five Kodak
stocks on Endura Premier and `fuji_print_engine.py` for the five Fujifilm stocks
...
with (B−R)/(G−R) running 1.414 to 2.049, is real and is exactly that kind of
...
differs by at most 0.114 ΔE2000, and colours below C\*ab 5 differ by a median of
exactly zero. Neutral rendering is stock-independent by construction, which is
what a printer's colour balance does to a mask in practice.

**Off the neutral axis the mask survives.** For realistic stock pairs 10–16% of
nodes exceed 1 ΔE2000, reaching 4.1–4.5 ΔE2000. The worst node is consistently
at Dnorm [0, 0.208, 1.0], L\* ≈ 50 and C\*ab ≈ 61, a reachable saturated colour
of middling lightness rather than a degenerate corner.
...
0.173 D with essentially no change of spectral shape, and yields 3.7% of nodes
above 1 ΔE and a maximum of 2.33. Fujicolor 100 → Superia Premium 400 changes
strength by 0.009 D, **nineteen times less**, while changing shape by as much as
the extreme pair, and yields 9.9% and a maximum of 4.11. A mask difference with
...
The vocabulary for what this project does exists in the literature and the
repository does not currently use it.
