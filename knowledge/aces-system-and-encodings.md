...
procedure written for cameras is silent on scanners.**

Every quantity the project hands to ACES is therefore judged by the ACES
definitions in §2 and by the decode described in the companion note; the
project chooses none of these.
...
| **A, primary organisational documentation, explanatory** | docs.acescentral.com, "Derivation of the ACES White Point" | Fetched 2026-09-12; its discussion and Table 2 are the source of the white-point clarification in §2 |
...
(part 4); the editions read here are 2021, 2020, 2020 and 2023.

The docs site describes ACES 2 as bringing "New Output Transforms for rendering
ACES images to displays", with "A less aggressive tone scale with reduced
mid-tone contrast and a gentler highlight rolloff" and "Robust gamut mapping for
improved perceptual uniformity and reduced clipping artifacts", and lists no
change to the core encoding or to ADX. The ADX-to-ACES transform shipped with
ACES 2 carries the same constants as the 1.0.3 file (companion note §5).

The ACES 1 vocabulary, in the pipeline's order, from TB-2014-002's engineering block
...
- **LMT, Look Modification Transform**: "imparts an image-wide creative 'look'
  ...
  unrestricted."
- **RRT, Reference Rendering Transform** (an ACES 1 architecture term, as is
  ODT below; ACES 2 replaces the pair, see Output Transform): "Converts the scene-referred
...
The white point is exactly x = 0.32168, y = 0.33767. S-2008-001 called it the
chromaticity of "CIE Standard Illuminant D60", and that older wording is kept
here only as an attributed quotation: the Academy's own derivation page states
that the specified chromaticity "differs from both the chromaticity coordinates
of CIE daylight with a CCT of 6003 K and CIE daylight with a CCT of 6000 K",
its Table 2 giving those as (0.32163, 0.33774) and (0.32169, 0.33780) against
the ACES (0.32168, 0.33767), and that the point lies on neither the Planckian
nor the daylight locus. "D60-like" is the accurate description; use the
specified coordinates.

"A color encoded in ACES is considered neutral if its RGB values are equal."
The green and blue primaries lie outside the spectrum locus, so the triangle
holds every real colour, and Annex B notes that valid ACES values "include those
with one or more negative ACES color component values".
...
The RICD, the Reference Image Capture Device, is a virtual camera whose spectral
sensitivities the 2021 edition moved into a normative Annex A and "changed from
a list of tabulated values to a mathematical formula". Its scale is fixed by
flare and a grey card (Annex A):
...
- **ACEScg** (S-2014-004): linear on AP1, range [−65504.0, +65504.0], 16- or
  ...
  don't support ACES2065 or ACEScc" (TB-2014-012). Scene-linear on AP1 with
  the shared white point; its range and representation constraints are its
  own, so it is not ACES2065-1 with different primaries.
...
- **ACEScct** (S-2016-001 §1.1), the same log with a linear toe:
  ...
  in Arrighetti's phrase; the breakpoint's domain in the specification is
  linear AP1 input, as the formulas above show.
...
The AP0-to-AP1 conversion preserves the linear mid-grey of §2: the rows of TRA_1 sum
to 1.0000, so 0.18 in ACES2065-1 remains 0.18 on the AP1 neutral axis, which
is its value in linear ACEScg; ACEScc and ACEScct, the two log encodings,
place it at (log2(0.18) + 9.72) / 17.52 = 0.4136, and integer ACESproxy at
its own code value.
...
The docs site lists "Recommendations for using compression in ACES OpenEXRs"
among ACES 2 changes; that is a workflow recommendation, and the container
standard's §8.19 as fetched requires no compression.
...
may not be colorimetric" (§1.1).

The scale rule is the grey card again: values "are uniformly scaled such that a
spectrally neutral 18% reflector captured under the scene adopted white would
map to ACES RGB relative exposure values of [0.18, 0.18, 0.18]" (§5.6). Vendors
are asked for two IDTs, "one optimized for CIE Illuminant D55 (daylight) and a
second optimized for the ISO 7589 Studio Tungsten illuminant".

The matrix is fitted by non-linear least squares over training spectra with a
colour-appearance cost, and Clark's 2010 study applied exactly this procedure
to a broadcast camera, reaching average ΔE00 of 1.6–1.9 on the training set and
2.3–2.7 on a real chart.
...
when put through the RRT and ODT for film projection."

The ST 2065-1 text dropped §5 entirely; the SMPTE standard defines the encoding
and leaves entry to the Academy's transforms. The "appropriate IDT" for APD is
the universal unbuild, examined in the companion note.
...
- **The ECN-2 ADX16 cube is an ST 2065-3 encoder** and no IDT. It emits
...
- **The scene-referred ECN-2 route lands on DaVinci Wide Gamut**, short of
...
- **The C-41 print route does not enter ACES as shipped.** It is
  display-referred by design (Display P3 output), so it is a finished picture
  in ACES terms; an ACES production can still take that rendered image in
  under an explicitly chosen treatment, as it would any delivered output, and
...
- docs.acescentral.com, *Derivation of the ACES White Point* – https://docs.acescentral.com/white-point/ (tier A, Academy explanatory documentation, fetched 2026-09-12; discussion and Table 2 used in §2)
