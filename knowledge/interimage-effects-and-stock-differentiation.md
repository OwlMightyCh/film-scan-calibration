...
Source: the IS&T 1997 paper (collected 2026-08-16).
...
1. **Interimage is a plausible contributor to the physical differences between
   ...
   §4a) is the one inter-stock difference traced into a cube.
...
5. **The basis-sensitivity measurement does not exonerate the basis.** Measured
   from the shipped dye data, basis sensitivity is 0.027–0.049 D against
   inter-stock distances of 0.021–0.219 D. The model separates some pairs under
   this distance criterion: 16 of the 66 pairs sit within the band on the
   shipped fits (recomputed 2026-09-23 with `c41_stock_compare.py`), and the
   count moves with the dye-fit state, so it is re-derived before each use.

   The remedy identified is MEASURED per-layer dye data, which requires suitable
   independent spectral characterisation; colour-separation wedges are one
   useful measurement design and no guarantee of chemically pure per-layer data.
   This reinforces item 2 unconditionally.
...
- The IS&T 1997 paper is read in §4a. It contains a
