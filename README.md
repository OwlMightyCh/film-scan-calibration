...
> over a CuteNew RGB Pro Light Source, a narrowband RGB LED source named
> here so that the setup can be reproduced (this project has no affiliation
> with, and no sponsorship from, its maker), and none of its output has been
> checked against a physical reference measurement. Please read
...
The print rendering is a deliberate choice, and as
[Limitations](#limitations) sets out, the print stage has no verified
reference. These frames are shown as delivered, so they carry the author's own `Print Adjustment` trim,
...
- **Motion picture (ECN-2)**: the Kodak Vision3 family, 50D, 250D, 200T and
  ...
  sensitivities, a per-stock model estimate of the scene exposure and the
  path for graded work.
...
A frame is captured as three exposures: the light source's
...
file contains, ignoring its extension, and refuses a file whose structure
...
Both converters refuse a triplet whose captures differ in size, since a
mixed capture would otherwise merge into a
plausible but misregistered frame, and refuse a sequence that is not a whole
number of triplets, or whose frame counters show a capture repeated, since
every frame after such a fault would merge the wrong channels. A skipped
counter is only warned about, with the triplet assignment printed for
confirmation, since a capture deleted by hand leaves the same gap as one that
never landed.
They and the anchor tool's numeric core are
...
published tables come from; a response tilting by 1% per 10 nm is a scenario, measured on no
sensor, and costs under 0.008 D up to a
dye density of 1.8.
...
The light source on this apparatus is the CuteNew RGB Pro Light Source
([cutenewdesign.com](https://cutenewdesign.com)), a narrowband RGB LED panel
whose red, green and blue channels fire one at a time; the engines read its
measured spectrum, and the name appears here only so that the
apparatus can be reproduced. This project has no affiliation with, and no
sponsorship from, its maker, and the figures quoted here are properties of
the one unit measured.
...
materially worse residual after a substitution points at the new inputs. The per-roll anchoring step is unchanged and
...
- **Their orange masks do differ**, by 0.346 D across the twelve stocks, read
  ...
  by up to 5.5 ΔE2000. The print tables are metrically sound prints whose
  per-stock signature lies in saturated colour; a grey card cannot
  distinguish them.
- **Interimage effects are not modelled.** They arise during development, so a
  ...
  datasheet data. Reversal film relies on
...
- **The print branch renders an idealised print, by design.** It reproduces
  ...
  nominal.
- **The tables embed this light source**, which ties them to one
  apparatus. They name no camera: they are built for a
...
- **A Bayer camera departs from them by an amount that grows with density.**
  ...
  0.032–0.063 D. No second physical camera has been used to scan film through
  the chain.
- **The reversal tables lose accuracy in the deepest shadows.** Sampled against
  the full chain under the tetrahedral lookup the Resolve setup requires,
  they hold to a maximum error of 0.001 D up to a dye density of 2.0 and
  0.006 D up to 2.5, then degrade to 0.30 D by 3.4. A monochrome
  ...
  reaches, so the table holds the nearest colour the scanner model closes on
  instead, which is not always one the film can produce (a share of those
  solutions carries a negative dye amount; PROJECT.md gives the counts).
  Lowering the corridor from 6.0 to 5.0 D recovers part of it; the
  remainder is not recoverable by any corridor. A build named to a specific
  camera does not have this limitation, holding to 0.0005 D throughout on the
  a7R III and to 0.0006 D on the GFX 100 except Velvia 50 at 0.0021 D. The
...
- **The Vision3 scene tables are a model estimate, unvalidated off the
  neutral series, and lose precision in deep shadows and at the top of a
  layer's published curve.** The datasheet's neutral series fixes one path
  through the film's response to colour, and the tables assume the simplest
  rule off that path; a different rule reproduces the same neutral and
  decodes colours differently, and only separation exposures on a measured
  roll can choose between them. Their output is linear exposure, stored on
  a lattice that is uniform in density, so interpolation departs from the
  exact chain, under the tetrahedral lookup the Resolve setup requires, by
  1.6–1.9% on average within each stock's published characteristic span;
  80–85% of that span lies in lattice cells whose corners all converged and
  sit within the published curves, at 1.5–1.7% mean and 7–8% at the 99th
  percentile, the cells that straddle a curve's shoulder carry the tail, to
  48%, and a ColorChecker four stops under nominal exposure reads up to
  22–97% off in its worst channel. Beyond either end
  of a published curve the inverse clamps, so no
  exposure the sheet never documented is synthesised. Those tails are
  lattice error, reducible by finer sampling, sitting
  ...
  mid-grey, and reports it separately; the table itself carries no mark of
  it. The colour matrix behind them is fitted on measured reflectances to
  2.2–2.6 ΔE2000 mean on its own training set, and no frame has been
...
determination of what counts as evidence are the author's. The light source
is a CuteNew RGB Pro Light Source, bought at retail; its maker had no part in
this work and no affiliation or sponsorship exists.
