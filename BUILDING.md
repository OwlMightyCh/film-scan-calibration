...
Those forty-five are the cubes recorded in `builds/cube_manifest.json`.
...
Status M residual when everything is consistent. Four further lines: `corridor`
reads the sheet's neutral series (three Status M records solved together
through the spectral base) and the equal-dye heuristic box into scan density
against the corridor, neither being the film's gamut; `neutral offset` is the
image-dye Status M departure of that series from the diagonal at k = 0.22,
which the cube SUBTRACTS so the series lies on the diagonal (the print engines
add it back and check the header), with the share of lattice nodes that clip
at zero because a channel's image-dye density sits under the offset; the `LUT
65^3` line and the serialised-artifact line print the probe under both lookups,
tetrahedral first (the scheme the Resolve setup requires), the serialised
figure over the samples above the output floor with the floor share beside
it; and the
`quality-control tap` line gives the image-dye reading, the full reading a
densitometer sees, and how far adding integrated D-min misses it.
...
Outputs land in `builds/c41/print_endura/` and `builds/c41/print_fuji/`. The
engine refuses to run against a Status M cube whose header carries a
different neutral offset from its own. `neutral locus` reports the offset
added back to the input and how the sheet's neutral series prints through
the lock over k = 0.22 ± 0.12 and over the whole calibration span (its own
curvature, never fitted), then the sheet's midscale-neutral spectrum placed at
Y = 0.18 by a common offset, the other end of the neutral bracket.
`balanced-encoding floor loss` measures what the Status M cube's floor
clipped, exact render before against after the clip on a uniform dye probe:
the share below the floor, the mean on that share and the maximum. Two more
lines carry the print model's health. `paper tables`
...
The validator samples BOTH delivered cubes, the ADX16 cube and each stock's
scene cube, with the tetrahedral lookup the Resolve setup requires, checks
each cube's header against the requested stock and sensor and its eight
corners against their nodes under both lookups, and scores the decoded ADX
result against the scene cube's; the exact engine without its cube is printed
beside the scene cube as a diagnostic, and a trilinear line for the
ColorChecker set only. Read its dE2000 lines. In Resolve the ADX16 cube is followed by "Printer
...
quantity), the share of each Status M channel's weight that the midscale
trace does not cover (a warning above 5%: that channel's anchor is a
partial-band density, 500T red), its matrix-fit residuals with the weighted
residual of the constrained solve beside that of an unconstrained fit
rescaled by row (the build asserts the constrained one is never higher), the
neutral amount tables' closure on all
three characteristic curves, the neutral-axis exposure ramp, the corridor
requirement, a LUT-versus-exact-chain probe under both lookups,
tetrahedral (the scheme the Resolve setup requires, and the figure the docs
quote) and trilinear beside it, the tetrahedral figure split
...
region, a ColorChecker exposure sweep (the same colours at −4 to +2 stops,
retained where the exact inverse is unclamped and the scan lies inside the
cube: worst-channel error of the delivered lookup and how far a doubling of
exposure fails to double its output; read it for the shadows the uniform
dye-box probe under-samples), and an endpoint assertion that the inverse
lookup clamps
...
Four builds, selected by a positional name.
...
**The reversal corridor**). Before the sheet checks the build prints the
model-side self-report: `tail bracket` gives the D50 observer weight the
support-derived grid omits at each end and, on a neutral stack at dye 1.0,
2.0 and 2.5, how far each admissible tail policy (blocked or held below the
grid, held or clear above it) sits from the truncated observer the cube
ships, the bracket hard constraint 1 requires (register #2; on Provia the
cyan is measured to zero at the edge, so held and clear all but coincide
and the truncation sits outside the pair); `reachable nodes` carries the count of
reached nodes that close only with a negative dye amount, kept and counted
because convergence is not admissibility; `serialized` prints the LUT-versus-
exact-chain probe under BOTH lookups, tetrahedral (the scheme the Resolve
setup requires, and the figure the docs quote) and trilinear, since neither
bounds the other (one implementation for every engine, `engine/common/lut.py`),
and a second line for the corridor's corners and faces
over the samples the film reaches. Each build then ends with lines that are
checks against the sheet, independent of the model. `sheet closure` reads the
...
`base surrogate re-read` reads the surrogate base back through the Status A
responsivities and must reproduce the sheet's D-min triplet (within 0.001 D
on every stock); `base term` then gives the roll anchor's integrated D-min
subtraction error under that one surrogate tint, a sensitivity and never a
bound over other base spectra (register #17). The `written cube on
the sheet neutral` line is plumbing only.
...
per-camera fleet is twenty-one cubes.
...
directory, and does not fail on them
...
A wider corridor is numerical headroom for the lattice, nothing more: it says
nothing about whether the scanner reads those densities linearly or above
its stray light, which only a transmission wedge can establish.
...
lattice: an off-neutral stack can read
