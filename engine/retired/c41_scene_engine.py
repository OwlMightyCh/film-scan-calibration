...
this file produced were deleted; git history keeps them.
...
and is NOT duplicated anywhere else in the repo:
  ...
  * the ColorChecker dE2000 harness, whose "full chain" figure is NOT
    independent evidence about the film model and must not be read as such.
    L_to_statusm_norm followed by statusm_norm_to_L is an exact self-inverse,
    so the full-chain residual equals the matrix-only residual to every digit
    printed, and that matrix was fitted on the same 24 patches. Measured
    directly: both report mean 2.460 / max 6.323, and both were UNCHANGED by a
    refit that moved every stock's dye set. The saturated-red dE 6.3 is
    therefore a property of the 3x3 fit on red, not a demonstrated
    forward-model limit. Testing the dye model this way needs an inverse the
    forward model did not supply
