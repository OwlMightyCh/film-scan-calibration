...
def project_to_reachable(dye_nodes, residual, size,
    ...
    amounts reproduces the node to within `tolerance`. Reached is numerical
    convergence, not physical admissibility: the solve admits negative dye
    amounts, and a reached node may close only with one (the build counts
    them). The substitution is made in dye space, before the target
    integration, so every value written to the cube is the colorimetric
    density of a dye solution the scanner model closes on, the nearest such
    lattice node to an unreached one (nearest on the lattice, not on a
    continuous boundary). Reached nodes are returned unchanged, so the
    projection never moves a colour the solve closed on.
