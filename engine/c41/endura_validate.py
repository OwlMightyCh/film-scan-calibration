...
Tn, _ = T_neg_of(KS_B2)
# Two exposure kernels, deliberately kept apart. The engine integrates against
# EXP_W, which is SENS_P * L_enl already masked to the band where the negative's
# spectra are measured; the unmasked product is what that mask throws away. A
# check written against the unmasked kernel measures a configuration the engine
# does not ship, and cannot see the mask working (or failing) at all.
W_UNMASKED = eng.SENS_P * eng.L_enl             # (3, Nc) what the mask prevents
W_SHIPPED = eng.EXP_W                           # (3, Nc) what the engine uses
lo_band = CG < 400.0
hi_band = CG > 700.0
def _oos_fracs(weights_row):
    """(lo, hi) out-of-support exposure fractions for one layer's weights."""
    contrib = Tn * weights_row[None, :]         # (nk, Nc)
    tot = contrib.sum(1)
    return (contrib[:, lo_band].sum(1) / tot, contrib[:, hi_band].sum(1) / tot)


oos_shipped_max = 0.0
for i, l in enumerate(LAYERS):
    flo_u, fhi_u = _oos_fracs(W_UNMASKED[i])
    flo_s, fhi_s = _oos_fracs(W_SHIPPED[i])
    oos_shipped_max = max(oos_shipped_max, float(flo_s.max()), float(fhi_s.max()))
    print("B2: %-7s UNMASKED  380-400nm frac %s ; 700-730nm frac %s   (k=0.05/0.22/0.55)"
          % (l, "/".join(f(x, 4) for x in flo_u), "/".join(f(x, 4) for x in fhi_u)))
    print("B2: %-7s SHIPPED   380-400nm frac %s ; 700-730nm frac %s   (k=0.05/0.22/0.55)"
          % (l, "/".join(f(x, 4) for x in flo_s), "/".join(f(x, 4) for x in fhi_s)))
# a-priori, not a tuned threshold: exp_support zeroes those bands by construction,
# so anything but exactly zero means the mask is broken.
print("B2: max out-of-support fraction under the SHIPPED kernel = %s%s"
      % (f(oos_shipped_max, 6), verdict("B2", oos_shipped_max == 0.0)))
...
def logE_raw_of(ks, dyes_neg, dmin_vec, weights):
    """Raw paper log-exposure for an arbitrary edge treatment x exposure kernel."""
    ks = np.atleast_1d(np.asarray(ks, float))
    D_od = np.repeat(ks[:, None], 3, axis=1) * eng.cfg.dmax_input
    dye = eng.invert_statusm(D_od)
    N = dmin_vec + dye @ dyes_neg
    Eh = (10.0 ** (-N)) @ weights.T
    return np.log10(np.clip(Eh, 1e-30, None))


KS_B3 = np.array([0.05, 0.55])
# shipped configuration: zero-filled negative, masked kernel
raw_ship = logE_raw_of(KS_B3, eng.DYE_neg_C, eng.dmin_spec_C, W_SHIPPED)
# edge treatment alone -- both sides masked, only the fill differs
raw_held_s = logE_raw_of(KS_B3, DYE_neg_held, dmin_held, W_SHIPPED)
# the mask's own effect -- both sides zero-filled, only the kernel differs
raw_zf_u = logE_raw_of(KS_B3, eng.DYE_neg_C, eng.dmin_spec_C, W_UNMASKED)


def _dk(a, b):
    return ((a[1] - a[0]) - (b[1] - b[0])) / (KS_B3[1] - KS_B3[0])


d_edge = raw_held_s - raw_ship
s_edge = _dk(raw_held_s, raw_ship)
d_mask = raw_zf_u - raw_ship
s_mask = _dk(raw_zf_u, raw_ship)
note("B3")
for i, l in enumerate(LAYERS):
    print("B3: %-7s EDGE (held vs zero-fill, both masked)  dlogE@k=0.05 %s  @k=0.55 %s  d(dlogE/dk) %s"
          % (l, f(d_edge[0, i], 4), f(d_edge[1, i], 4), f(s_edge[i], 4)))
for i, l in enumerate(LAYERS):
    print("B3: %-7s MASK (unmasked vs masked, both zero-fill)  dlogE@k=0.05 %s  @k=0.55 %s  d(dlogE/dk) %s"
          % (l, f(d_mask[0, i], 4), f(d_mask[1, i], 4), f(s_mask[i], 4)))
...
W_trunc = W_UNMASKED.copy()
W_trunc[:, lo_band] = 0.0
W_trunc[:, hi_band] = 0.0
treat = [("A zero-fill, unmasked (not shipped)", eng.DYE_neg_C, eng.dmin_spec_C, W_UNMASKED),
         ("B held-neg,  unmasked (not shipped)", DYE_neg_held, dmin_held, W_UNMASKED),
         ("C zero-fill, trunc-int  [SHIPPED]  ", eng.DYE_neg_C, eng.dmin_spec_C, W_trunc),
         ("D held-neg,  trunc-int (not shipped)", DYE_neg_held, dmin_held, W_trunc)]
note("B5")
...
print("VERDICTED: %d of %d checks carried a pass criterion"
      % (len(VERDICTED), len(VERDICTED) + len(NO_VERDICT)))
