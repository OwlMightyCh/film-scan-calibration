...
# A curves file without a digitization_audit.spectral_dye_density.endpoints
# block has no measured support, so the fit would run over the whole grid,
# fabricated edges included. That is refused for a canonical fit; the flag
# below admits it for an EXPLORATORY run only, and requires --out-suffix so
# the result cannot land on shipped data.
_bp.add_argument("--allow-unaudited-support", action="store_true",
                 help="run the fit over the whole grid when the curves file carries no "
                      "measured-support audit block (exploratory; requires --out-suffix)")
...
ALLOW_UNAUDITED = _ba.allow_unaudited_support
if ALLOW_UNAUDITED and not OUT_SUFFIX:
    raise SystemExit("--allow-unaudited-support is exploratory and requires --out-suffix")
...
if BASIS_NAME == "parametric":
    ...
    for _k in ("cyan", "magenta", "yellow"):
        ...
        _m = ~np.isnan(_v)
        basis_support[_k] = (float(_w[_m].min()), float(_w[_m].max()))
        basis[_k] = np.interp(bw, _w[_m], _v[_m], left=_v[_m][0], right=_v[_m][-1])
if BASIS_NAME == "parametric":
    basis_support = {k: (float(bw[0]), float(bw[-1])) for k in basis}
...
def warp(name, s, w, grid=GRID):
    """Basis dye 'name' interpolated onto lambda_peak + (l-lambda_peak)/w - s.

    The basis is measured on basis_support[name] (402-798 nm for the Vision3
    family) and held flat outside it, so a red shift or a narrowing pulls the
    blue end of the warped curve onto that held value: a cyan at +25 nm reads
    the basis below 402 nm for every grid wavelength under ~427 nm, and at
    width 0.85 under ~465 nm. The chart holds no data there, so no other value
    is available; the fit records which output samples rest on the hold
    (fit_audit.basis_held) rather than pretend they were traced."""
    p = peak_wl[name]
    src = p + (grid - p) / w - s
    v = basis[name]
...
def warp_held(name, s, w):
    """Boolean mask over GRID of samples whose warp source lies outside the
    basis's measured support (they carry the held edge value)."""
    p = peak_wl[name]
    src = p + (GRID - p) / w - s
    lo, hi = basis_support[name]
...
try:
    ...
    # The audit endpoints describe the traced chart, whose red end runs past
    # the stored array on eleven of twelve stocks (to 719 nm on Superia
    # Premium 400 against an array ending at 700). Beyond the array `resample`
    # zero-fills, so the fit support is the INTERSECTION of the audit span
    # with the stored array's span: were the grid ever widened, no zero-filled
    # sample could enter the objective as a measurement.
    _lo = max(_lo, float(swl.min()))
    _hi = min(_hi, float(swl.max()))
    _support = (_lo, _hi)
    _fit_mask = (GRID >= _lo) & (GRID <= _hi)
    if not _fit_mask.any():
        raise SystemExit("measured support %s lies outside the fit grid" % (_support,))
except (KeyError, TypeError) as _exc:
    # A missing or malformed audit block means the measured support is
    # unknown, and a fit over the whole grid would be driven by fabricated
    # edges while reporting a residual that looks no different from a clean
    # one. A canonical fit therefore refuses to run. An exploratory run may
    # proceed under --allow-unaudited-support (which requires --out-suffix),
    # and its output is labelled as unaudited in the report and in the file.
    _msg = ("%s carries no usable digitization_audit.spectral_dye_density.endpoints "
            "block (%s: %s), so the measured support is unknown"
            % (STOCK["curves_json"], type(_exc).__name__, _exc))
    if not ALLOW_UNAUDITED:
        raise SystemExit("REFUSED: " + _msg + "; a canonical fit never runs over "
                         "fabricated edges. Recover the audit block, or pass "
                         "--allow-unaudited-support with --out-suffix for an exploratory run.")
    print("WARNING (exploratory, unaudited support): " + _msg + "; the fit runs over "
          "the WHOLE %.0f-%.0f nm grid, fabricated edges included." % (GRID[0], GRID[-1]))


# ---------- the traced red tail, where the registry admits it ----------
# The Fujifilm sheets trace the dye-density chart to 716-719 nm; the digitiser
# keeps that tail in `spectral_full_support` beside the 400-700 arrays every
# consumer reads. A stock whose registry entry sets `fit_traced_tail` fits
# those samples too, as extra residual terms on their own wavelengths: the
# output curves, the Status M integral and PHI stay on the 400-700 grid. The
# admission is per stock because it is measured data, not a prior (Invariant
# 7); which stocks carry it, and why, is stated in portra_stocks.py.
_tail_wl = _tail_agg = None
if STOCK.get("fit_traced_tail"):
    _fs = dj.get("spectral_full_support")
    if not _fs:
        raise SystemExit("%s sets fit_traced_tail but %s carries no "
                         "spectral_full_support block" % (STOCK["speed_key"], STOCK["curves_json"]))
    _fw = np.array(_fs["wavelength_nm"], float)
    _fm = np.array([np.nan if v is None else v for v in _fs["midscale_neutral"]], float)
    _fd = np.array([np.nan if v is None else v for v in _fs["dmin"]], float)
    _tm = (_fw > GRID[-1]) & ~np.isnan(_fm) & ~np.isnan(_fd)
    if not _tm.any():
        raise SystemExit("fit_traced_tail: no traced sample beyond %g nm" % GRID[-1])
    _tail_wl, _tail_agg = _fw[_tm], (_fm - _fd)[_tm]


# ---------- bounded least-squares fit ----------
# params: a,b,c, sC,sM,sY, wC,wM,wY
def model(p, grid=GRID):
    a, b, c, sC, sM, sY, wC, wM, wY = p
    return (a * warp("cyan", sC, wC, grid) +
            b * warp("magenta", sM, wM, grid) +
            c * warp("yellow", sY, wY, grid))


def resid(p):
    r = (model(p) - aggregate)[_fit_mask]
    if _tail_wl is not None:
        r = np.concatenate([r, model(p, _tail_wl) - _tail_agg])
    return r
...
# The width bound is per DYE and the same on every stock. Cyan keeps 0.85-1.15:
# its peak lies at or past the 700 nm grid edge, so a wider cyan is a claim
# about unmeasured bands, and releasing it makes the withheld Fujifilm red tail
# (701-719 nm) predict worse (Superia 0.0070 -> 0.0518 D, Fujicolor 100
# 0.0491 -> 0.0700 D). Magenta and yellow absorb inside the measured band, so
# their widths are set by traced data: at 0.5-2.0 every stock lands in the
# interior, and the released fit of Pro 400H over 400-700 nm alone predicts its
# withheld tail to 0.0017 D where 0.85-1.15 gave 0.0525 D.
WIDTH_BOUND = {"cyan": (0.85, 1.15), "magenta": (0.5, 2.0), "yellow": (0.5, 2.0)}
_WB = [WIDTH_BOUND[k] for k in ("cyan", "magenta", "yellow")]

p0 = [1.0, 1.0, 1.0, 0, 0, 0, 1.0, 1.0, 1.0]
lo = [0, 0, 0] + [-SHIFT_BOUND] * 3 + [b[0] for b in _WB]
hi = [10, 10, 10] + [SHIFT_BOUND] * 3 + [b[1] for b in _WB]
...
recon = model(sol.x)
err = resid(sol.x)                            # quality is judged on measured data only
rmse = float(np.sqrt(np.mean(err ** 2)))
maxerr = float(np.max(np.abs(err)))
_tail_audit = None
if _tail_wl is not None:
    _inband = (recon - aggregate)[_fit_mask]
    _terr = model(sol.x, _tail_wl) - _tail_agg
    _tail_audit = {
        "support_nm": [float(_tail_wl[0]), float(_tail_wl[-1])],
        "n_samples": int(_tail_wl.size),
        "rmse_density": round(float(np.sqrt(np.mean(_terr ** 2))), 5),
        "inband_rmse_density": round(float(np.sqrt(np.mean(_inband ** 2))), 5),
        "note": "traced samples beyond the 400-700 nm grid (spectral_full_support) "
                "fitted as extra residual terms; aggregate_rmse_density is over "
                "in-band support plus tail, inband_rmse_density over 400-700 only. "
                "The output curves stay on 400-700 nm",
    }
...
for (_k, _v), _wb in zip((("wC", wC), ("wM", wM), ("wY", wY)), _WB):
    for _b in _wb:
        if abs(float(_v) - _b) < _PIN_EPS:
            _pins[_k] = "width at %.2f (bound %g-%g)" % (float(_v), *_wb)
if _pins:
...
# ---------- which output samples rest on the basis's held edge ----------
# Recorded per dye as a wavelength band, a sample count and the value held
# there, on the peak-normalized output curve. See warp().
_held = {}
for _k, _s, _w in (("cyan", sC, wC), ("magenta", sM, wM), ("yellow", sY, wY)):
    _hm = warp_held(_k, _s, _w)
    if _hm.any():
        _held[_k] = {"band_nm": [int(GRID[_hm].min()), int(GRID[_hm].max())],
                     "n_samples": int(_hm.sum()),
                     "held_value": round(float(norm[_k][_hm][0]), 4)}
    else:
        _held[_k] = None
if any(_held.values()):
    print("NOTE: %s rest on the basis's held blue edge (no chart data below %g nm): %s"
          % (", ".join(k for k, v in _held.items() if v),
             min(v[0] for v in basis_support.values()),
             "; ".join("%s %d-%d nm (%d samples at %.4f)" % (k, v["band_nm"][0], v["band_nm"][1],
                                                           v["n_samples"], v["held_value"])
                       for k, v in _held.items() if v)))


# ---------- scanner effective responsivities (mirror adx_engine) ----------
...
# The roll anchor divides the base+mask out in integrated density, so the
# scanner reads the image dyes through the mask: the effective responsivity is
# LED x CFA x 10^-Dmin(l), as c41_statusm_engine builds it. Diagnostic only
# here (the fit above never touches PHI).
PHI = np.stack([L_R * S_R, L_G * S_G, L_B * S_B]) * 10.0 ** (-dmin)
PHI_n = PHI / PHI.sum(1, keepdims=True)   # per-channel effective responsivity behind the mask
...
out = {
    ...
    "uncertainty": ("Surrogate-basis method: Portra's actual couplers are not published, "
                    ...
                    "dyes (shift parameter +/-%g nm, width-scale cyan %g-%g, magenta and yellow "
                    "%g-%g; the peak moves by shift x width, see peak_shift_nm). Aggregate "
                    ...
                    % (BASIS_SHORT, SHIFT_BOUND, *WIDTH_BOUND["cyan"], *WIDTH_BOUND["magenta"],
                       rmse, maxerr)),
    "shared_full_curves": {
    ...
    "per_stock": {
        STOCK["speed_key"]: {
            ...
            "matrix_note": "rows=R/G/B scan density @640/544/450; columns=cyan/magenta/yellow; "
                           "diag-normalized inverse of the LED-SPD x CFA x 10^-Dmin mixing matrix "
                           "(scan density per unit peak dye density, read through the base+mask "
                           "as the roll anchor leaves it); off-diagonals = crosstalk removed",
        }
    },
    "fit_audit": {
        ...
        "bounds": {"shift_nm": SHIFT_BOUND,
                   "width_scale": {k: list(v) for k, v in WIDTH_BOUND.items()}},
        ...
        "basis_support_nm": {k: list(v) for k, v in basis_support.items()},
        "basis_held": _held,
        "basis_held_note": "output samples whose warp source falls outside the basis's "
                           "measured support and therefore carry the basis's held edge "
                           "value; the chart publishes nothing there, so the value is a "
                           "prior, not a trace. Treat these samples as assumed",
...
if _tail_audit:                      # key present only where a tail was fitted
    out["fit_audit"]["traced_tail_fit"] = _tail_audit
...
print("RMSE %.4f D   max abs %.4f D   over the MEASURED support %s"
      % (rmse, maxerr,
         ("%.1f-%.1f nm (audit span intersected with the stored %.0f-%.0f nm array)"
          % (_support + (swl.min(), swl.max()))) if _support else "400-700 nm (no audit block)"))
if _tail_audit:
    print("  plus the traced red tail %.0f-%.0f nm (%d samples): tail RMSE %.4f D, "
          "in-band (400-700) RMSE %.4f D"
          % (_tail_audit["support_nm"][0], _tail_audit["support_nm"][1],
             _tail_audit["n_samples"], _tail_audit["rmse_density"],
             _tail_audit["inband_rmse_density"]))
if _support:
    _n = int((~_fit_mask).sum())
    print("  fitted interval %.0f-%.0f nm, %d grid points; excluded %d of %d as synthesized "
          "datasheet edge (never measured)"
          % (GRID[_fit_mask].min(), GRID[_fit_mask].max(), int(_fit_mask.sum()), _n, len(GRID)))
