...
D_MID = 0.74                               # mid-gray anchor (historical density mode): paper reflection density

# ----- data files (negative reuses the same files as c41_scene_engine) -----
sys.path.insert(0, str(ROOT))
from engine.common.lut import trilerp_unit, tetralerp_unit
...
from neutral_locus import K_LO, K_HI, K_MID, MID_BAND, sheet_neutral_locus, header_offset  # noqa: E402  (single source, shared with c41_statusm_engine)
...
class PrintConfig:
    ...
    # neutral LOCUS: WHAT the chain treats as a neutral negative (neutral_locus.py).
    #   "sheet"  (default) -> the stock's published neutral exposure series (the three
    #             characteristic-curve records solved together through the spectral
    #             base).  The stock's Status M cube SUBTRACTS this locus's offset from
    #             the diagonal, so the series arrives on k*(1,1,1); this engine adds
    #             the same offset back before inverting Status M into dye amounts and
    #             locks the diagonal onto the master neutral tone curve.  Print
    #             Adjustment's per-channel Pivot at K_MID therefore sits ON the neutral
    #             and a gamma about it leaves a neutral neutral.  The engine checks its
    #             offset against the one in the shipped Status M cube's header.
    #   "spectrum" -> the sheet's midscale-neutral SPECTRUM (one exposure) instead.
    #             The sheet's two charts disagree about the neutral (Portra 400: the
    #             spectrum puts green 0.15 D above red where the curves put it 0.04 D)
    #             and each chart's neutral prints ~20 units of a*/b* off under the
    #             other's lock.  The two BRACKET the sheet neutral; the series ships,
    #             the spectrum is the other end and the error budget carries their
    #             distance ("neutral reference").  A/B and budget only: a cube built
    #             with it does not match the shipped Status M cube's offset.
    #   "equal"  -> no offset: equal image-dye Status M treated as neutral, a synthetic
    #             neutral no C-41 sheet produces (the sheet's own neutral rendered at
    #             b* -31 on Portra 400, -48 on Ektar 100 under it).  A/B only.
    neutral_locus: str = "sheet"
...
class PrintEmulationEngine:
    ...
    def __init__(self, config=None):
        ...
        # ---------- neutral locus, then the gray-axis lock calibrated on it -------------
        self._solve_sheet_neutral_locus()
    ...
    # ---------- neutral locus ----------
    def neutral_input(self, k):
        """The input the chain treats as neutral at grey level k: the diagonal
        k*(1,1,1), the stock's Status M cube having subtracted the neutral
        offset upstream (balanced Status M)."""
        k = np.atleast_1d(np.asarray(k, float))
        return np.repeat(k[:, None], 3, axis=1)

    def input_to_od(self, Dnorm):
        """Cube input (balanced Status M / DMAX) -> image-dye Status M density."""
        return (np.atleast_2d(np.asarray(Dnorm, float)) + self.locus_offset[None, :]) * self.cfg.dmax_input

    def od_to_input(self, D_od):
        """Image-dye Status M density -> cube input (balanced Status M / DMAX)."""
        return np.atleast_2d(np.asarray(D_od, float)) / self.cfg.dmax_input - self.locus_offset[None, :]

    def _solve_sheet_neutral_locus(self):
        """Neutral offset (Dnorm, R/G/B) per cfg.neutral_locus; see neutral_locus.py."""
        cfg = self.cfg
        self.locus_offset = np.zeros(3)
        self.sheet = None
        if cfg.neutral_locus == "equal":
            return
        if cfg.neutral_locus not in ("sheet", "spectrum"):
            raise ValueError("neutral_locus must be 'sheet', 'spectrum' or 'equal', got %r"
                             % cfg.neutral_locus)
        if cfg.neg_curves_path is None:
            raise ValueError("neutral_locus=%r needs neg_curves_path" % cfg.neutral_locus)
        if cfg.neutral_locus == "spectrum":
            sm = self.spectrum_neutral_statusm() / cfg.dmax_input
            self.locus_offset = sm - sm.mean()
            return
        dmin_d = np.interp(self.DGRID, self.CGRID, self.dmin_spec_C)
        self.sheet = sheet_neutral_locus(cfg.neg_curves_path, dmin_d, self.DYE_neg, self.PRT_n,
                                         cfg.dmax_input, k_mid=cfg.k_mid, k_lo=cfg.k_lo, k_hi=cfg.k_hi)
        self.locus_offset = self.sheet.offset

    def check_statusm_cube_offset(self, cube_path):
        """The shipped Status M cube must carry the offset this engine adds back."""
        if not Path(cube_path).exists():
            return None
        h = header_offset(cube_path)
        if h is None:
            raise ValueError("%s carries no neutral offset in its header; rebuild it" % cube_path)
        mine = self.locus_offset * self.cfg.dmax_input
        if np.abs(h - mine).max() > 5e-4:
            raise ValueError("neutral offset mismatch: Status M cube %s has %s D, this engine %s D"
                             % (Path(cube_path).name, np.round(h, 4).tolist(), np.round(mine, 4).tolist()))
        return h

    def spectrum_neutral_statusm(self):
        """Image-dye Status M of the sheet's midscale-neutral spectrum less its
        D-min spectrum (spectral subtraction, the cube's own metric)."""
        sp = json.load(open(self.cfg.neg_curves_path))["spectral"]
        swl = np.array(sp["wavelength_nm"], float)
        mid = resample(swl, sp["midscale_neutral"], self.DGRID)
        base = resample(swl, sp["dmin"], self.DGRID)
        return -np.log10(self.PRT_n @ 10.0 ** (-(mid - base)))

    def report_sheet_neutral(self):
        """How the stock's published neutral prints: the sheet series over the
        band (its own curvature about the constant offset, never fitted), and the
        sheet's midscale-neutral SPECTRUM, a second chart the offset never reads,
        placed at Y = 0.18 by a common density offset only (no per-channel trim)."""
        cfg = self.cfg
        d65 = colour.CCS_ILLUMINANTS["CIE 1931 2 Degree Standard Observer"]["D65"]
        print("neutral locus: %s; offset added back to the input [R,G,B] = %s Dnorm (= %s D)"
              % (cfg.neutral_locus, np.round(self.locus_offset, 4).tolist(),
                 np.round(self.locus_offset * cfg.dmax_input, 3).tolist()))
        sh = self.sheet
        if sh is not None:
            print("  sheet neutral series (three Status M records solved together through the spectral "
                  "base; %d exposures, solve max |dD| %.1e D, amounts min %+.3f): departure from the "
                  "offset ranges %s Dnorm over k=%.2f+/-%.2f and %s over k in [%.2f, %.2f]"
                  % (sh.n, sh.resid, sh.amount_min, np.round(sh.spread_band, 4).tolist(), cfg.k_mid,
                     MID_BAND, np.round(sh.spread_span, 4).tolist(), cfg.k_lo, cfg.k_hi))
            for label, mask in (("k=%.2f+/-%.2f" % (cfg.k_mid, MID_BAND), sh.band),
                                ("k in [%.2f, %.2f]" % (cfg.k_lo, cfg.k_hi), sh.window)):
                P = sh.locus[mask] - self.locus_offset[None, :]       # as the Status M cube delivers it
                lab = colour.XYZ_to_Lab(self.dnorm_to_linP3(P)[1], d65)
                print("  sheet series printed over %s (n=%d): |a*| max %.2f, |b*| max %.2f, mean a* %+.2f "
                      "b* %+.2f (the series' own curvature; a per-channel offset cannot remove it and "
                      "nothing is fitted to it)"
                      % (label, len(P), np.abs(lab[:, 1]).max(), np.abs(lab[:, 2]).max(),
                         lab[:, 1].mean(), lab[:, 2].mean()))
        if cfg.neg_curves_path is not None:
            sm = self.spectrum_neutral_statusm()                         # image-dye Status M
            P0 = self.od_to_input(sm)[0]

            def y_of(offset):
                return float(self.dnorm_to_linP3((P0 + offset)[None, :])[1][0, 1])

            # a denser negative prints LIGHTER, so Y rises with the offset
            lo, hi = -P0.min() - 0.5, 1.0
            for _ in range(60):
                m = 0.5 * (lo + hi)
                if y_of(m) < 0.18:
                    lo = m
                    ...
                    hi = m
            offset = 0.5 * (lo + hi)
            lab = colour.XYZ_to_Lab(self.dnorm_to_linP3((P0 + offset)[None, :])[1], d65)[0]
            print("  sheet midscale-neutral SPECTRUM (image-dye Status M %s; departure from the diagonal %s "
                  "Dnorm against the series' %s) placed at Y=0.18 by a common offset %+.3f Dnorm: a* %+.2f "
                  "b* %+.2f. The sheet's two charts disagree about the neutral; this is the other end of "
                  "the bracket (neutral_locus='spectrum' locks on it instead)"
                  % (np.round(sm, 4).tolist(),
                     np.round(sm / cfg.dmax_input - (sm / cfg.dmax_input).mean(), 4).tolist(),
                     np.round(self.locus_offset, 4).tolist(), offset, lab[1], lab[2]))

    def report_floor_loss(self, n=5000, dye_max=2.2, seed=1):
        """Colour lost at the Status M cube's output floor.  Balanced Status M is
        image-dye Status M minus the neutral offset, so a channel whose image-dye
        density is below a positive offset goes negative and the [0,1] cube clips
        it; adding the offset back cannot recover it.  Uniform non-negative dye
        triples over [0, dye_max]^3 (seed `seed`), exact render before and after
        clipping the balanced coordinate, Display-P3 clipped, dE2000 in Lab D65:
        the share below the floor, the mean on that share and the maximum over
        all samples.  Returns (fraction, subset_mean, max)."""
        a = np.random.default_rng(seed).uniform(0.0, dye_max, (n, 3))
        P = self.od_to_input(self.statusm_fwd(a))
        below = (P < 0).any(1)
        p3_to_xyz = np.linalg.inv(self.XYZ_to_P3)
        d65 = colour.CCS_ILLUMINANTS["CIE 1931 2 Degree Standard Observer"]["D65"]

        def lab(points):
            lin = np.clip(self.dnorm_to_linP3(points)[0], 0.0, 1.0)
            return colour.XYZ_to_Lab(lin @ p3_to_xyz.T, d65)

        de = colour.delta_E(lab(P), lab(np.clip(P, 0.0, 1.0)), method="CIE 2000")
        frac = float(below.mean())
        subset = float(de[below].mean()) if below.any() else 0.0
        print("balanced-encoding floor loss (dye 0-%.1f uniform, n=%d, seed %d; exact render before vs "
              "after clipping the balanced coordinate): %.2f%% of samples below the floor in some "
              "channel, mean dE2000 %.4f on that share, max %.4f over all samples; the print engine "
              "cannot recover what the Status M cube clipped"
              % (dye_max, n, seed, 100 * frac, subset, float(de.max())))
        return frac, subset, float(de.max())
    ...
    def lock_nonaffinity(self, k_lo, k_hi, n=200):
        ...
        LEraw = self.paper_logE_raw(self.neutral_input(kk))
        out = []
        for i in range(3):
    ...
    def solve_gray_axis_lock(self):
        ...
        neutral = self.neutral_input(KK)                                # (N,3) k*(1,1,1) + locus offset
        ...
        # density triplet
        LEmid = self.paper_logE_raw(self.neutral_input(cfg.k_mid))[0]  # (3,)
...
def trilinear(lut, pts):
    """Trilinear sample of a (S,S,S,3) LUT at pts in [0,1]^3 (engine/common/lut.py)."""
    return trilerp_unit(lut, pts)


def tetrahedral(lut, pts):
    """Tetrahedral sample, the scheme docs/resolve.md sets Resolve to."""
    return tetralerp_unit(lut, pts)
...
def report_cube_fidelity(path, lut_mem, size, exact_fn, unit_scale=255.0):
    ...
    exact = np.asarray(exact_fn(pts), float)
    err = tetralerp_unit(disk, pts) - exact
    err_tri = trilerp_unit(disk, pts) - exact
    rmse = float(np.sqrt(np.mean(err ** 2)))
    mx = float(np.max(np.abs(err)))
    print("serialized %s: INTERPOLATION tetrahedral (deployment, docs/resolve.md) RMSE %.3e  max %.3e "
          "code value 0-1 (= %.2f / %.2f code values out of 255); trilinear RMSE %.3e  max %.3e "
          "(neither bounds the other; n=%d off-lattice points, seed %d)"
          % (path.name, rmse, mx, rmse * unit_scale, mx * unit_scale,
             float(np.sqrt(np.mean(err_tri ** 2))), float(np.max(np.abs(err_tri))),
             PROBE_N, PROBE_SEED))
    return rmse, mx
...
def main(argv=None):
    ...
    eng.check_statusm_cube_offset(BUILDS / "c41" / ("%s_StatusM.cube" % neg_prefix))
    ...
    D_mid_ach = eng.dnorm_to_reflectance(eng.neutral_input(K_MID))[1][0]
    print("  achieved print density @K_MID [C,M,Y] = %s (anchor %.4f)"
          % (np.round(D_mid_ach, 4).tolist(), eng.d_anchor))
    eng.report_sheet_neutral()
    eng.report_floor_loss()

    # ---- neutral-axis ramp ----
    print("=== neutral-axis ramp: Dnorm = k*(1,1,1) (the Status M cube puts the sheet neutral there) ===")
    print("   k      DisplayP3(R,G,B)          chroma_err     Y")
    ks = np.arange(0.05, 0.601, 0.05)
    for k in ks:
        lin, XYZ, D_P, a = eng.dnorm_to_linP3(eng.neutral_input(k))
    ...
    linf, XYZf, D_Pf, af = eng.dnorm_to_linP3(eng.neutral_input(kfine))
    ...
    Dn = eng.dnorm_to_reflectance(eng.neutral_input(kf))[1].mean(1)
