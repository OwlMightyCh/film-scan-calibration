...
basis sensitivity of 0.027-0.049 D (--basis-sensitivity re-derives it) which is
LARGER than the distance between 16 of the 66 pairs, so two stocks agreeing after
...
# basis: how far a stock's reconstructed aggregate spectral density moves when
# the assumed basis is swapped for a plausible alternative. Any new distance is
# meaningless without this scale.
#
# `--basis-sensitivity` re-derives it over all twelve stocks and the five
# alternative bases in data/films/_ensemble, using the same shape_residual below
# that the inter-stock distances use, so the two are directly comparable. The
# figure is the worst displacement per stock, then the band across stocks.
#
# Read it as basis SENSITIVITY and not as an error bar on the truth: the
# canonical basis fits the datasheets better than every alternative (per stock,
# the best alternative's residual is 1.04-2.49 times the canonical one; fleet
# ranges 0.0078-0.0207 D against 0.0110-0.0550), so the alternatives are not
# equally credible, although on Fujicolor 100 and Fujifilm 200/400 the best of
# them comes within 4%.
BASIS_UNCERTAINTY_D = (0.027, 0.049)
...
def speed_offset(xa, ya, xb, yb, dens=1.0):
    ...
    Density is the lookup axis here, and np.interp requires it to ascend. Eight
    of the fleet's curves do not: Gold 200's blue dips 0.031 D, Pro 400H's green
    0.007 D, and fujifilm200 G, portra160 B, pro400h R and B and ultramax400 G
    and B carry smaller dips. These are digitisation noise -- a characteristic
    curve does not reverse -- so the axis is made monotonic before the lookup
    rather than the dips being treated as real.
    """
    def logh_at(x, y, d):
...
# The canonical basis, and the alternatives it is measured against.  The three
# further variants in the ensemble directory are not `--basis` choices and are
# reported separately: none of them widens the band, and b25 is byte-identical
# to the canonical fit.
CANONICAL_BASIS = "vision3"
ALT_BASES = ("e100", "provia100f", "velvia50", "velvia100", "parametric")
LEGACY_BASES = ("hyb", "b25", "shift25")
ENSEMBLE = DATA / "_ensemble"


def _reconstructed_aggregate(path):
    """Sum the fitted dye amounts over the fitted (warped) basis curves.

    This is the object the fit is trying to match, and the same object the
    inter-stock distances are computed on, which is what makes the two
    directly comparable.
    """
    d = json.load(open(path))
    s, prm = d["shared_full_curves"], d["fit_audit"]["params"]
    wl = np.array(s["wavelength_nm"], float)
    agg = (prm["a"] * np.array(s["cyan"], float)
           + prm["b"] * np.array(s["magenta"], float)
           + prm["c"] * np.array(s["yellow"], float))
    return wl, agg


def basis_sensitivity(bases):
    """Worst displacement per stock when the basis is swapped, over `bases`."""
    worst = {}
    for f in sorted(ENSEMBLE.glob("*_dye_density__%s.json" % CANONICAL_BASIS)):
        stock = f.name.split("_dye_density__")[0]
        w0, a0 = _reconstructed_aggregate(f)
        vals = []
        for b in bases:
            alt = ENSEMBLE / ("%s_dye_density__%s.json" % (stock, b))
            if not alt.exists():
                continue
            w1, a1 = _reconstructed_aggregate(alt)
            if not np.array_equal(w0, w1):
                raise SystemExit("ABORT: %s and %s are on different grids"
                                 % (f.name, alt.name))
            vals.append(shape_residual(a0, a1)[1])
        if vals:
            worst[stock] = max(vals)
    return worst


def report_basis_sensitivity():
    worst = basis_sensitivity(ALT_BASES)
    if not worst:
        raise SystemExit("ABORT: no ensemble fits found under %s" % ENSEMBLE)
    print("BASIS SENSITIVITY: worst displacement of each stock's reconstructed")
    print("aggregate when the assumed basis is swapped, over %d alternatives."
          % len(ALT_BASES))
    for stock, v in sorted(worst.items(), key=lambda kv: kv[1]):
        print("  %-20s %.4f D" % (stock, v))
    v = sorted(worst.values())
    print("basis sensitivity: %.3f-%.3f D over %d stocks" % (v[0], v[-1], len(v)))
    w = sorted(basis_sensitivity(ALT_BASES + LEGACY_BASES).values())
    print("  including %s: %.3f-%.3f D (partial coverage; does not widen it)"
          % ("/".join(LEGACY_BASES), w[0], w[-1]))
    print("  the constant in this file is %.3f-%.3f D" % BASIS_UNCERTAINTY_D)


def main():
    ap = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    ap.add_argument("--basis-sensitivity", action="store_true",
                    help="re-derive the basis-sensitivity band from the "
                         "ensemble fits and print it")
