...
#
# (The Fujifilm-family fits sit in and near the Kodak range: Pro 400H 0.0103 and
# Fujifilm 200/400 0.0119 inside the Kodak band 0.0078-0.0138, Superia 0.0196
# and Fujicolor 100 0.0207 above it. Any claim that "Fujifilm chemistry does not
# suit this basis" is refuted by its own fleet. Separately, the user reports Fujifilm 200/400 are
...
# The WIDTH bound is per dye and the same on every stock: cyan 0.85-1.15 (an
# extrapolation guard, since the cyan peak lies at or past 700 nm), magenta and
# yellow 0.5-2.0 (in-band dyes; no stock reaches either end). portra_decompose.py
# states the withheld-tail evidence for the split.
STOCKS = {
    "portra400": {
    ...
    "pro400h": {
        ...
        "digitizer": "fuji_digitize.py",
        "shift_bound_nm": 25.0,
        # The dye fit also takes the traced samples beyond 700 nm that the
        # digitiser keeps in `spectral_full_support`. Admitted for THIS stock.
        # Withheld, those samples validate the 400-700 fit (predicted to
        # 0.0017 D, cyan peak 700.0 nm), and admitting them moves no dye curve
        # by more than 0.0009 D; under a 0.85-1.15 magenta width the same
        # samples refuted a +25.00 cyan pin (0.0525 D), which is why that
        # width is left to the data (portra_decompose.py). Not admitted elsewhere: Fujifilm 200/400 and Superia Premium 400
        # already predict their tails below in-band error and move under
        # 0.3 nm; Fujicolor 100 worsens in-band with no withheld validation;
        # the Kodak sheets end at 700 nm. Measured support, not a bound
        # (PROJECT.md Invariant 7), so it is admissible per stock.
        "fit_traced_tail": True,
    },
}
