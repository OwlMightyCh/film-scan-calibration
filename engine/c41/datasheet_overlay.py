...
frame detection, axis ORIGIN, axis STEP, curve assignment and curve sampling
SIMULTANEOUSLY. Everything else in the pipeline can be clean while all of those
are wrong together:
...
If the replotted curve lands on the printed ink, all of that is right at once.
...
def ink_hit(img, px, py, radius=2, thresh=170):
    ...
    origin, a wrong axis step or a swapped channel all move the curve OFF the
    ink, and all three leave every numerical residual untouched -- so this is a
    check no fit statistic can substitute for.
...
def overlay(stock, dpi=170):
    ...
    def plot(xd, yd, xmap, ymap, colour, label=""):
        ...
        if tot:
            scores.append((label, 100.0 * ok / tot, tot))
    ...
    print("  %-12s %-26s ink-hit min %s  mean %5.1f%%  (%d curves)"
          % (stock, out.name,
             ...
             len(scores)))
    for lab, pct, n in scores:
        if pct < 90.0:
            print("       !! %-18s only %.1f%% of %d samples on ink -- INSPECT" % (lab, pct, n))
    return out
