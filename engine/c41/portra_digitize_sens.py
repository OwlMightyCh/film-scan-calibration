...
def main():
    ...
    # Fragments at the same y are MERGED before the span tests, mirroring the
    # vertical path above. Two separate failures are in play here:
    #   * Gold 200 draws its 1.0 log-sens gridline short (x 74.87-179.14 of a
    #     74.87-275.39 plot), so a full-width test drops it;
    #   * Pro Image 100 BREAKS its 3.0 gridline where the curves cross it, into
    #     x 362.76-367.66 and x 454.24-563.07. The first is too short and the
    #     second does not reach the left edge, so tested as separate segments
    #     neither survives and the gridline disappears entirely -- taking one
    #     decade off every sensitivity on the sheet.
    # Merged, both span their plot and pass. The x-overlap condition keeps a
    # stray segment from another chart at the same y out of the merge.
    _hsegs = {}
    for l in lines:
        if (abs(l.y1 - l.y0) < 0.5 and FY0 - 1 <= l.y0 <= FY1 + 1
                and max(l.x0, l.x1) > FX0 - 2 and min(l.x0, l.x1) < FX1 + 2):
            k = round(l.y0, 1)
            _lo, _hi = min(l.x0, l.x1), max(l.x0, l.x1)
            _a, _b = _hsegs.get(k, (_lo, _hi))
            _hsegs[k] = (min(_a, _lo), max(_b, _hi))
    # 25% of the plot width is well clear of tick marks while accepting a
    # genuinely short gridline.
    hy = {k for k, (_lo, _hi) in _hsegs.items()
          if _lo < FX0 + 5 and _hi > FX0 + 0.25 * (FX1 - FX0)}
    for r in rects:
        if inframe(*r.bbox, pad=2.0):
