...
from endura_print_engine import (  # noqa: E402
    ...
    K_LO, K_HI, N_CAL, K_MID, NEG_STOCKS, NEG_STOCK, neg_paths,
)
...
def main(argv=None):
    ...
    ap.add_argument("--stock", choices=sorted(NEG_STOCKS), default=DEFAULT_NEG,
    ...
    if inside.any():
        print("printable neutral window (Fuji, COMPUTED -- Endura's is k in [0.082, 0.348]): "
    ...
    # ---- inversion diagnostics over the emitted lattice ----
    D_check = eng.statusA_fwd(a) + eng.Dbase
    ares = np.abs(D_check - D_P)
    print("Status A inversion residual |D| over the %d^3 lattice: median %.3e  95th %.3e  max %.3e"
          % (SZ, float(np.median(ares)), float(np.percentile(ares, 95)), float(ares.max())))
    zc = (a <= 1e-12).any(1)
    print("dye-amount inversion clipping at zero: %d/%d nodes (%.2f%%) clip some paper dye amount "
          "to 0 (non-negativity infeasibility, not a solver defect), carrying %.1f%% of the "
          "residual mass"
