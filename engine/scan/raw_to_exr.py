...
def check_sequence(files):
    ...
    capture (a raw and its DNG, say) and is refused. A jump in the counter is
    only WARNED about, never refused: a gap in the numbering is as often a
    deliberate deletion (a spoiled capture and its reshoot removed by hand) as
    a genuinely missing frame, and the file list alone cannot tell the two
    apart. The warning says whether the gap sits on a triplet boundary, and
    the triplet listing printed after it is where the assignment is
    confirmed by eye. Counters that all tie carry no information and are not
    checked; a file with no number ends the check the same way.
    """
    ...
    for i in range(1, len(files)):
        ...
        gaps.append(f"{missing} counter(s) skipped between {prev.name} and "
                    f"{cur.name} ({counters[i - 1]} -> {counters[i]}), {where}")
    if problems:
        raise SystemExit("Frame counters do not form an unbroken R, G, B "
                         "sequence; nothing written.\n  " + "\n  ".join(problems))
    if gaps:
        print("Warning: frame counters skip. Deleted captures leave the "
              "triplets aligned; captures that never landed would shift "
              "them. Confirm the R, G, B assignment listed below.\n  "
              + "\n  ".join(gaps))
    return groups
