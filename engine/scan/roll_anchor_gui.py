...
def _parse_seconds(spec):
    spec = spec.strip()
    return (float(spec.split("/")[0]) / float(spec.split("/")[1])
...
def measure_frame(args_list, roi=None):
    ...
    if len(args_list) == 1:
        ...
        for channel in CHANNELS:
            exposure = (override_for(override, channel)
                        or (meta.get("exposure_s") or {}).get(channel))
            if exposure is None:
    ...
    for channel, arg in zip(CHANNELS, args_list):
        ...
        exposure = override_for(override, channel) or exif_exposure_seconds(path)
        iso = exif_iso(path)
...
def frame_hist_planes(frame_args, box):
...
def pick_roi_for_frame(name, frame_args, prev_box):
    ...
    def hist_values(box):
        return frame_hist_planes(frame_args, box)
