...
other file in the repo — the ISO 640 gain ratio falls back to an embedded
constant (override with --gain-ratio) and the save dialog falls back to the
cwd. Outside dependencies are the `exiftool` binary and, per input format,
...
# Fallback copy of data/equipment/a7r3_gain_ratio.json, so this file measures
# an ISO 640 frame correctly when run outside the repo. Measured 2026-07-17,
# 3 interleaved frame pairs, per-pair spread <=0.15%. The JSON on disk WINS
# when present — this is the stale copy by construction, so if the rig is
# re-measured (new body or firmware), update both.
EMBEDDED_GAIN_RATIO = {
    "measured": "2026-07-17",
    "nominal_ratio": 6.4,
    "gain_ratio": {"R": 6.28897, "G": 6.29428, "B": 6.274},
    "source": "embedded in roll_anchor_gui.py",
}


def load_gain_ratio(path=None):
    """Measured ISO640/ISO100 dual-conversion-gain ratio (rig constant).

    Order: explicit --gain-ratio, then the repo's data/equipment copy, then
    the embedded constant. Never None, so an ISO 640 roll can be measured
    from a lone copy of this script.
    """
    if path is not None:
    ...
    if repo_root() is not None:
        on_disk = repo_root() / "data" / "equipment" / "a7r3_gain_ratio.json"
        if on_disk.exists():
            record = json.loads(on_disk.read_text())
            record.setdefault("source", "data/equipment/a7r3_gain_ratio.json")
            return record
    return dict(EMBEDDED_GAIN_RATIO)


def iso_factor(iso, channel, gain_ratio, label):
    """Factor converting a frame's linear rate onto the ISO 100 scale."""
    if iso is None or int(iso) == 100:
        return 1.0
    if int(iso) == 640:
        if gain_ratio is None:  # defensive: load_gain_ratio always returns one
            raise SystemExit(
                f"{label}: shot at ISO 640 but no gain ratio is available — "
                f"pass --gain-ratio, or run the interleaved-pairs calibration "
                f"(see PROJECT.md, per-roll anchoring).")
        return float(gain_ratio["gain_ratio"][channel])
    raise SystemExit(f"{label}: ISO {iso:.0f} unsupported — use 100 or 640 "
                     f"(only the 640/100 gain ratio is calibrated).")


def exif_consistency(paths):
    """Aperture drift across frames breaks shutter/gain-only normalization.

    (ISO is no longer required to be uniform: 100 vs 640 is normalized per
    channel via the measured gain ratio; other ISOs are rejected at
    measurement time. TIFF inputs are skipped — their aperture is recorded
    in the merged-frame metadata for the audit trail but not enforced here.)
    """
    tags = {}
    for p in paths:
...
def load_linear_planes(path, roi=None):
    ...
    with rawpy.imread(str(path)) as raw:
        ...
        for ci, letter in enumerate(desc):
            ...
            spans[letter] = max(1.0, white - black[ci])
...
def measure_frame(args_list, roi=None, gain_ratio=None):
    ...
    legacy tiff_maker TIFF (per-channel
    exposure/ISO read from its embedded metadata). Rates are expressed on
    the ISO 100 scale: ISO 640 frames are divided by the measured
    dual-conversion-gain ratio per channel. `roi` (see resolve_roi) applies
    to the whole frame set; the effective pixel box and a bimodality
    (contamination) flag are recorded per channel.
    """
    result = {}
    if len(args_list) == 1:
        ...
        for channel in CHANNELS:
            ...
            factor = iso_factor(iso, channel, gain_ratio, f"{path.name} {channel}")
            values = planes[channel]
            mean, std, n = robust_stats(values)
            result[channel] = dict(rate=mean / exposure / factor, mean_adu=mean,
                                   std_adu=std, n_pixels=n, exposure_s=exposure,
                                   iso=iso, iso_factor=round(factor, 5),
    ...
    for channel, arg in zip(CHANNELS, args_list):
        ...
        for cam in CHANNELS:
            ...
            cam_factor = iso_factor(iso, cam, gain_ratio, f"{path.name} {cam}")
            response[cam] = cam_mean / exposure / cam_factor

        result[channel] = dict(rate=mean / exposure / factor, mean_adu=mean,
                               std_adu=std, n_pixels=n,
                               exposure_s=exposure, iso=iso,
                               iso_factor=round(factor, 5),
...
def run_extraction(plain, dmin, dmax, roll_id, out, rois=None,
                   film_family="reversal", gain_ratio_path=None):
    ...
    """
    rois = rois or {}
    gain_ratio = load_gain_ratio(gain_ratio_path)
    all_paths = [parse_frame_arg(a)[0] for a in plain + dmin + (dmax or [])]
    exif_consistency(all_paths)

    plain_frame = measure_frame(plain, rois.get("plain"), gain_ratio)
    dmin_frame = measure_frame(dmin, rois.get("dmin"), gain_ratio)
    ...
    record = {
        ...
        "dmin_exr_scale_note": "density vs sensor white level, matching raw_to_exr "
                               ...
                               "was shot at the roll's per-channel exposure/ISO",
    ...
    if dmax:
        dmax_frame = measure_frame(dmax, rois.get("dmax"), gain_ratio)
    ...
    if any(f[ch].get("iso_factor", 1.0) != 1.0
           for f in record["frames"].values() for ch in CHANNELS):
        record["gain_correction"] = {
            "applied_ratio_iso640_over_iso100": gain_ratio["gain_ratio"],
            "ratio_measured": gain_ratio.get("measured"),
            "ratio_source": gain_ratio.get("source", "unknown"),
            "note": "ISO 640 frames divided by the measured per-channel ratio "
                    "to land on the ISO 100 scale before the density log",
        }
...
def dialog_flow():
    ...
    if messagebox.askyesno("Dmax", "Also measure Dmax (unexposed rebate / frame gap)?\n"
                                    "Diagnostic only — needs a long exposure "
                                    "(or ISO 640 with ~6x less)."):
...
def main():
    if len(sys.argv) == 1:
        ...
        parser.add_argument("--film-family", choices=("reversal", "negative"),
                            default="reversal")
        parser.add_argument("--gain-ratio", metavar="JSON",
                            help="a7r3_gain_ratio.json to use for ISO 640 frames; "
                                 "defaults to the repo copy, else the embedded "
                                 "constant. ISO 100 frames never consult it.")
    ...
    record = run_extraction(args.plain, args.dmin, args.dmax,
                            ...
                            gain_ratio_path=args.gain_ratio)
