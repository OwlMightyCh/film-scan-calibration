...
def merge_triplet(index, triplet, flat_models, out_dir, exifs, role=None):
    ...
    read = make_reader()
    # One channel per decode is kept, as in the colour engine, so one full
    # image is resident per worker rather than three.
    ...
    h, w = rte.require_same_size([(r_path.name, r_ch), (g_path.name, g_ch),
                                  (b_path.name, b_ch)])
...
def main():
    ...
    groups = rte.check_sequence(files)
