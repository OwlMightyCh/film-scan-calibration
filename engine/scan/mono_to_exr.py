...
def merge_triplet(index, triplet, flat_models, out_dir, exifs):
    """Worker: one triplet -> one EXR. The merge, flip and container are the
    colour engine's; only the decode differs."""
...
def main():
    ...
    files = sorted((p for p in in_dir.iterdir()
                    if p.is_file() and p.suffix.lower() in rte.RAW_EXTS
                    and not p.name.startswith(".")), key=rte.sort_key)
    if len(files) < 3:
    ...
    if args.flats is None:
        flats_folder = rte.prompt_flats()
    else:
        flats_folder = None if args.flats.lower() == "skip" \
            else Path(args.flats).expanduser()
    flat_models = rte.build_flat_models(flats_folder, read) if flats_folder else None

    exifs = rte.batch_exif(files)
