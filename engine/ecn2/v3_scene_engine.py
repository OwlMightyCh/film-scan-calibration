...
engine. The output is a per-stock, model-based estimate of relative scene
exposure: each layer's response away from the sheet's neutral series is
taken to be that layer's neutral-series table (DIR_MATRIX identity), and the
scene is rendered under the stock's nominal balance illuminant. The neutral
series cannot choose that off-neutral rule (a coupling fitted to the same
series decodes the same neutral and different colours), and no frame has
been checked against a known scene, so the route is the model reference the
ADX16 route is scored against, not a validated accuracy reference; it is the
path for graded work, and the ADX16 route is the branch's primary delivery.
(C-41 is the opposite case: those negatives were designed to be printed, and
print emulation is their sole delivery route.)
...
Pipeline per lattice node:
  ...
  4. relative layer exposure L = 10^(logH - logH_mid). logH_mid comes from
     ...
     Where a trace stops short of a channel's responsivity (500T's ends at
     640 nm, under 69% of the red weight) that channel's value is a
     partial-band density inverted against a full-band curve; the build
     prints each channel's excluded weight and warns above 5%. The three
     per-channel values disagree (spread 0.026-0.103 logH) because the
     midscale spectrum does not lie on the characteristic series' neutral:
     passed through the chain it decodes off-neutral (the build prints the
     residual where the trace spans the grid). The scalar average sets the
     exposure scale only; that disagreement is a colour residual, not a trim.
  ...
  5. a 3x3 matrix (weighted least squares over ColorChecker 24 + Munsell
     ...
     200T/500T -- Bradford-adapted to D65, with the grey constraint
     M . 1 = XYZ(18% grey) imposed inside the least-squares solve, so the
     matrix is the weighted least-squares minimizer among grey-preserving
     matrices rather than an unconstrained fit rescaled by row) maps
     L -> XYZ(D65). The set weights (checker 24, Munsell glossy 24, matt 12,
     Agfa 12, skin 24) are a stated preference, not a consequence of the
     data, and the residual is a training-set figure;
...
from engine.common.lut import read_cube, trilerp_unit, tetralerp_unit  # noqa: E402
...
class V3SceneEngine:
    def __init__(self, stock, sensor_path, sensor_label):
        ...
        # support, clamped to the array's own range and to 800 nm -- the same
        # policy, for the same reasons, as adx_engine (no synthesized tail, no
        # perfectly-clear zero-fill at the frame edge). The LED SPD is
        # measured to 780 nm and resample() is zero beyond a table's support,
        # so the grid asserts nothing the apparatus did not measure. A 730 nm
        # cap (the colorimetric CGRID's, a separate question) discarded the
        # red LED's tail behind the cyan dye: under the unity sensor that
        # moved scanner density by up to 0.013-0.020 D and decoded layer
        # exposure by 10-16% at the 99th percentile over the dye box; a
        # camera response with its own IR cut sees none of it.
        lo = max(400.0, min(m.min() for m in meas), wl_all.min())
        hi = min(800.0, max(m.max() for m in meas), wl_all.max())
        ...
        # share of each channel's Status M weight, on the density grid, that
        # the midscale trace does not cover: that share of the channel's
        # density is asserted by renormalization, not measured
        outside = (self.DGRID < self.mid_wl.min()) | (self.DGRID > self.mid_wl.max())
        self.mid_excluded_weight = self.SM_n[:, outside].sum(1)
    ...
    def _solve_matrix(self, L_arr, X_arr, w_arr):
        """Weighted least-squares 3x3 (X = M @ L) subject to the exact grey
        constraint M @ (1,1,1) = XYZ of a flat 18% grey. The constraint is
        eliminated analytically: with g the grey target, the third column is
        g minus the other two, so X - L3*g is regressed on (L1-L3, L2-L3).
        An unconstrained fit rescaled by row to hit g is a feasible point of
        the same problem and never a better one; its grey and both weighted
        residuals are returned for the self-report."""
        w = np.sqrt(w_arr)[:, None]
        g = self.XYZ_of_reflectance(GRAY * np.ones_like(CGRID))
        Z = L_arr[:, :2] - L_arr[:, 2:3]
        T = X_arr - L_arr[:, 2:3] * g[None, :]
        a, *_ = np.linalg.lstsq(Z * w, T * w, rcond=None)   # (2, 3): Z @ a = T
        M = np.vstack([a, g[None, :] - a.sum(0, keepdims=True)]).T   # X = M @ L
        M_free, *_ = np.linalg.lstsq(L_arr * w, X_arr * w, rcond=None)
        M_free = M_free.T
        g_free = M_free @ np.ones(3)
        M_scaled = M_free * (g / g_free)[:, None]
        sse = lambda Mx: float(np.sum(w_arr[:, None] * ((L_arr @ Mx.T) - X_arr) ** 2))
        info = {"grey_unconstrained": g_free, "sse_rowscaled": sse(M_scaled),
                "sse_constrained": sse(M)}
        return M, info
    ...
    def _fit_matrix(self):
        ...
        self.M_checker, info_checker = self._solve_matrix(self.cc_L, self.cc_X, self.cc_w)
        ...
        self.M_broad, info_broad = self._solve_matrix(L_all, X_all, w_all)
        ...
        self.M_fit_info = info_broad if MATRIX_MODE == "broad" else info_checker
        self.M_gray_after = self.M @ np.ones(3)

    # ---------- midscale spectrum through the chain (consistency check) ----------
    def midscale_chain_check(self):
        """The traced midscale-neutral spectrum, read by the bare LEDs and
        anchored against the traced D-min as the roll anchor does, decoded
        through the delivered inverse and the matrix. logH_mid is the average
        of three per-channel inversions of this spectrum, so a spectrum that
        sat on the characteristic series' neutral would return L = (1,1,1)
        and Y = 0.18; the departure is the disagreement between the sheet's
        two charts of one neutral, scored as chroma dE2000 at matched Y. The
        scanner integrals run over the trace's measured support only,
        renormalized, so nothing is asserted beyond it; the share of each
        LED's weight outside that support is returned beside the result, and
        where it is large (500T red, the trace ending at 640 nm) the figure is
        a partial-band approximation."""
        m = (self.DGRID >= self.mid_wl.min()) & (self.DGRID <= self.mid_wl.max())
        P = self.PHI_bare_n[:, m] / self.PHI_bare_n[:, m].sum(1, keepdims=True)
        mid = resample(self.mid_wl, self.mid_spec, self.DGRID[m])
        sc = -np.log10((P @ 10.0 ** (-mid)) / (P @ 10.0 ** (-self.DMIN[m])))
        L = self.scan_norm_to_L(sc / DMAX)
        XYZ = (self.M @ L.T).T[0]
        ref = self.XYZ_of_reflectance(GRAY * np.ones_like(CGRID))
        dE = float(self.dE(XYZ * (GRAY / XYZ[1]), ref))
        excluded = 1.0 - self.PHI_bare_n[:, m].sum(1)
        return L[0], float(XYZ[1]), dE, excluded
...
def main(argv=None):
    ...
    print("midscale trace vs Status M: share of each channel's weight on the grid outside "
          "the trace's support R,G,B = %s%s"
          % (np.round(100 * eng.mid_excluded_weight, 1).tolist(),
             "".join("  WARNING %s channel's anchor is a partial-band density inverted "
                     "against a full-band curve" % ch
                     for i, ch in enumerate("RGB") if eng.mid_excluded_weight[i] > 0.05)))

    # ---- the midscale spectrum through the chain ----
    mL, mY, mdE, mex = eng.midscale_chain_check()
    print("midscale spectrum through the chain (bare LEDs over the trace's support, anchored on "
          "the traced D-min, delivered inverse, matrix): L = %s, Y = %.4f against 0.18, chroma "
          "%.2f dE2000 at matched Y; LED weight outside the trace's support R,G,B = %s%%%s"
          % (np.round(mL, 3).tolist(), mY, mdE, np.round(100 * mex, 2).tolist(),
             "  (partial-band: an approximation)" if mex.max() > 0.05 else ""))
    ...
    print("gray-18 XYZ: unconstrained fit %s; constrained fit %s (target %s); weighted "
          "XYZ SSE of the unconstrained fit rescaled by row %.5f -> constrained %.5f "
          "(%.2f%% lower; the constrained solve must never be higher)"
          % (np.round(fi["grey_unconstrained"], 4).tolist(),
             np.round(eng.M_gray_after, 4).tolist(),
             np.round(eng.XYZ_of_reflectance(GRAY * np.ones_like(CGRID)), 4).tolist(),
             fi["sse_rowscaled"], fi["sse_constrained"],
             100.0 * (1.0 - fi["sse_constrained"] / fi["sse_rowscaled"])))
    assert fi["sse_constrained"] <= fi["sse_rowscaled"] * (1 + 1e-9), "constrained fit worse than row-scaled"
    ...
    # ---- LUT vs exact chain over probe dye boxes, both lookups ----
    # Mirrors adx_engine's validation: the cube is exact at its nodes by
    # construction, so what needs measuring is the interpolation error
    # between them, in the output's own units. Relative DWG error is the
    # meaningful figure for a scene-linear output spanning decades. The
    # tetrahedral lookup is the scheme the Resolve setup requires and the
    # figure the docs quote; trilinear is printed beside it because neither
    # bounds the other (engine/common/lut.py).
    def tetralerp(Larr, pts):
        return tetralerp_unit(Larr, pts / DMAX)

    def trilerp(Larr, pts):
        return trilerp_unit(Larr, pts / DMAX)
    ...
    def probe(label, ceiling, detail=False):
        ...
        rels = {}
        for name, fn in (("tetrahedral", tetralerp), ("trilinear", trilerp)):
            approx = fn(rt, sv)
            rel = np.abs(approx - truth) / np.maximum(np.abs(truth), 1e-3)
            rels[name] = rel
            print("LUT %s vs exact chain [%s, dye 0-%s]: rel err "
                  "mean %.4f p99 %.4f max %.4f  (abs max %.4f DWG)"
                  % (name, label, "/".join("%.2f" % c for c in ceiling), rel.mean(),
        ...
        # the CLEAN-cell ones under the tetrahedral lookup, banded by how
        # bright the scene value is.
        clean = cell_clean(sv)
        per = rels["tetrahedral"].max(1)
        print("  tetrahedral, by lattice cell: CLEAN (all 8 corners converged to %.0e D and "
        ...
        per_tri = rels["trilinear"].max(1)
        print("  trilinear, same split, for comparison: CLEAN mean %.4f p99 %.4f max %.4f; the rest mean %.4f p99 %.4f max %.4f"
              % (stats(per_tri[clean]) + stats(per_tri[~clean])))
        yrel = XYZ_t[:, 1] / Y_mid
        edges = [(0, 1), (1, 10), (10, 100), (100, np.inf)]
        print("  by exact-chain luminance, multiples of mid-grey, CLEAN cells only, tetrahedral "
              "(n, mean, p99, max):")
        for lo, hi in edges:
        ...
        m = clean & (yrel < 100)
        print("  DECLARED OPERATING REGION -- clean cells, below 100x mid-grey, tetrahedral: "
              "%.1f%% of samples, mean %.4f p99 %.4f max %.4f (trilinear %.4f / %.4f / %.4f)"
              % ((100 * m.mean(),) + stats(per[m]) + stats(per_tri[m])))
    ...
    # The same scene colours at different exposures: the uniform dye-box probe
    # above under-samples the shadows, where a valid, unclamped state can lie
    # in a cell whose other corners are clamped or unconverged. ColorChecker
    # layer exposures scaled by powers of two enter through the forward
    # model; a patch is retained only where the exact inverse returns it
    # unclamped and its scanner input lies inside the cube. Two figures per
    # exposure: the worst-channel relative error of the delivered lookup, and
    # how far a doubling of exposure fails to double the lookup's output.
    # Not restricted to clean cells, and not a perceptual error.
    print("=== ColorChecker exposure sweep, tetrahedral, exact-inverse unclamped and "
          "inside the cube only (worst channel; doubling = |LUT(2x)/(2 LUT(x)) - 1|) ===")
    for stops in (-4, -2, 0, 2):
        src = eng.cc_L * 2.0 ** stops
        sc = eng.L_to_scan_norm(src)
        ok = (np.max(np.abs(np.log2(eng.scan_norm_to_L(sc) / src)), 1) < 1e-6) & np.all((sc >= 0) & (sc <= 1), 1)
        truth = (eng.XYZ_to_DWG @ (eng.M @ src.T)).T
        ap = tetralerp(rt, sc * DMAX)
        rel = (np.abs(ap - truth) / np.maximum(np.abs(truth), 1e-3)).max(1)
        sc2 = eng.L_to_scan_norm(src * 2.0)
        ok2 = ok & (np.max(np.abs(np.log2(eng.scan_norm_to_L(sc2) / (src * 2.0))), 1) < 1e-6) & np.all((sc2 >= 0) & (sc2 <= 1), 1)
        ap2 = tetralerp(rt, sc2 * DMAX)
        dbl = np.abs(ap2[ok2] / np.maximum(np.abs(2.0 * ap[ok2]), 1e-3) - 1.0).max(1) if ok2.any() else np.zeros(0)
        print("  %+d stops: %2d/24 patches retained (%d outside the cube): rel err mean %.4f max %.4f; "
              "doubling error max %s"
              % (stops, int(ok.sum()), int(np.any((sc < 0) | (sc > 1), 1).sum()),
                 rel[ok].mean() if ok.any() else np.nan, rel[ok].max() if ok.any() else np.nan,
                 "%.4f" % dbl.max() if dbl.size else "--"))
    ...
    corners = np.array([[a, b, c] for a in (0.0, DMAX) for b in (0.0, DMAX) for c in (0.0, DMAX)])
    node_at = rt[tuple((corners / DMAX * (SZ - 1)).round().astype(int).T)]
    print("endpoint check: the eight corners of the serialised cube under tetrahedral/trilinear "
          "differ from their nodes by %.1e / %.1e"
          % (np.max(np.abs(tetralerp(rt, corners) - node_at)), np.max(np.abs(trilerp(rt, corners) - node_at))))
