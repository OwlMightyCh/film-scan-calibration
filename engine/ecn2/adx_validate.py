...
scanner density for the chosen sensor. That density goes two ways:
  ...
  A  per-stock scene route: the delivered 'Vision3 <stock> to Scene DWG.cube'
     sampled tetrahedrally (the lookup the Resolve setup requires) -> DWG ->
     XYZ(D65); the exact engine (scan_norm_to_L -> fitted 3x3) is scored
     beside it as a separately named diagnostic, not as the artifact's score;
  B  ADX route: the delivered 'Vision3 to ADX16.cube' sampled tetrahedrally
     -> CSC.Academy.ADX16_to_ACES (numpy port below, constants verbatim from
     the Academy CTL, a2.v1) -> ACES2065-1 -> XYZ(D65), Bradford.

Both cubes are read through engine/common/lut.py; a trilinear line is
printed for the ColorChecker set only, since neither lookup bounds the other.
Before scoring, each cube's header is checked against the requested stock and
sensor, and the eight lattice corners are sampled and compared with their
nodes under both lookups.
...
from engine.common.lut import read_cube, trilerp_unit, tetralerp_unit
SZ = 65
def header(path):
    return [l for l in Path(path).read_text().splitlines()[:8] if l.startswith("#")]
def check_identity(path, must_contain):
    text = "\n".join(header(path))
    for needle in must_contain:
        if needle not in text: raise SystemExit("cube identity: %s does not name %r in its header" % (path.name, needle))
def check_endpoints(lut, name):
    corners = np.array([[a, b, c] for a in (0.0, 1.0) for b in (0.0, 1.0) for c in (0.0, 1.0)])
    node = lut[tuple((corners * (SZ - 1)).round().astype(int).T)]
    dt = float(np.max(np.abs(tetralerp_unit(lut, corners) - node))); dr = float(np.max(np.abs(trilerp_unit(lut, corners) - node)))
    assert dt < 1e-12 and dr < 1e-12, (name, dt, dr)
    print("  endpoints of %s: eight corners differ from their nodes by %.1e tetrahedral / %.1e trilinear" % (name, dt, dr))
...
LUT = read_cube(cube_path, SZ)
check_identity(cube_path, ["ADX16", "sensor: %s" % sensor_label])
print("ADX16 cube: %s (%d^3)   sensor: %s" % (cube_path.relative_to(ROOT), SZ, sensor_label))
check_endpoints(LUT, cube_path.name)
...
for stock in sorted(v3.STOCKS):
    ...
    scene_path = cube_path.parent / ("Vision3 %s to Scene DWG.cube" % eng.name)
    SCENE = read_cube(scene_path, SZ)
    check_identity(scene_path, [eng.p["display_name"], "sensor: %s" % sensor_label])
    DWG_to_XYZ = np.linalg.inv(eng.XYZ_to_DWG)
    route_A_xyz = lambda Dn, fn=tetralerp_unit: (DWG_to_XYZ @ fn(SCENE, Dn).T).T   # the delivered scene cube
    exact_A_xyz = lambda Dn: (eng.M @ eng.scan_norm_to_L(Dn).T).T                     # the exact engine, diagnostic
    print("=== %s (illuminant %s) ===" % (stock, eng.p["scene_illuminant"]))
    print("  scene cube: %s" % scene_path.relative_to(ROOT))
    check_endpoints(SCENE, scene_path.name)
    # grey placement and trims from a flat 18% grey (L = 1,1,1)
    Dn_g = eng.L_to_scan_norm(np.ones((1, 3)))
    cv_g = tetralerp_unit(LUT, Dn_g)
    ...
    print("  both cubes sampled tetrahedrally; 'A exact' is the engine without its cube, a diagnostic")
    print("  %-20s %6s | %-13s | %-13s | %-13s | %-13s | %-13s | %-13s" % ("set", "n", "A scene cube", "A exact", "B ADX raw", "B +exposure", "B +APD trims", "B +AP0 gains"))
    for nm, La, Xa in sets:
        Dn = eng.L_to_scan_norm(La)
        XA = route_A_xyz(Dn); XAe = exact_A_xyz(Dn)
        cv = tetralerp_unit(LUT, Dn)
        ...
        print("  %-20s %6d | %s | %s | %s | %s | %s | %s   (trims vs gains: max dE %.2f; scene cube vs exact: mean %.2f max %.2f)"
              % (nm, len(La), f(XA), f(XAe), f(XB), f(XBe), f(XBt), f(XBg), eng.dE(XBt, XBg).max(),
                 eng.dE(XA, XAe).mean(), eng.dE(XA, XAe).max()))
        if nm == "ColorChecker24":
            XA_tri = route_A_xyz(Dn, trilerp_unit); cv_tri = trilerp_unit(LUT, Dn)
            XBt_tri = aces_to_xyz_d65(adx16_to_aces(apply_apd_trims(cv_tri, trims)))
            print("  %-20s %6s | %s | %13s | %13s | %13s | %s | %13s   (trilinear, for comparison only)"
                  % ("  (trilinear)", "", f(XA_tri), "", "", "", f(XBt_tri), ""))
    ...
    for d in np.arange(-2.0, 2.001, 0.5):
        ...
        YA = float(route_A_xyz(Dn)[0, 1]) / 0.18
        ap0 = adx16_to_aces(tetralerp_unit(LUT, Dn))[0] * s_exp
