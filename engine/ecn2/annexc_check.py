...
The gate is a PROJECT criterion, not a reproduction of the Academy's
experiment: per-channel mean absolute model-to-matrix disagreement <= 0.02 D
over the traced neutral series, the 0.02 D being the expected error the
document states for its fitted transforms over 97 patches (64 of them
colours). The populations differ (a traced neutral series against measured
patches), so the threshold is borrowed, not the residual reproduced. Signed
mean (bias) and pointwise maximum are printed beside it, ungated, and the
family basis the ADX16 cube reads through is reported but does not gate.
Green above 2.1 D and red at D-min carry the largest disagreements; the red
one correlates with the traced mask's D-min disagreement with the sheet but
is not thereby decomposed (mask, matrix fit and dye model are not separated
by this comparison).
...
for stock in args.stocks.split(","):
    ...
    for label, src in (("own dyes + own mask", dj), ("family basis (ADX16 cube)", FAMILY)):
        ...
        straight = D[:, 1] < 2.1   # strictly below: the first point at or above 2.1 D is a shoulder point
...
# ABSOLUTE disagreement over the traced series against a project threshold of
# 0.02 D, the expected-error figure the document states for its transforms
# over 97 patches; the statistic's form matches the document's "mean abs
# error (all)", its population does not, so this is a consistency criterion
# and not the document's residual. A signed mean would let alternating errors
# cancel into a pass. The signed channel mean is printed beside it as the bias
# figure, and the pointwise maximum as the tail; neither is gated.
...
print("\nAnnex C check on the stock's own basis: worst per-channel MEAN ABSOLUTE disagreement %.4f D against the "
      "project threshold of %.2f D (the document's expected error over its 97 patches, a different population): %s"
      % (worst_mae["own"], TOL, "PASS" if gate_own else "FAIL"))
print("  own basis, ungated: worst per-channel signed mean (bias) %.4f D, worst pointwise |error| %.4f D"
      % (worst["own"], worst_max["own"]))
print("family basis (the ADX16 cube's averaging, bounded separately by adx_engine; reported, NOT gated): worst mean |error| %.4f D "
      "(%s the %.2f D threshold), signed mean %.4f D, pointwise max %.4f D"
