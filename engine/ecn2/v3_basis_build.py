...
def load(stock):
    ...
    for k in DYES:
        ...
        out[k] = r
    # the dashed Minimum Density curve: base plus unreacted coloured coupler,
    # absolute, traced by the same tool; NaN outside its traced support
    md = d["minimum_density"]
    w = np.array(md["wavelength_nm"], float)
    v = np.array([np.nan if t is None else t for t in md["density"]], float)
    m = ~np.isnan(v)
    r = np.full(GRID.shape, np.nan)
    inside = (GRID >= w[m][0]) & (GRID <= w[m][-1])
    r[inside] = np.interp(GRID[inside], w[m], v[m])
    out["minimum_density"] = r
    return d, out
...
def main():
    ...
    # family-average Minimum Density, the mask the stock-blind ADX16 route reads
    # the dyes through; per stock the spread is the bound on that averaging
    dm_stack = np.vstack([per[s]["minimum_density"] for s in STOCKS])
    dm_n = (~np.isnan(dm_stack)).sum(0)
    with np.errstate(invalid="ignore"):
        dm_avg = np.nanmean(dm_stack, axis=0)
        dm_sp = np.nanmax(dm_stack, 0) - np.nanmin(dm_stack, 0)
    dm_avg[dm_n == 0] = np.nan
    dm_ok = np.isfinite(dm_sp)
    led = {w: int(np.argmin(np.abs(GRID - w))) for w in (450, 544, 640)}
    dm_spread = {"max_D": round(float(dm_sp[dm_ok].max()), 4),
                 "max_at_nm": int(GRID[dm_ok][int(np.argmax(dm_sp[dm_ok]))]),
                 "median_D": round(float(np.median(dm_sp[dm_ok])), 4),
                 "at_led_peaks_D": {str(w): round(float(dm_sp[i]), 4) for w, i in led.items()}}
    ...
    doc = {
        ...
        "minimum_density": {
            "wavelength_nm": [int(v) for v in GRID],
            "density": ser(dm_avg),
            "units": "diffuse spectral density of the unexposed, processed film: base "
                     "plus unreacted coloured coupler, absolute; family average of the "
                     "four traced dashed Minimum Density curves",
            "n_stocks_contributing": [int(v) for v in dm_n],
            "inter_stock_spread": dm_spread,
            "note": "the mask the stock-blind ADX16 route reads the image dyes through "
                    "(the roll anchor divides it out in integrated density) and the "
                    "APD_Dmin term of ST 2065-3; a stock's own curve is in its own file",
        },
        "audit": {
    ...
    print("  minimum density (family average): support %d-%d nm; at 450/544/640 nm %s; "
          "inter-stock spread median %.4f D, max %.4f D @ %d nm, at the LED peaks %s"
          % (int(GRID[np.isfinite(dm_avg)][0]), int(GRID[np.isfinite(dm_avg)][-1]),
             [round(float(dm_avg[i]), 3) for i in led.values()], dm_spread["median_D"],
             dm_spread["max_D"], dm_spread["max_at_nm"], dm_spread["at_led_peaks_D"]))
