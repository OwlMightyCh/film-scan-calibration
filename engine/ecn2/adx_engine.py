...
from engine.common.lut import trilerp_unit, tetralerp_unit, read_cube
...
# ---- validate the in-memory LUT against truth, in APD density ----
# Both lookups the shared implementation offers (engine/common/lut.py):
# tetrahedral is the scheme the Resolve setup requires and the figure the docs
# quote; trilinear is printed beside it because neither bounds the other.
PROBE_DYE_MAX = 2.2
rng = np.random.default_rng(1); gv = rng.uniform(0, PROBE_DYE_MAX, (5000, 3))
sv = scan_fwd(gv); pv = apd_fwd(gv)
tetralerp = lambda Lut, pts: tetralerp_unit(Lut, pts / DMAX)
trilerp = lambda Lut, pts: trilerp_unit(Lut, pts / DMAX)
for name, fn in (("tetrahedral", tetralerp), ("trilinear", trilerp)):
    pl = adx16_decode(fn(lut, sv))
    print("LUT 65^3 %-11s: RMSE %.4f D  max %.4f D  [APD, working range]" % (name, np.sqrt(np.mean((pl - pv) ** 2)), np.max(np.abs(pl - pv))))
c10 = adx10_code(pv)
...
wl = read_cube(CUBE, SZ)
for name, fn in (("tetrahedral", tetralerp), ("trilinear", trilerp)):
    err = adx16_decode(fn(wl, sv)) - pv
    print("serialized %s %s: RMSE %.4f, max %.4f D  [APD, working range]" % (CUBE.name, name, np.sqrt(np.mean(err ** 2)), np.max(np.abs(err))))
# endpoints: the lattice corners are exact under both lookups (cell clamped before the fraction)
_corners = np.array([[a, b, c] for a in (0.0, DMAX) for b in (0.0, DMAX) for c in (0.0, DMAX)])
_node = wl[tuple((_corners / DMAX * (SZ - 1)).round().astype(int).T)]
print("endpoint check: the eight corners under tetrahedral/trilinear differ from their nodes by %.1e / %.1e"
      % (np.max(np.abs(tetralerp(wl, _corners) - _node)), np.max(np.abs(trilerp(wl, _corners) - _node))))
