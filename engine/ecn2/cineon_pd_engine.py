...
# --sensor: 'none' (the default) presumes no particular camera -- unity response,
# so PHI is the illuminant alone. Name a camera SSF by bare name
# (data/cameras/<name>_ssf.json) or by path to opt in to a particular body; that
# cube is written under builds/sensor-<Body>/.
DEFAULT_SENSOR="none"
...
def resolve_sensor(value):
    ...
    if not p.exists(): raise SystemExit("sensor file not found: %s\n(look in %s)"%(p,cams))
    return p,p.name
def sensor_stem(value):
    ...
    return n
SENSOR_PATH,SENSOR_LABEL=resolve_sensor(_a.sensor); MONO=SENSOR_PATH is None
...
_CH = ("cyan", "magenta", "yellow")
_vals = {k: np.array([np.nan if v is None else v for v in fc[k]], float) for k in _CH}
_meas = [_wl_all[~np.isnan(_vals[k])] for k in _CH]
# The grid spans the UNION of the three dyes' support, as
# reversal_transform.dye_support_grid() does: a wavelength outside EVERY dye's
# support carries no information and is dropped, while one inside ANY dye's
# support still carries that dye's measured value and is worth keeping.
#
# Both ends are additionally CLAMPED to the array's own wavelength range, so a
# support figure can never reach past the data it purports to describe.  That is
# the failure mode which bit the C-41 decomposer, where support was read from a
# metadata field that outran its array by up to 19 nm; see PROJECT.md
# "Invariants".  Here support is derived from the data, so the clamp is
# belt-and-braces rather than a fix, and it costs nothing on the shipped basis.
_lo = max(GRID[0], min(m.min() for m in _meas), _wl_all.min())
_hi = min(GRID[-1], max(m.max() for m in _meas), _wl_all.max())
...
# A union grid can admit wavelengths at which SOME dye has no measured value.
# resample() zero-fills those, modelling that dye as perfectly clear there, which
# is defensible only while the band lies in the dye's far tail.  Report it rather
# than leave it silent: bound the omitted density by the value at the dye's own
# nearest measured endpoint, since a dye tail does not rise as it recedes from
# the peak.  Silent on the shipped basis, where all three dyes span 402-798 nm.
for _k, _m in zip(_CH, _meas):
    _gap_lo, _gap_hi = GRID < _m.min(), GRID > _m.max()
    if not (_gap_lo.any() or _gap_hi.any()):
        continue
    _v, _ok = _vals[_k], ~np.isnan(_vals[_k])
    _edge_lo = float(_v[_ok][0]) if _gap_lo.any() else 0.0
    _edge_hi = float(_v[_ok][-1]) if _gap_hi.any() else 0.0
    _omitted = _edge_lo * _gap_lo.sum() + _edge_hi * _gap_hi.sum()
    _total = float(np.trapezoid(resample(_wl_all[_ok], _v[_ok], GRID), GRID))
    print("  WARNING %s dye zero-filled over %d nm inside the grid "
          "(measured %.0f-%.0f nm); omitted density bounded by %.4f D.nm, "
          "<= %.2f%% of its in-grid integral"
          % (_k, int(_gap_lo.sum() + _gap_hi.sum()), _m.min(), _m.max(),
             _omitted, 100.0 * _omitted / _total if _total else float("nan")))
...
def _dye(channel):
    ...
    synthesized tail enters the interpolation.

    GRID spans the UNION of the three dyes' support, so a channel whose own
    support is narrower is zero-filled over the difference -- resample() returns
    0 outside its support, not the terminal value. On the shipped Vision3 basis
    all three dyes span 402-798 nm and no channel is zero-filled, but that is a
    property of the data rather than a guarantee of this function; the loop
    above reports and bounds any band where it does occur.
    """
...
if _a.out_cube: CUBE=Path(_a.out_cube)
elif MONO: CUBE=BUILDS/"ecn2"/"Vision3 to Cineon PD.cube"
# A per-apparatus build lands beside, never on top of, the canonical cube.
else: CUBE=BUILDS/("sensor-%s"%sensor_stem(_a.sensor))/"ecn2"/"Vision3 to Cineon PD.cube"
