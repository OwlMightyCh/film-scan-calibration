...
def trilerp(lut, points, dmax):
    return trilerp_unit(lut, points / dmax)


def tetralerp(lut, points, dmax):
    """Tetrahedral lookup, the scheme docs/resolve.md sets Resolve to (engine/common/lut.py)."""
    return tetralerp_unit(lut, points / dmax)
...
def sheet_neutral_closure(config, grid, dye, scan_forward, target_forward,
    ...
    right if it describes this film:
      ...
      * the D50 colorimetry of that series is expected to be near neutral,
        a reversal film's daylight neutral series being what it is designed
        to render neutral on the light table; the sheet's daylight exposure
        and Status A convention do not fix exact D50 neutrality at every
        exposure, so the a*/b* printed here is an unresolved consistency
        signal (register #19), never a correction to fit. a*/b* is the
        traced dyes, the Status A responsivities and the CIE observer
        disagreeing about one set of amounts.

    A tetrahedral lookup of the written cube on the same series is printed
    too, but that line is plumbing: the LUT and the reference share the
    machinery.
    """
    ...
    chain = tetralerp(written_lut, scan_forward(amounts), corridor) - colorimetric
...
def base_term_bound(config, grid, scanner, dye, status_a_dmin, scan_forward):
    ...
    wavelengths and joined by straight lines, one smooth tint the
    densitometer readings admit and no extremal one: the figure is a
    sensitivity to this surrogate, never a bound over other bases. The scan side then sees the dyes through
    ...
    # The surrogate must reproduce the triplet it was built from when read
    # back through the Status A responsivities; otherwise it is not a base
    # the sheet's readings admit.
    reread = -np.log10((status_a / status_a.sum(1, keepdims=True) * 10.0 ** (-base)).sum(1))
    print(f"{config.name}: base surrogate re-read under Status A "
          f"{reread[0]:.4f}/{reread[1]:.4f}/{reread[2]:.4f} against the sheet triplet "
          f"{status_a_dmin[0]:.3f}/{status_a_dmin[1]:.3f}/{status_a_dmin[2]:.3f}; "
          f"max |diff| {np.abs(reread - status_a_dmin).max():.4f} D")
...
TAIL_BRACKET_DYES = (1.0, 2.0, 2.5)


def tail_bracket(config, grid, dye):
    """The bracket hard constraint 1 requires around the truncated observer.

    The cube integrates the D50 observer over the dye support only,
    renormalised, which asserts that each unmeasured tail transmits like the
    in-band mean. That is one point, never the bound. The admissible extremes
    are printed per tail on a neutral stack: below the grid the yellow dye and
    the base absorb, so the tail is bracketed by BLOCKED against the edge
    value HELD; above it cyan is still dense at the chart edge and real cyan
    dyes clear towards the infrared, so the bracket is HELD against CLEAR.
    Each figure is the colorimetric density that tail policy would give minus
    the truncated figure the cube ships. Where a dye is measured to zero at
    the edge, held and clear all but coincide (the other dyes' edge density
    keeps them apart) and the truncation can sit outside the bracket; the
    line makes that visible.
    """
    cmfs = json.loads((DATA / "standards" / "CIE1931_2deg_CMFs.json").read_text())
    d50 = json.loads((DATA / "standards" / "D50_illuminant.json").read_text())
    full = np.arange(360.0, 831.0)
    weights = np.stack([resample(np.array(cmfs["wavelength_nm"], float), np.array(cmfs[k], float), full)
                        for k in ("x_bar", "y_bar", "z_bar")])
    weights = weights * resample(np.array(d50["wavelength_nm"], float), np.array(d50["spd"], float), full)
    inside = (full >= grid[0]) & (full <= grid[-1])
    below = weights[:, full < grid[0]].sum(1)
    above = weights[:, full > grid[-1]].sum(1)
    total = weights.sum(1)
    omitted_below = below / total * 100
    omitted_above = above / total * 100
    print(f"{config.name}: tail bracket (hard constraint 1; the cube ships the truncated observer): "
          f"D50 observer weight omitted below {grid[0]:.0f} nm X/Y/Z "
          f"{omitted_below[0]:.3f}/{omitted_below[1]:.3f}/{omitted_below[2]:.3f}%, above {grid[-1]:.0f} nm "
          f"{omitted_above[0]:.3f}/{omitted_above[1]:.3f}/{omitted_above[2]:.3f}%")
    neutral = dye.sum(0)
    for amount in TAIL_BRACKET_DYES:
        transmission = 10.0 ** (-amount * neutral)
        in_band = (weights[:, inside] * transmission).sum(1)
        truncated = -np.log10(in_band / weights[:, inside].sum(1))
        edge_low, edge_high = transmission[0], transmission[-1]

        def with_tails(low, high):
            return -np.log10((in_band + below * low + above * high) / total) - truncated

        blocked = with_tails(0.0, edge_high)
        held = with_tails(edge_low, edge_high)
        clear = with_tails(edge_low, 1.0)
        print(f"{config.name}: tail bracket neutral dye {amount:.1f}: below-grid tail blocked/held "
              f"Z {blocked[2]:+.4f}/{held[2]:+.4f} D; above-grid tail held/clear "
              f"X {held[0]:+.4f}/{clear[0]:+.4f} D (Y {held[1]:+.4f}/{clear[1]:+.4f}); "
              f"cyan at the red edge {dye[0, -1] * amount:.2f} D")
...
def build(config, sensor=DEFAULT_SENSOR, corridor=None):
    ...
    reached = residual <= REACH_TOLERANCE_D
    # Reached is numerical convergence, not physical admissibility: a node
    # can close on the scanner with a negative dye amount. Those are kept
    # (clamping them displaces real colours at the boundary) and counted.
    negative_reached = int((reached & (dye_nodes.min(1) < -1e-6)).sum())
    at_floor = int((reached & (dye_nodes.min(1) <= -0.5 + 1e-9)).sum())
    ...
    # Exact endpoints, cube faces and the corridor corner are added because a
    # uniform draw never lands on them; those are where a lookup's clamp shows.
    corner_axis = np.array([0.0, corridor])
    corners = np.array(np.meshgrid(corner_axis, corner_axis, corner_axis, indexing="ij")).reshape(3, -1).T
    faces = np.random.default_rng(2).uniform(0, corridor, (600, 3))
    faces[np.arange(600), np.arange(600) % 3] = np.where(np.arange(600) % 2 == 0, 0.0, corridor)
    edge_density = np.vstack([corners, faces])
    edge_amounts = edge_density @ inverse_fit
    for iteration in range(iterations):
        current, jacobian = scan_jacobian(edge_amounts)
        step = np.linalg.solve(jacobian, (current - edge_density)[:, :, None])[:, :, 0]
        edge_amounts = np.clip(edge_amounts - step, -0.5, 8.0)
    edge_reached = np.max(np.abs(scan_forward(edge_amounts) - edge_density), axis=1) <= REACH_TOLERANCE_D
    edge_samples = edge_amounts[edge_reached]   # only densities the film reaches have an exact value
    exact = target_forward(samples)
    scan = scan_forward(samples)
    error = trilerp(written_lut, scan, corridor) - exact
    error_tet = tetralerp(written_lut, scan, corridor) - exact
    edge_scan = scan_forward(edge_samples)
    edge_error = tetralerp(written_lut, edge_scan, corridor) - target_forward(edge_samples)
    print(f"{config.name}: grid {grid[0]:.0f}-{grid[-1]:.0f} nm (derived from dye support)")
    tail_bracket(config, grid, dye)
    print(f"{config.name}: sensor {sensor_label}")
    print(f"{config.name}: node residual mean {residual.mean():.4f}, max {residual.max():.4f} D")
    print(f"{config.name}: {reached.sum()} reachable nodes, of which {negative_reached} "
          f"({100 * negative_reached / max(reached.sum(), 1):.1f}%) close only with a negative dye amount "
          f"({at_floor} at the -0.5 floor) and are kept; "
          f"{unreachable} unreachable projected onto the nearest reachable node")
    print(f"serialized {cube_path.name}: tetrahedral (deployment, docs/resolve.md) RMSE "
          f"{np.sqrt(np.mean(error_tet**2)):.4f}, max {np.max(np.abs(error_tet)):.4f} D; "
          f"trilinear RMSE {np.sqrt(np.mean(error**2)):.4f}, max {np.max(np.abs(error)):.4f} D "
          f"(neither bounds the other)")
    print(f"serialized {cube_path.name}: tetrahedral on the corridor corners and faces "
          f"(reached {len(edge_samples)} of {len(edge_density)}, exact chain through the solved amounts): "
          f"RMSE {np.sqrt(np.mean(edge_error**2)):.4f}, max {np.max(np.abs(edge_error)):.4f} D")
