...
2. **The fleet cannot distinguish every stock from every other.** Basis
   sensitivity is 0.032–0.063 D while inter-stock distances run 0.021–0.219 D,
...
| **Basis sensitivity** | the displacement of a fitted result when the assumed dye basis is changed. It is 0.032–0.063 D here, and it bounds every stock comparison |
...
between 1.64 D of scan density (Pro 400H) and 2.26 D (Portra 400), so the
...
0.0045; the serialised cube over the whole declared corridor 0.0124 D and
0.7159, over the 98% of samples above the output floor. Both are printed.
...
basis sensitivity of the surrogate decomposition is **0.032–0.063 D**. Below
...
| Ektar 100 | `print_endura/` | 0.3559 · 36.5% | 0.0005 / 0.0094 |
| Fujicolor 100 [JP] | `print_fuji/` | 0.3609 · 34.3% | 0.0004 / 0.0075 |
| Pro Image 100 | `print_endura/` | 0.3689 · 38.2% | 0.0006 / 0.0103 |
| Gold 200 | `print_endura/` | 0.3842 · 40.2% | 0.0006 / 0.0139 |
| Portra 160 | `print_endura/` | 0.3845 · 40.6% | 0.0005 / 0.0083 |
| Portra 800 | `print_endura/` | 0.3869 · 40.0% | 0.0007 / 0.0122 |
| Ultra Max 400 | `print_endura/` | 0.3891 · 40.9% | 0.0006 / 0.0137 |
| Portra 400 | `print_endura/` | 0.3918 · 37.7% | 0.0005 / 0.0102 |
...
anomaly tracks the artwork: the two best-fitting
...
8. **Per-layer C-41 dyes are inferred, across the whole
   ...
   |---|---|---|---|---|
   | Portra 800 | 0.0078 | 0.0339 | +21.97 | 1.4135 |
   | Ultra Max 400 | 0.0083 | 0.0439 | +22.27 | 1.4182 |
   | Ektar 100 | 0.0086 | 0.0727 | +22.48 | 1.4079 |
   | Gold 200 | 0.0088 | 0.0396 | +21.28 | 1.4140 |
   | Pro 400H | 0.0107 (in-band 0.0109, traced tail 701–716 nm 0.0021) | 0.0302 | +14.93 (free; magenta width pinned at 1.15) | 1.4607 |
   | Portra 400 | 0.0108 | 0.0686 | +23.71 | 1.4175 |
   | Pro Image 100 | 0.0135 | 0.1244 | +25.00 (free, 0.004 nm inside the bound) | 1.4032 |
   ...
   - **Every stock's cyan sits to the red of the Vision3 basis cyan**, sC
     ...
     within 0.01 nm of it, so for three of the twelve the value is the
     constraint. The fit establishes a
     direction only; the other nine fit freely between
     +12.3 and +23.7 nm, Pro 400H at +14.93 with its traced red tail in the
   ...
   - **The shift bound is a uniform ±25 nm on every stock** (`portra_stocks.py`),
     ...
     ±25 moves which stocks pin and how many, three of twelve resting on
     ...
     0.0171, 0.0148, 0.0132, 0.0126 and 0.0126 D, so the solve converges by
     ±35, with no stock pinned there and the largest fitted shift +30.0 nm.
     Releasing the bound that far buys 5% of residual and
     ...
     which hard constraint 1 forbids. Tightening to ±20 or ±15 costs 12% and
     30% of residual and leaves the peak extrapolated regardless. Only data would
     ...
     component. Fujicolor 100 and Pro 400H overshoot it by
     0.045 and 0.050 D mean, the modelled cyan lying too far red. The shipped
     Pro 400H fit admits its tail (registry `fit_traced_tail`; the 16
     samples enter as extra residual terms on their own wavelengths, the
     output curves staying on 400–700 nm): against the 400–700 fit that
     releases the +25.00 pin to +14.93, moves the cyan peak from 713.7 to
     699.3 nm and costs 0.0107 → 0.0109 D in-band, the tail residual
     falling to 0.0021 D as training error; the pinned solution is one of
     two basins 2.4% apart in residual, and the withheld band selects the
     other, at the price of the magenta width landing on its 1.15 bound.
     Downstream, against the cubes built from the 400–700 fit: the Status
     M cube moves its equal-scan-density diagonal (not thereby a
     photographed neutral) by 0.010 D mean and 0.022 max, and
     the unsaturated working-range lattice (scan density to 2.30 D) by a
     median of 0.013/0.008/0.005 D R/G/B with a 90th percentile of
     0.041/0.034/0.011, the neutral-axis check improving from −0.008 to
     −0.003 D in red; the Fuji print cube moves that diagonal by at
     most 0.007 (Display P3 encoding) and the working range by a median of
     0.001–0.003 with a 90th percentile of 0.006/0.014/0.045, the gray lock
     unchanged (RMS 0.030 D, series |b*| max 3.0). Inverted on a common
     working-range population, the two dye sets agree where both solve
     (residual mean 0.0002 D on 1,947 of 3,375 sampled nodes) and differ in
     reach: 62.1% of the sample solves under the 400–700 set, 60.2% under
     the shipped one, 4.4% only under the former and 2.5% only under the
     ...
     within 5% of each stock's best residual the cyan peak moves 0.6 nm on
     Fujifilm 200/400, 3–7 nm on six further stocks and 12–17 nm on Portra
     160, Portra 800, Pro Image 100 and Pro 400H; the same four stocks are the ones whose
     peak moves 11–16 nm across the four individual Vision3 stock bases and
     the family average where the other eight move under 3 nm, ambiguity
     ...
     inverts validly) deviates in Status M by up to 0.089 D in green on
     unequal-amount inputs for Portra 160, 0.070 Pro 400H, 0.060 Portra
     800, 0.057 Superia Premium 400, 0.052 Pro Image 100, 0.040 Fujicolor
     100, and 0.003–0.026 D on the other six; equal-amount inputs, which
     are not a photographed neutral since the curves are independently
     normalised, deviate 0.003–0.046 D. The two figures are not a ratio.
   ...
   - **A stock that pins more than one bound simultaneously signals
     ...
     widths. Nine of the twelve stocks fit against a bound, only Portra 160,
     Portra 800 and Pro Image 100 being free of one: Pro 400H pins the 1.15
     magenta width (its cyan shift, pinned at +25.00 on the 400–700 fit,
     comes free once the traced red tail above is in the objective); Ultra Max 400 pins the 1.15 cyan width together with the 0.85
     magenta width; Ektar 100, Gold 200, Portra 400, Fujicolor 100 and
     Superia Premium 400 pin the 1.15 cyan width. That the bound binds this widely under the
...
9. **The shipped cubes embed no camera, and naming one is worth a bounded and
   ...
   *The bound is a function of density.* Substituting each of the forty-four
   ...
   - **Below approximately 1.2 peak dye the camera is a second-order term**,
     ...
     climbs past the 0.032–0.063 D basis-sensitivity band of entry 8. A
...
11. **Reversal interimage is unmodelled, and no structure exists for it.**
   ...
   §4a.
12. **Most of the C-41 fleet fits against a bound.** Nine
   of the twelve stocks rest at least one shape parameter exactly on its
   limit; only Portra 160, Portra 800 and Pro Image 100 are free of one.
   Three pin the cyan shift at +25 nm (Pro 400H and both Fujifilm stocks)
   and eight pin a width: Ektar 100, Ultra Max 400, Gold 200, Portra 400,
   Fujicolor 100 and Superia Premium 400 at the 1.15 cyan ceiling, Ultra Max
   400 also at the 0.85 magenta floor, and the two Fujifilm stocks at the
   0.85 cyan and magenta floor. A
   ...
   method, and on nine of the twelve stocks a seeded 64-point multistart
   over the same bounds finds a strictly better optimum than a single fixed
   start, by up to 29.5% in RMSE on Ektar 100. `fit_audit.multistart`
   records the seed, the start count, the single-start RMSE and the
   improvement. The ±25 nm bound is load-bearing for four of the twelve,
   counting Pro Image 100 at 0.004 nm inside it, and the fitted cyan shift is
   not a per-stock measurement for those stocks; for the other eight the
...
17. **The roll anchor subtracts D-min in integrated density, and the engines
   ...
   *The reversal path carries the same term under a surrogate base.* No
   ...
   the surrogate: a base whose tint varies within an LED's band and
   ...
   *The missing band as a term.* Truncation at the support edge is exact for
   ...
   the shipped truncation, each with its own re-solved lock, gives 0.31 mean
   / 1.79 max ΔE2000 off-neutral on Portra 400 and 0.64 / 1.68 on Fujifilm
   400, the fourth-largest term on Portra and the third on Fujifilm; clear
   film in those bands, a permissive diagnostic and not a model of the
   missing spectrum, gives 8.1 / 13.0 and 9.9 / 14.9 and is printed beside
...
- **The C-41 fleet cannot distinguish every stock it holds by their modelled
  dye sets.** Basis sensitivity is 0.032–0.063 D against inter-stock
  distances of 0.021–0.219 D, so 20 of the 66 pairs sit inside the ambiguity
...
| term | mean | max |
|---|---|---|
| basis sensitivity, six refits under different surrogate bases, propagated through the scan inverse and the print | **1.89** | **8.66** |
| the same at fixed Status M, print engine only (comparison only) | 1.26 | 7.46 |
...
| **combined, root-sum-square** (under independence; no bound) | 2.00 | 9.08 |
| combined, plain sum (aligned perturbations) | 3.20 | 14.37 |

Fujifilm 400 on Fuji Pro Laser, 285 reachable patches: basis 3.21 / 12.50,
fit residual through the scan 0.73 / 2.03 (0.35 / 1.51 at fixed Status M),
missing-band policy 0.64 / 1.68, composed cubes 0.04 / 0.46, paper dye
wavelength axis 0.20 / 0.99.
...
Portra 400 and 9.33 / 26.62 on Fujifilm 400, the largest figure on the page
...
- **The figure these documents lead with is among the smallest terms that
  ...
  sensitivity contributes 1.89, a factor of six; a reader who takes the
...
- **The budget is one term, and it is consumed twice.** The dye set enters
  ...
  alternative basis also performs the scan conversion, gives 1.89, and it is
  ...
  term, the paper's dye digitisation, sitting a factor of four below. The
