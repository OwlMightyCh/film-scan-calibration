...
quantity, free of the combined idiosyncrasies of the scanner and its
...
- **Negative / ECN-2** (Kodak Vision3 50D/200T/250D/500T) → ADX16 (ST 2065-3)
  ...
  characteristic curves. It is a per-stock, model-based estimate of relative
  scene exposure, the model reference the ADX16 route is scored against
  (ColorChecker ΔE2000 6–7 mean after the printer-light trims), and the path
  for graded work; no frame has been checked against a known scene.
...
1. **The chain functions, although no number within it has been checked
   ...
   disagreement over the traced neutral series being 0.018 D on each stock's
   own dye set against a project threshold of 0.02 D, the expected error the
   document states for its transforms over 97 measured patches, a different
   population, with per-point maxima to 0.035 D and the family basis the
   ADX16 cube reads through at 0.024 D, reported and not gated. It
...
2. **The fleet cannot distinguish every stock from every other.** Basis
   sensitivity is 0.027–0.049 D while inter-stock distances run 0.021–0.219 D,
   so 16 of the 66 pairs sit inside the ambiguity band and cannot be told apart
   by their modelled dye sets; the remaining 50 are separated by more than the
...
| class | meaning |
|---|---|
| **measured** | read from a manufacturer's published chart, or measured on this apparatus |
| **computed** | a numerical property of this repository's software, obtained by running an engine over the measured data; it establishes implementation consistency and model sensitivity, never physical accuracy |
...
| the aggregate fit residual, node-solve residual and serialisation figures | computed, by running the engines |
| the Vision3 dye model's Status M to APD relation for 5207 and 5219 | computed against the Academy's published S-2008-002 Annex C matrices: worst per-channel mean absolute disagreement 0.018 D on the stock's own basis along the traced neutral series, gated at a project threshold of 0.02 D borrowed from the expected error the document states over its 97 patches (a different population, so a consistency criterion and not the document's residual); signed mean (bias) and per-point maxima printed beside it ungated, 0.018 D and 0.035 D; family basis 0.024 D mean absolute with per-point maxima to 0.037 D, reported and not gated; a FAIL exits nonzero; the scan side is untouched by it |
| the end-to-end error budget | derived, from computed perturbations |
| the surrogate Vision3 basis standing in for C-41 couplers | **assumed** |
| the uniform ±25 nm shift bound and the uniform per-dye width bounds, cyan 0.85–1.15, magenta and yellow 0.5–2.0 | **assumed** |
| `DMAX = 3.30`, the negative corridor | **assumed**, with the requirement computed |
| `DIR_MATRIX = identity`, interimage disabled | **assumed**; harmless where a transform ends in a density, since the developed film carries its coupling in the measured densities, and a modelling assumption on the Vision3 scene route, which inverts those densities to exposure: the neutral series cannot choose it (Known limitations) |
| that each Vision3 layer's response away from the neutral series is its neutral-series amount table, and that the sheet's spectral sensitivity is an exposure-invariant additive kernel | **assumed**; the sheet's neutral series is one path through a three-dimensional response, and its sensitivity chart is defined at one density criterion |
...
| D65 viewing for the print render, D50 for the reversal target | **assumed**; D65 against D50+CAT02 computed at ΔE00 median 0.75, 0.017 on the neutral axis |
| that the rendered cast of a frame matches its physical illuminant | **unverified** on film; hard constraint 5 guarantees only that no stage neutralises it |
...
`*_datasheet_curves.json`, all eleven `*_spectral_sensitivity.json` and all
sixteen `*_mtf.json` carry a `digitization_audit` block naming the source sheet, the device-to-data
...
carry neither, their per-layer curves being traced directly, which
...
| **Scene-linear** | relative scene exposure, 10^(logH − logH_mid) per layer, the exposure scale set by the datasheet's midscale neutral (its three per-channel inversions averaged) so that a flat 18% grey on the characteristic series lands at 0.18, delivered as DWG (D65). The Vision3 scene route's output, and the only transform destination here that is not a density |
...
| | |
|---|---|
| **Narrowband / trichrome** | three sequential exposures under R/G/B LEDs at 640/544/450 nm (on this apparatus a CuteNew RGB Pro Light Source, no affiliation), in place of a single white-light exposure. The only supported mode here |
...
| **Basis sensitivity** | the displacement of a fitted result when the assumed dye basis is changed. It is 0.027–0.049 D here, and it bounds every stock comparison |
| **Serialisation RMSE** | the error introduced by writing a cube's floating-point values at six decimal places |
| **MTF** | modulation transfer function, the sharpness chart on the datasheets, traced into `data/films/<Stock>_mtf.json` for the sixteen sheets that print one |
...
- **A stale linked worktree is a full second checkout**, `builds/` included,
  ...
  commits reachable.
- **Diagnostic artifacts are untracked.** `builds/_forensics/` and
  `data/films/_ensemble/` hold overlay renders and basis-sensitivity reruns
  (ensemble cubes, when built, go to a gitignored `_ensemble/` beside them
  under `builds/`). `.gitignore` does not untrack retroactively:
...
1. **No unbracketed synthesis of spectral shape.** A dye, sensitivity or
   ...
   - **A bracket**, the standard treatment of a band outside support: the two
     ...
     register as the bracket's spread: the extremes bracket the input and
     the entry records what each does at the output, which is not a proof
     that every admissible prior lands between them once a nonlinear
     inverse has re-solved. Truncation with renormalisation is one point
...
2. **Numerical grounding takes precedence over qualitative review.** Every
   ...
   negligible, whereas it reaches 0.24 D in deep shadow. The precedence runs
   over judgement alone: a computation over this repository's
   data establishes what the model does and how sensitive it is, never that
   the model is right, since a closure the model's own forward produces is
   exact by construction (invariant 10). A reproducible observation on real
   film outranks every synthetic check that passes and opens an
   investigation; a pleasing image justifies no change to spectral data
   until a reference measurement has scored it. Match the evidence to the
   claim: a computed figure bounds implementation consistency and
   sensitivity, an independent reference measurement bounds accuracy over
   its stated domain, and visual review establishes usability and can
   expose a failure the numerical probes omit without establishing any
   numerical bound (Evidence classes).
3. **Validate the artifact as shipped.**
   Every `.cube` is quantised to six decimal places on write, and the
   density and print cubes are clipped to [0,1] besides, the scene cubes
   alone being written unclamped. Re-parse the written file and validate
   that, through the interpolation the host applies.
4. **Metric and aesthetic operations remain separated.** An operation is
   classified by what it estimates. A per-channel
   subtraction that estimates a measured physical quantity, the roll
   anchor's D-min or the mask-filtered illuminant of register #17, belongs
   inside the metric and is recorded with its measurement. A per-channel
   log-space offset chosen for the look (printer lights, white balance, CC
   filtration) is aesthetic and belongs in a node *above* the metric
   transform. Folding the latter into the metric, for instance by adopting
   a neutral point set by eye, corrupts it; this is the central criticism of
   NamiColor's reversal-mode workflow. A rendering convention inside the
   metric is recorded as one: the gray-axis lock places the sheet's neutral
   series on the diagonal by per-channel maps that vary with density, its
   mid-grey intent is assumed (Evidence classes), and neither its fixity nor
   its datasheet provenance makes it evidence that an enlarger behaves that
   way. The reversal base-white mapping is a second rendering convention.
   The roll anchor places the measured D-min at density 0.0, and the
   reversal cubes integrate only the dyes above it, so the film base renders
   as D50 white. The mapping removes the base's attenuation and its tint.
   It is a base-white rendering with a base-free dye model: the target
   integrates the image dyes alone, and exact film-relative colorimetry
   would also weight the target by the base spectrum. The register #17
   surrogate, built from each sheet's Status A D-min triplet, gives
   0.085–0.159 D visual density on the D50 table and, at the base's own L\*,
   a\* +0.65 / b\* −0.81 on Ektachrome E100, a\* −0.28 / b\* +0.14 on
   Velvia 100 and within 0.01 of zero on Provia 100F and Velvia 50. These
   are computed from one surrogate spectrum per stock; the sheets do not
   specify a base colour. Three channel readings do not fix the base colour
   either: a smooth base that matches the E100 surrogate's scanner readings
   exactly can sit 1.23 ΔE\*ab from it. To render the base's own colour,
   the chain needs a measured base spectrum or base colorimetry and a
   declared output white.
5. **No scene-dependent decisions occur anywhere in the chain.** Every
   ...
   anchor). Ordinary picture content never determines the transform. The two
   normalisations that could have introduced such a dependence are keyed to
   scene-independent references by construction: the roll anchor to
   unexposed film base, light
   that never formed an image, and the gray-axis lock to the stock's own
   published neutral series (the three characteristic-curve records solved
   together through the spectral base, placed on the diagonal by the Status M
   cube), solved once per stock and paper. A reference frame counts as a
   measurement: a target photographed on the roll under
   a declared illuminant, and identified as such, may calibrate a per-roll
   parameter the way the film base calibrates the anchor, provided the
   fitted parameters, their reference and the frames they govern are
   recorded and held fixed across those frames, and the correction stays as
   low-dimensional as the reference supports (one grey patch supports a
   constant balance and exposure, never a matrix; a grey card under an
   undeclared scene illuminant supports nothing, since the cast it shows is
   the one to preserve). Diagnostics may read content to flag clipping, an
   unreachable node or an inversion failure, and alter no value.
   Consequences: no stage neutralises a frame's illuminant, so a
   tungsten-lit frame stays warm as this emulsion and this paper render it,
   a photographic record of the light, and
   frame-to-frame relationships across a roll survive. Whether the rendered
   cast matches the physical illuminant is a separate, empirical claim,
   unverified on film (Known limitations): frame-independent parameters are
   an architectural guarantee, and a fixed nonlinear transform can still
   move a cast. The property is one-directional, since a cast can be removed
   later in the grade whereas a cast removed by a content-dependent
   estimator cannot be restored. Any automatic neutralisation, automatic
   exposure or content-driven correction belongs above the metric transform,
   per constraint 4.
...
together with broadband white W1–W100 columns that are unused. The unit
measured is a CuteNew RGB Pro Light Source (https://cutenewdesign.com),
named for reproducibility only: the engines consume the measurement, the
figures describe this one unit, and the project has no affiliation with, and
no sponsorship from, its maker.
...
- `<Stock>_mtf.json` for the sixteen sheets that print a modulation transfer
  function chart: the four Kodak professional C-41 stocks and Ektachrome E100
  (three labelled records each), the four Vision3 stocks (three records, one
  order per sheet), and the seven Fujifilm sheets (one unlabelled curve). Each
  carries the traced curve on a fixed log grid inside its printed support, a
  summary (MTF50, MTF30, MTF20, response at fixed frequencies, chart end) and
  a `digitization_audit` block with both log-axis fits, the gridline residual,
  the numeral check and the channel-letter pairing. Where the printed curves
  cross inside one line width on a bitmap sheet the affected span is listed
  per record and the sheet carries a `crossing_bracket` instead of a
  per-record MTF50 (Vision3 50D). The Kodak consumer sheets (Gold 200,
  Ultramax 400, Pro Image 100) and the 2023 Fujifilm 200 and 400 sheets print
  no MTF chart. Not consumed by any engine; the evidence behind the scanning
  resolution note.
...
density silently (register #5). The corridor, the lattice and the print
pivot are conventions with a computed requirement behind them: a change to any of them moves the shapers, the paired cubes, the
DCTLs, the identity checks and the instructions together, and is judged on
what the shipped chain gains, never adopted silently.
...
interpolation error scales as the square of spacing (measured under the
trilinear probe: 4.5 to 6.0 at 33³ raises error by 1.78 against 1.78
predicted). Under that probe, at a 6.0 corridor the a7R III reversal builds
measure RMSE 0.0004 D, maximum 0.0012 D, and reach 58–62% of the lattice; at
5.25, 0.0003 D and 0.0009 D and 68–73%, which the tetrahedral lookup the
Resolve setup requires brings to **0.0002 D and 0.0005 D**. A 4.5 corridor is
...
of 2.2e-03 with a worst case of 7.7e-02, 20 code values out of 255, its PQ
pair 3.5e-03 and 1.3e-01. Those are the print branch's real lattice accuracy,
the largest such error in the chain.

**The negative corridor carries deliberate headroom.** `DMAX = 3.30` is
uniform across the twelve stocks, whose published neutral series reach
between 1.63 D of scan density (Pro 400H) and 2.26 D (Portra 400), so the
corridor holds 46% to 101% more than any datasheet documents. Real film is exposed past the
...
artifact, already one to two orders below the 0.027 to 0.049 D basis
...
shaper DCTLs). The accuracy probe over the working dye range, reaching 2.15 D
on Portra 400, reports a tetrahedral RMSE of 0.0003 D and a maximum of
0.0045; the serialised cube over the whole declared corridor 0.0122 D and
0.7153, over the 98% of samples above the output floor. Both are printed.
...
to 14–36% at +1.5 stops, which the printer-light trims remove at one density
...
of k = 0.22 returns 0.4613/0.4612/0.4613, which is 18% grey through the sRGB
transfer function (0.4614). That diagonal input is the stock's own published
neutral: a C-41 neutral does not read equal in
Status M (Portra 400's sits 0.10/0.06 D below and 0.16 D above equal channels
in R/G/B at mid-grey), so the Status M cube subtracts each stock's neutral
offset, solved from its characteristic-curve series through the spectral base
(`engine/c41/neutral_locus.py`, shared with the print engines, which add it
back before inverting), and writes the offset into its header. The print
cube's diagonal is therefore the sheet neutral, Print Adjustment's per-channel
pivot sits on it, and a gamma about the pivot leaves a neutral neutral. The
encoding is not reversible at its floor: a channel whose image-dye density is
below a positive offset goes negative in balanced density and the cube clips
it, and the print engine cannot recover what was clipped. Every print build
measures the loss on a uniform dye 0–2.2 probe, exact render before against
after the clip: 4.9% of samples below the floor on Portra 400, mean 0.49
ΔE2000 on that share and 2.50 at worst; 3.8%, 0.38 and 2.81 on Ektar 100;
1.0%, 0.01 and 0.07 on Fujifilm 400; none on Pro 400H. All of it sits at the
toe of one record on a strongly off-neutral negative; a fleet-wide shift of
the encoding would remove it at the cost of moving the mid-grey pivot on
every cube, and is not taken on these figures: information lost against a
pivot kept is a tradeoff the probe re-measures on every build, and a
reversible shifted encoding is a legitimate successor if the loss grows. Do
not
place a colour space transform after it on the assumption that it emits
scene-linear data. For quality control, stop after the Status M cube,
multiply by 3.30 and add the header's neutral offset: the result is the
Status M density of the image dyes alone, which is not what a densitometer or
the E-4050 characteristic curves read (those include base and fog). The
engine prints the full reading beside it, the chain's dyes on the base; adding
the sheet's integrated D-min instead is off by 0.009 D in green on Portra 400.
Printing density was deliberately avoided for
...
reference to the actual roll, per roll, because
...
3. `portra_decompose.py`: a nine-parameter warped-basis fit, over per-dye
   ...
   0.0108 D, Status M reproduction deltas 0.004, −0.001 and −0.002 D in R, G
   and B, and an LED crosstalk condition number, for the LEDs as the roll
   anchor leaves them behind the mask, of 1.4175. The solve is a seeded
...
4. `c41_statusm_engine.py`: the scanner-to-Status M cube, D-min excluded,
   ...
   (0.49% of the red area on the 1 nm integration grid; the 10 nm source
   table sums to 0.28%, a different number). Its scan-side responsivity is the LED (times the
...
with 18% gray pinned to DWG 0.18. Its chain renders the Portra 400 sheet's
neutral midscale magenta (DWG R/G 1.30, B/G 1.24), and no cube from it ships.
It is the only home for the broad-set 3×3
...
no claim about which factory coated the film. The guard is a shipped default
against an accidental cross-pair and carries no claim of physical
invalidity: the paper is a rendering choice, and a cross-paired
build needs only the engine offered the stock, which none does.
...
that the stock's published neutral series prints neutral: the diagonal of the
input space, where the Status M cube has placed that series by subtracting its
offset (`neutral_locus.py`; the engine adds it back and refuses a Status M
cube whose header carries a different offset). The lock is calibrated on a
constant offset; the series' own curvature about it is reported (within 4.4
units of a\* and 4.1 of b\* on Portra 400 over k = 0.22 ± 0.12) and never
fitted, and the lock's departure from a per-channel offset, the print-model
shape error it absorbs, is reported beside it. The sheet's other chart of the
same neutral, the midscale spectrum, disagrees with the series (Known
limitations). The self-report also prints the paper-table solve (machine
...
|---|---|---|---|
| Endura Premier | [0.084, 0.346] | 0.86 | 0.35 stop per 0.01 k |
| Fuji Pro Laser | [0.034, 0.374] | 1.12 | 0.26 stop per 0.01 k |

The sensitivity is the system gamma over the window (3.23 on Endura, 2.39 on
Fuji Pro Laser with Fujifilm 400) times the 0.033 D that 0.01 k represents.
The window varies by at most 0.002 k across the Kodak stocks on Endura and 0.012 k
...
blue-versus-red difference) is discarded: `Dbase` is
...
|---|---|---|---|---|
| baseline | 3.035 | 0.1828 | −0.01 | 0.02 |
| gain +0.010 | 2.937 | 0.2367 | −0.01 | 0.00 |
| gain −0.010 | 3.101 | 0.1377 | −0.02 | 0.02 |
| gamma 1.20 | 3.293 | 0.1834 | −0.01 | 0.02 |
| gamma 0.85 | 2.758 | 0.1824 | −0.01 | 0.01 |
| gamma 0.85, pivot 0.10 | 2.882 | 0.1069 | −0.01 | 0.01 |
| literal gamma 0.90 | 2.691 | **0.4153** | −0.01 | 0.02 |
| trim R +.005 B −.005 | 3.026 | 0.1871 | 1.84 | 4.79 |
...
arithmetic; Resolve-specific macro faults escape it:
...
|---|---|---|
| neutral offset subtracted by the Status M cube, R/G/B (D) | −0.105 / −0.049 / +0.154 | +0.068 / −0.045 / −0.022 |
| sheet series' curvature about that offset, k = 0.22 ± 0.12, max \|a\*\| / \|b\*\| | 4.4 / 4.0 | 2.4 / 0.9 |
| printable window (Dnorm k) | **[0.084, 0.346]** | **[0.034, 0.374]** |
| system gamma over the window | 3.227 | 2.391 |
| neutral input k landing Y = 0.18 | 0.221 | 0.221 |
| gray-axis lock, mean correction per layer C/M/Y (logE) | +0.30 / +0.02 / +0.07 | +1.52 / +0.84 / +0.80, relative-axis origin folded in |
| gray-axis lock, range of correction over the window C/M/Y (logE) | 0.12 / 0.13 / 0.59 | 0.12 / 0.26 / 0.50 |
| outside P3 pre-clip | 26.8% | 14.4% |
| neutral-scale Status A solve, max residual | 4 × 10⁻¹⁶ D | 1.1 × 10⁻⁴ D |
| gray-lock solve residual | not tabulated | RMS 0.030, max 0.133 D |
| serialised 65³ interpolation RMSE / max (P3), tetrahedral | 2.2e-3 / 7.7e-2 | 1.4e-3 / 5.5e-2 |
| serialised 65³ interpolation RMSE / max (PQ), tetrahedral | 3.5e-3 / 1.3e-1 | 3.5e-3 / 1.0e-1 |
...
pairs: inter-stock spectral shape distances span **0.021–0.219 D** and the
basis sensitivity of the surrogate decomposition is **0.027–0.049 D**. Below
approximately 0.049 D the model cannot distinguish a genuine film difference
from an artefact of the assumed basis, which is the case for 16 of the 66
pairs, the last of them Portra 400 / Pro 400H at 0.0491 D against a band edge
of 0.0494 D. The closest pairs, all inside the ambiguity band:

| pair | shape distance (D) |
|---|---|
| Fujifilm 200 / Fujifilm 400 | 0.0000, identical by construction, one shared dye chart |
| Portra 800 / Ultra Max 400 | 0.0207 |
| Portra 400 / Pro Image 100 | 0.0211 |
| Gold 200 / Portra 400 | 0.0243 |
| Portra 160 / Pro Image 100 | 0.0265 |
| Portra 160 / Portra 400 | 0.0277 |
| Ektar 100 / Portra 160 | 0.0307 |
| Gold 200 / Ultra Max 400 | 0.0311 |

The other 50 pairs sit clear of the band, the widest being Superia Premium
400 against most Kodak stocks at 0.15–0.22 D.
...
well: the ratio (B−R)/(G−R) runs from 1.450 on Portra 160 to 2.117 on
...
masks (Portra 400 0.623, Portra 160 0.643, Ektar 100 0.648, Pro Image 100
0.690), the consumer and Fujifilm stocks among the strongest (Gold 200 0.737,
Ultra Max 400 0.749, Fujifilm 200/400 0.947), Portra 800 at 0.775 sitting
...
- Off the neutral axis the mask survives: for realistic pairs 13–20% of grid
  ...
  colours.
- The driver is SHAPE: a swap changing strength by 0.173 D with
...
that Kodak saturation differences are designed in by interimage and leave the
dye set alone, the axis this model has set to identity).
...
families jointly give a worst-case output error of 0.029–0.045 D max and
0.008–0.014 D mean over the dye 0–2.5 box and the neutral series, Ektachrome
E100 best at 0.029 D and Velvia 100 worst at 0.045 D: the largest model-side
term this branch has a measurement for, two orders above cube serialisation.
A shifted or stretched curve holds its measured end value across the part
of the original support the moved support no longer covers, so the term
measures shape alone; the instrument also prints the zero-boundary policy
beside it (`SHIFT/zero-boundary`, the moved support dropping to clear film),
which mixes shape with a boundary step of the edge density and puts Velvia
50's shift term at 0.119 D (0.097 D over the box) against 0.030 D with the
edge held, Ektachrome's at 0.041 D against 0.021 D, Velvia 100 and Provia
unchanged. What lies beyond the
original support is the missing-band term of register #2, bracketed there,
not a tracing term. Maxima sit at sign-pattern corners; means are
representative. The figures are extrema over the perturbation shapes
tested; no shape outside that set is covered.
...
a dye solution the scanner model closes on, the nearest such lattice node
and no point on a continuous boundary; reached nodes are
bit-identical. Reached means numerical convergence, and says nothing about
physical admissibility: of the reached nodes, 10.0% on Velvia 100, 17.4% on Velvia
50, 21.0% on Provia 100F and 14.4% on Ektachrome E100 close only with a
negative dye amount (Known limitations). The purpose is
...
Sampled against the full chain under the tetrahedral lookup Resolve is set
to, the shipped cubes hold to a maximum of 0.0012 D up to dye 2.0 and
0.0059 D up to 2.5, then 0.2993 D by 3.4 (every build prints the trilinear
figure beside it; neither scheme bounds the other). Lowering the
...
holds RMSE 0.0002 D and a maximum of 0.0005 D across the whole range on the
a7R III; the GFX 100 builds hold 0.0006 D except Velvia 50 at 0.0021 D.
...
reading is dominated by it. Five remedies were measured and rejected:
...
100F, failures at a median residual of 2.65 D); a robust solver (bounded
least squares, five starts, yellow ceiling 12, on the worst Velvia 50 cell
at dye 0.04/3.55/3.91, where the blue scan density plateaus at 2.603 D once
yellow passes 4: two of the cell's eight corners close, the other six stop at
the yellow ceiling with residuals of 0.004–0.050 D, so the unreachable nodes
are saturation and not nonconvergence); a 3×3 decoupling matrix in
...
0–4.0 under the trilinear probe, comparable within the table (the tetrahedral
figures the builds now quote run smaller). A monochrome sensor filtered per
exposure outperforms a Bayer camera on
both measures. The least costly realisation filters the LEDs
(nothing moves), needs no code change (the sensitivity schema carries
...
**Input is routed by what LibRaw reports, whatever the extension.** A
...
default; `repo_root()` returns `None` on a shallow path),
...
(a warning and no refusal, because only the operator knows whether a
...
the scene route, both delivered cubes sampled with the tetrahedral lookup the
Resolve setup requires after a header check against the requested stock and
sensor and a corner check against the nodes; the exact engine without its
cube is printed beside the scene cube as a diagnostic (the scene cube sits
0.2–0.3 ΔE2000 mean and at most 1.4 from it over the reflectance sets, 0.46–0.79
maximum on the ColorChecker), and the route's accuracy bound per stock is the
cube's figure. Its
...
stock's own basis and on the family basis the cube reads through. The gate
is a project criterion on that neutral-locus model-to-matrix comparison: the
worst per-channel mean absolute disagreement over the traced series, held to
0.02 D, the expected error the document states for its fitted transforms over
97 measured patches, 64 of them colours. The statistic has the form of the
document's "mean abs error" but not its population, a traced neutral series
against measured patches, so the threshold is borrowed and the result is a
consistency check; it reproduces neither the Academy's experiment nor an
independent 0.02 D bound on film reconstruction. It reads 0.018 D on the
stocks' own bases (per-channel 0.001–0.018 D), which passes, and 0.024 D on
the family basis the cube actually reads through, which exceeds the threshold
and is reported without gating. A signed mean would let alternating errors
cancel into a pass, so it is printed only as the bias figure, 0.018 D on the
own basis, where every red residual shares one sign and the two coincide, and
0.024 D on the family basis; the pointwise maximum, 0.035 D own and 0.037 D
family, is printed as the tail and not gated. A FAIL on the gate is a nonzero
exit status; no word is printed. Blue agrees within 0.005 D throughout. Green
below 2.1 D (strictly, the first point at or above 2.1 D being a shoulder
point) runs 0.003–0.006 D mean absolute with maxima of 0.015–0.019 D, and
climbs to 0.037 D at the shoulder, where the Academy's own linear fit misfits
green by 0.032–0.053 D. Red carries a near-constant offset of opposite sign
per stock, present at D-min, which correlates with the traced mask's
disagreement with the sheet's red D-min; the comparison does not separate
mask error from matrix-fit error and dye-model error, so that correlation is
an explanation to test. The comparison tests the
dye spectra, the mask and the two responsivity tables; it does not reach the
scan side. The Status M table sits on both sides of it, in the neutral solve
and in the remeasurement, while the Annex C matrix is fixed, so an error there
does not cancel: a +1 nm shift of the Status M red responsivity moves 250D's
red mean by 0.003 D and single points by up to 0.009 D (a sensitivity test,
not an estimate of the table's error; the table's explicit entries match the
1995 edition of ISO 5-3). `knowledge/academy-printing-density-and-the-adx-unbuild.md`
§4 carries the method, the matrices and the caveats.

A trace-budget instrument (not distributed) propagates four perturbation
families into the quantity the cube delivers, APD(mask + dye) − APD(mask)
under the ST 2065-2 responsivities on the engine's own 402–798 nm grid, which
with the mask fixed is the density of the dye stack under the weights
APD × 10^−Dmin(λ), the form the instrument asserts against the engine's; RP 180
is carried through every family under the same convention as a separately
named comparison. Every per-layer family is worst-cased over all eight sign
patterns and the joint term over the 128 combinations, opposite signs not
being mirror images through the nonlinear chain. The sample groups are never
merged: the dye 0–2.2 box; an equal-amount diagonal (k, k, k) of the three
peak-normalised dyes, named for what it is, since equal amounts of
differently shaped dyes are not a neutral exposure (decoded through each
stock's scene engine the diagonal spreads the layers' exposures by 1.2–2.4
stops); and each stock's own characteristic-curve neutral series, the
amounts the scene engine solves from all three curves at once, which the
family model decodes from that negative's scanner observation with the
decode residual printed as its reachability (at the numeric floor on every
stock). A wavelength-flat per-layer offset at the basis's inter-stock spread
(median 0.021–0.031 D per dye, the family traces' measured disagreement)
gives 0.010–0.012 mean / 0.033–0.037 D max on the neutral series and 0.057 D
over the box, the reading-error floor, structurally unable to alter the
metamerism between the scanner's view and the printing-density view. The real
perturbation, each stock's own dyes and own mask as the film against the
family dyes and family mask as knowledge, costs 0.001–0.005 D mean and at
most 0.015 D on the stock's neutral series and 0.003–0.010 D mean, at most
0.050 D, over the box, so the family averaging is confirmed in output space.
What the spread cannot see is a digitisation bias common to all four traces
(one digitiser, one method, one chart class), and the Vision3 record carries
no axis calibration to bound one with; under surrogate bounds taken from the
repository's traced-chart record (±1.5 nm per-layer lateral shift, 0.4% axis
stretch) the shift term reaches 0.010–0.014 D mean / 0.027–0.039 D max, the
stretch 0.002–0.003 / 0.007–0.015 D, and the three synthetic families jointly
0.018–0.026 mean / 0.069–0.110 D max (0.069–0.077 D on the neutral series,
0.110 D over the box), the branch's largest model-side exposure, bounded by
surrogate, and an extremum over the shapes tested only. The same perturbations read in RP 180
run three to four times larger in the mean and reach 0.47 D under the shift
and 0.60 D jointly, because RP 180's responsivities sit on steeper dye
flanks than APD's; those figures describe the archived metric and are not the
route's uncertainty. A second independent Vision3 source would convert the
surrogate into a measurement.
...
4. relative layer exposure L = 10^(logH − logH_mid). logH_mid comes from the
   ...
   per channel and averaged. The three per-channel values disagree
   (spread 0.026–0.103 logH) because the midscale spectrum does not lie on
   the characteristic series' neutral (Known limitations); the scalar
   average sets the exposure scale, and that disagreement is a colour
   residual the average does not remove. Where a trace stops short of a
   channel's responsivity the renormalised integral is a partial-band
   density inverted against a full-band curve: 500T's midscale trace ends at
   640 nm, below 69% of the red Status M weight on the density grid, and the
   build prints each channel's excluded share and warns above 5% (the other
   three stocks' traces cover the grid). The sheet's camera-stops zero is
   printed as a cross-check and not used; it differs by 0.067–0.404 logH, a
   uniform per-stock exposure trim, never a colour error;
5. a 3×3, weighted least squares over 3,282 measured reflectances
   ...
   blackbody for 200T/500T), Bradford-adapted to D65, with the grey
   constraint M·1 = XYZ(18% grey) imposed inside the solve, so that a flat
   18% grey on the characteristic series lands on Y = 0.18 and the matrix is
   the weighted least-squares minimiser among grey-preserving matrices (an
   unconstrained fit rescaled by row is a feasible point of the same problem
   and never a better one; the build prints both residuals and asserts the
   order). The set totals (ColorChecker 24, Munsell glossy 24, matt 12, Agfa
   12, skin 24, each spectrum weighted set total over set size) are a stated
   preference, giving the 100 skin spectra a quarter of the objective; maps L to XYZ (D65);
...
yellow-layer toe below 380 nm; synthesising reflectance data to
integrate against it is forbidden. The densitometry grid is a separate
question, the scanner's own: it runs from 402 nm to the end of the
dyes' measured support, 760 nm on 50D and 798 nm on the other three, capped
at 800 nm as the ADX16 engine's grid is, and the LED spectral power
distribution is measured to 780 nm and zero beyond it, so nothing is
asserted the apparatus did not measure. A 730 nm cap once shared with the
colorimetric grid discarded the red LED's tail behind the cyan dye; under the
unity sensor that tail moves scanner density by up to 0.013–0.020 D and the
decoded layer exposure by 10–16% at the 99th percentile and 29–51% at most
over 2,000 uniform dye states per stock, an observer-area fraction of
0.04–0.19% amplified by in-band attenuation and the exposure inverse, while
under the a7R III response, which carries its own infrared cut, the same band
moves density by less than 5 × 10⁻⁶ D.

Self-report across the four stocks: ColorChecker matrix residual 2.16–2.62
ΔE2000 mean, maxima 4.72–5.81 (the worst patch cyan on 50D and 250D, red on
200T and the white patch on 500T), a training-set figure and not a floor:
the grey constraint imposed in the solve leaves the weighted XYZ residual
4.8–9.0% below that of an unconstrained fit rescaled by row, the 100 NIST
skin spectra withheld entirely from the fit score 2.52–3.28 against
1.58–2.10 in the fit, and halving or doubling any one set's weight moves the
pool mean by up to 0.31–0.38 ΔE2000 from the shipped matrix; the
...
the traced midscale spectrum itself, read by the bare LEDs over its measured
support, anchored on the traced D-min and decoded through the delivered
inverse and matrix, landing at Y 0.159–0.184 against 0.18 and 1.4–11.3
ΔE2000 off neutral at matched luminance (200T 1.4, 500T 3.1 with the red LED
41% outside the trace's support and so an approximation, 50D 6.2, 250D
11.3), the disagreement between the sheet's two charts of one neutral that
the scalar anchor cannot remove (Known limitations); corridor
requirement 1.86–2.02 D against DMAX 3.30, 64–77% headroom; node solve on the
63–65% of nodes inside the dye box mean 0.006–0.023 D, above 0.02 D on
1.0–3.3% of them, the remainder being density combinations no dye stack
...
serialised round trip at 3 × 10⁻⁷. The ColorChecker "full chain" line is a
...
solve and no independent evidence for the fitted curves or for off-neutral
...
repository is the Annex C check. None of these checks bears on the
off-neutral inverse. The neutral series fixes one path through a
three-dimensional exposure response, and the identity coupling the chain
assumes is one of many rules that reproduce it: a diagnostic coupling of
1.10 on the diagonal and −0.05 off it, back-solved onto the same series,
reproduces the neutral scan within 4 × 10⁻⁴ D on 250D and 10⁻¹⁵ D on the
others while decoding the 3,282 training reflectances 1.6 ΔE2000 mean and
4.9–5.2 maximum apart from the shipped chain, single layers up to
0.42–0.73 stops apart. The coefficient is arbitrary and no such coefficient
is installed; the point is that no neutral check can choose it.

The route's characteristic cost is interpolation: the output is exponential in
the input on a lattice uniform in density, so a probe of the serialised cube
against the exact chain under the tetrahedral lookup the Resolve setup
requires gives, within each stock's published characteristic span (per
layer, the top of its neutral table, dye 0–1.68 to 0–2.02), a mean relative
error of 1.6–1.9% with a 99th percentile of 9–11% and a maximum of 20–48%;
over the full dye 0–2.2 working range the mean is 1.3–1.7% and the maximum
17–46%, the clamp bounding what any node beyond a table's end can carry. The
build prints the trilinear figure beside it (2.2–2.6% mean, 12–15% at the
99th percentile, 24–49% maximum within the span), since neither lookup bounds
the other; the tetrahedral figure is the delivered one. The build separates
that figure by lattice cell. A cell is clean when all eight corner nodes converged to within
...
hold 80–85% of the span's samples at 1.5–1.7% mean, 7–8% at the 99th
percentile and 15–18% maximum; the remaining 15–20% run 3.6–4.0% mean, 14–17%
at the 99th percentile and 20–48% maximum. The cells that produce those
...
covering 74–81% of the span's samples at 1.5–1.6% mean, 6–7% at the 99th
percentile and 15–18% maximum; within it, by exact-chain luminance, the
mean is 0.8–0.9% below mid-grey, 1.1–1.3% from mid-grey to ten times it and
2.0–2.3% to a hundred times it. The camera-named builds reach 83–88% clean
cells and hold the declared region at 1.1–1.2% mean, 4–6% at the 99th
percentile and 7–13% maximum. The probe is tetrahedral, on 5,000 fixed-seed
dye states drawn uniformly over each range, and its coverage figures describe
that synthetic probe alone; the
lookup probed is the one the Resolve setup requires, with trilinear printed
beside it for comparison only. That uniform dye box under-samples the
shadows, so the build also sweeps the ColorChecker through the delivered
lookup at −4 to +2 stops, retaining a patch only where the exact inverse
returns it unclamped and its scan lies inside the cube: the worst-channel
error is at most 1.1–1.7% at nominal exposure, 1.7–6.9% two stops under and
22–97% four stops under (15, 19, 24 and 24 of the 24 patches retained on
50D, 250D, 200T and 500T, the rest clamped or, on 250D, outside the cube),
where a doubling of exposure fails to double the output by up to 18–39%. A
valid, unclamped state can lie in a cell whose other corners are clamped or
unconverged, so those figures are not confined to clean cells; the
denominator floor of 0.001 DWG and the low signal levels make them lattice
errors. They are reducible: the same lattice storing
log exposure, exponentiated and matrixed after the lookup, probed at exact
corner values only, holds the declared region at 0.4% mean and 2.2–2.9% at
the 99th percentile with maxima of 7–17%, at the cost of a decode stage the
Resolve chain does not have; what ships is the linear cube. Two further
things the region does not
certify. A density residual bounds no exposure error: the exposure
derivative is the inverse table's slope over the scanner Jacobian, and
±0.01 D on one scanner channel about a neutral moves a layer's decoded
exposure by up to 0.09–0.10 stops at nominal exposure and 0.15–0.25 stops
four stops under, Y by 4.4–4.9% and 7–12%, the perturbation being a
diagnostic size and not a measured scanner noise. And the cube carries no
per-pixel flag: a consumer receives values for every node, the unconverged
and the clamped included, and nothing in the output marks whether a pixel
decoded inside the declared region.
...
|---|---|---|
| 2.0 | 0.0001–0.0002 | 0.0012 |
| 2.5 | 0.0002 | 0.0059 |
| 3.0 | 0.0002–0.0007 | 0.0329 |
| 3.4 | 0.0006–0.0043 | 0.2993 |

Tetrahedral lookup, the scheme the Resolve setup requires; the trilinear
figures each build prints beside it run 1.1–4.5× larger at dye 3.0 and
neither scheme bounds the other. On the cube's own faces and corners, where
one scanner channel sits at zero or at the corridor, the reached samples
hold to 0.005 D on Provia 100F and 0.025 D on Velvia 100 but reach 0.39 D on
Velvia 50 and 0.61 D on Ektachrome E100: the faces are where the projection
boundary runs, and a lookup there is a clamp.

A build named to a specific camera does not degrade in this way, holding RMSE
0.0002 D and a maximum of 0.0005 D across the whole range on the a7R III and
0.0006 D on the GFX 100, Velvia 50 excepted at 0.0021 D.
...
tungsten) and Bradford-adapted, with the grey constraint inside the solve.
ColorChecker matrix residuals are 2.16–2.62 dE2000 mean with maxima of
4.72–5.81, a training-set figure; the neutral exposure ramp is exact over
...
traced mask; the corridor requirement is 1.86–2.02 D against 3.30; and within
each stock's published characteristic span the serialised table tracks the
exact chain at 1.6–1.9% mean relative error under the tetrahedral lookup,
1.5–1.7% over the 80–85% of samples whose lattice cell is clean, with the
20–48% maxima confined to cells that straddle the top of a layer's
published curve, where the exact chain itself steepens, and shadow errors
in a ColorChecker exposure sweep of up to 22–97% four stops under nominal.
The mid-grey anchor is the traced midscale-neutral curve inverted through
the characteristic curves (per-channel spread 0.026–0.103 logH, the
spectrum decoding 1.4–11.3 ΔE2000 off the series' neutral); the sheets'
camera-stops zero differs from it by 0.067–0.404 logH, a uniform per-stock
exposure trim reported as a cross-check only. Off the neutral series the
decode rests on an assumption the neutral checks cannot test, and no frame
has been checked against a known scene.
...
| Ektar 100 | `print_endura/` | 0.3552 · 36.5% | 0.0005 / 0.0094 |
| Fujicolor 100 [JP] | `print_fuji/` | 0.3609 · 34.3% | 0.0004 / 0.0075 |
| Pro Image 100 | `print_endura/` | 0.3686 · 38.1% | 0.0006 / 0.0103 |
| Gold 200 | `print_endura/` | 0.3842 · 40.2% | 0.0006 / 0.0139 |
| Portra 160 | `print_endura/` | 0.3845 · 40.6% | 0.0005 / 0.0083 |
| Portra 800 | `print_endura/` | 0.3886 · 40.1% | 0.0007 / 0.0126 |
| Portra 400 | `print_endura/` | 0.3919 · 37.7% | 0.0005 / 0.0102 |
| Ultra Max 400 | `print_endura/` | 0.3925 · 41.1% | 0.0007 / 0.0145 |
| Pro 400H | `print_fuji/` | 0.4403 · 37.8% | 0.0007 / 0.0152 |
| Fujifilm 400 | `print_fuji/` | 0.5809 · 69.6%, least confident | 0.0016 / 0.0342 |
| Fujifilm 200 | `print_fuji/` | 0.5809 · 69.6%, identical to Fujifilm 400 | 0.0016 / 0.0342 |
...
tabulated above is RMSE 0.0004–0.0016 D with maxima of 0.007–0.034 D, and the
serialised round trip matches it. Over the whole declared corridor the same
probe reaches 0.005–0.20 D RMSE and up to 2.9 D, the excess sitting where the
sensor-free blue channel has saturated; a colour filter removes that plateau,
so the figure belongs to the unity response. Two
...
artwork. And the fleet as a whole cannot
...
so the per-stock ranking reports fit quality alone.
...
builds/c41/   fleet complete at twelve stocks; regenerable via engine/c41/ with --stock
  <Stock>_StatusM.cube           NEGATIVE BRANCH, one per stock at the root:
                                 ...
                                 corridor 3.30). The FRONT of the chain:
                                 both print branches and the Print
...
dctl/shapers/
  ...
  Postshaper 5.25.dctl           of a per-camera corridor
...
engines. The release carries these 45 alongside cube_manifest.json. Cubes are the only transform
...
anomaly tracks the artwork: the stock with the lowest node-solve residual
in the fleet is a Fujifilm stock, Superia Premium 400, and the only two
anomalous ones are the two that share a chart. Their figure of 0.5809 is the deliberately
accepted cost of applying the ±25 nm shift bound uniformly, with no
tighter bound for those two stocks (`portra_stocks.py`, register
...
`endura_print_engine.py:271-275`. This is by design:
...
cast (Glossary). The characteristic curves remain digitised
...
- **Scan to Status M: the approximation cancels.** The per-roll D-min
  ...
  described relative to D-min, as its `units` field states. Calibration and
  application share one reference convention, which is why the neutral axis
  closes as an algebraic identity of the model. The shared label does not
  establish that basis and target carry the same mask-consumption
  contribution; the per-dye measurement protocol behind the Vision3 curves is
  unpublished, and cancellation has not been demonstrated
  (`knowledge/orange-mask-and-the-scanning-workflow.md` §5c).
- **Print emulation: the approximation does not cancel, although the defect
  ...
  `N(l) = dmin_spec(l) + sum dye*DYE(l)` (`endura_print_engine.py:476`). The
  reconstruction returns D-min, the full mask, at zero effective dye amount.
  At the fitted midscale it reproduces the aggregate within the recorded
  residual (`fit_audit` RMSE, 0.0108 D for Portra 400). Fixed
  effective spectra, mask consumption linear in the dye formed, and the
  extrapolation of both to other dye ratios are modelling assumptions;
  neither the neutral output calibration of the gray-axis lock nor the
  aggregate fit quality establishes physical spectral exactness. The real
  systematic is **off-axis mis-attribution**: each fitted per-layer curve
  carries a share of mask consumption apportioned at the neutral ratio, the
  only ratio the datasheet publishes, so at saturated colours the per-layer
  split of the consumed mask is underdetermined. The expected residual is a
  chroma-dependent colour drift, held to zero on the grey axis by the lock,
  whose direction and dependence on saturation are not established.
...
The entries below are documented and left in place, in accordance with the
...
2. **Cyan long-wavelength truncation: handled for the shipped reversal cubes;
   ...
   *Corridor clipping is distinct from LUT resolution.* Beyond dye 4.0 on a
   ...
   at 5.25 both stocks sit at RMSE 0.0003 / max 0.0009 D over dye 0-4.0
   (trilinear probe; 0.0002 / 0.0005 D tetrahedral) with
   ...
   *What survives, for all four stocks:* the observer is truncated at the dye
   edge and renormalised (for Ektachrome, 0.138% of X weight and 0.049% of Y
   omitted above 700 nm, its chart ending there with cyan still ~0.75 D, and
   0.22–0.26% of Z omitted below 400 nm on every stock), which produces no
   hard ceiling. Truncation is one point inside the bracket hard constraint 1
   requires, never the bound, and every reversal build prints the bracket on
   a neutral stack (`tail bracket` lines). Below the grid the yellow dye and
   the base absorb, so the extremes are the tail blocked against the edge
   value held, and they differ from the truncated figure by no more than
   0.006 D in Z on any stock at dye 2.5: no material spread. Above the grid
   the extremes are the edge value held against clear film, and the spread
   is the real term: at neutral dye 2.0 a clear cyan tail lowers X against
   the truncated figure by 0.019 D on Velvia 100, 0.036 D on Velvia 50,
   0.022 D on Provia 100F and 0.060 D on Ektachrome E100; at dye 2.5 by
   0.051, 0.106, 0.070 and 0.166 D. The held end sits within 0.010 D of the
   truncation on the three stocks whose cyan is still dense at the edge, so
   for them the truncation is inside its bracket. Provia's cyan is measured
   to 0.00 D at 719 nm, so held and clear all but coincide there (−0.066
   against −0.070 D at dye 2.5, the residual being yellow's 0.01 D at the
   edge) and the shipped truncation sits 0.021 D (dye 2.0) and 0.066 D
   (dye 2.5) outside its own bracket in X. The cube is kept on the truncation for all four stocks,
   uniform treatment being the rule of this register; the spread is the
   bound carried, and the Provia figure is a known bias of that size. Deep shadows (dye ≳2.5) are treated as qualitative; a
   mini-LED display's own deep-shadow weakness covers the same region.
...
7. **Axis-calibration uncertainty varies by datasheet source.** Provia's
   ...
   since labels corroborate the gridlines.
...
8. **Per-layer C-41 dyes are inferred, across the whole
   ...
   |---|---|---|---|---|
   | Portra 800 | 0.0078 | 0.0331 | +21.99 | 1.4144 |
   | Ultra Max 400 | 0.0083 | 0.0435 | +22.18 | 1.4195 |
   | Ektar 100 | 0.0086 | 0.0729 | +22.50 | 1.4079 |
   | Gold 200 | 0.0088 | 0.0396 | +21.28 | 1.4140 |
   | Pro 400H | 0.0103 (in-band 0.0105, traced tail 701–716 nm 0.0009) | 0.0302 | +15.53 | 1.4739 |
   | Portra 400 | 0.0108 | 0.0685 | +23.71 | 1.4175 |
   | Fujifilm 400 / 200 | 0.0119 | 0.0413 | **+25.00 (pinned)** | 1.6676 |
   | Pro Image 100 | 0.0135 | 0.1243 | +25.00 (free, 0.0004 nm inside the bound) | 1.4032 |
   | Portra 160 | 0.0138 | 0.0785 | +20.85 | 1.4185 |
   | Superia Premium 400 | 0.0196 | 0.1479 | +19.90 | 1.4706 |
   ...
   - **Every stock's cyan sits to the red of the Vision3 basis cyan**, sC
     between +15 and +25 nm; two stocks rest exactly on the +25 nm bound
     (the two Fujifilm stocks, one shared chart) and Pro Image 100 sits
     within 0.001 nm of it, so for three of the twelve the value is the
     constraint. The fit establishes a
     direction only; the other nine fit freely between
     +15.1 and +23.7 nm, Pro 400H at +15.53 with its traced red tail in the
     objective (below). C-41 cyan genuinely differs
   ...
   - **The shift bound is a uniform ±25 nm on every stock** (`portra_stocks.py`),
     and it is an extrapolation guard, with no chemistry behind it. A former
     ...
     ±25 moves which stocks pin and how many, two of twelve resting on
     +25.00 where seven of twelve rest on +20.00 under a ±20 bound. Refitting
     the fleet at ±15, ±20, ±25, ±35 and ±50 gives mean aggregate RMSE
     0.0160, 0.0137, 0.0122, 0.0118 and 0.0118 D, so the solve converges by
     ±35, with no stock pinned there and the largest fitted shift +28.58 nm.
     Releasing the bound that far buys 3% of residual and
     moves the modelled cyan peak to 718 nm, while the dye arrays end at
     ...
     which hard constraint 1 forbids. Tightening to ±20 or ±15 costs 13% and
     32% of residual and leaves the peak extrapolated regardless. Only data would
     place the peak. The Fujifilm-family sheets publish
     spectral dye density out to 716–719 nm and the Kodak sheets to about
     703; the canonical fit runs on the 400–700 nm arrays every consumer
     reads, and raising that ceiling without the traced samples degrades
     measured RMSE by 500–1160% and inverts every fitted cyan shift
     (Invariants). The five Fujifilm-family curves files carry the traced
     tail beside those arrays (`spectral_full_support`, null outside each
     trace), and the canonical fit is tested against it as a WITHHELD band.
     The measured aggregate reaches its red-region maximum at or just
     before that band, at 694 nm on Fujicolor 100, 696 on Pro 400H, 701 on
     Fujifilm 200/400 and 705 on Superia Premium 400, so the withheld
     samples lie on the turnover and the descent. Fujifilm 200/400 and
     Superia Premium 400 predict them at 0.004 and 0.007 D RMSE, below
     their in-band residual: the aggregate extrapolation is corroborated by
     samples the fit never saw, which identifies no independent cyan
     component. Pro 400H, fitted over 400–700 nm alone, predicts its
     withheld tail to 0.0017 D with the cyan peak at 700.0 nm; Fujicolor 100
     overshoots its tail by 0.045 D mean, the modelled cyan lying too far
     red. The same test sets the WIDTH bounds, which are per dye and uniform
     across the fleet. Magenta and yellow absorb inside the measured band,
     so their widths are left to the data at 0.5–2.0, a range no stock
     reaches; held to 0.85–1.15, the magenta width pins Pro 400H at 1.15 and
     sends its 400–700 fit into the other of two cyan basins, pinned at
     +25.00 with the peak at 713.7 nm, which misses the withheld tail by
     0.0525 D. The cyan width keeps 0.85–1.15, an extrapolation guard like
     the shift bound: released, it makes the withheld tail predict worse,
     Superia Premium 400 0.0070 → 0.0518 D and Fujicolor 100
     0.0491 → 0.0700 D. The shipped Pro 400H fit admits its tail (registry
     `fit_traced_tail`; the 16 samples enter as extra residual terms on
     their own wavelengths, the output curves staying on 400–700 nm), at
     0.0105 D in-band and 0.0009 D on the tail as training error, no
     parameter on a bound and the cyan peak at 699.9 nm. Against the
     400–700 fit it moves no dye curve by more than 0.0009 D, and the Status
     M reconstruction by 0.0001 D. The other four
     Fujifilm-family stocks keep the 400–700 fit: Fujifilm 200/400 and
     Superia Premium 400 move under 0.3 nm with the tail admitted, and
     Fujicolor 100 has no withheld validation to admit it on. Fujicolor 100 moves from 702.4 to 699.0 nm
     at 0.0207 → 0.0216 D in-band and still misses the tail at 0.022 D,
     which suggests a mismatch of the warped form at the turnover that the
     bound does not cause. Across the
     fleet, holding the cyan shift fixed and refitting the rest bounds the
     ambiguity (a sampled estimate, 1 nm steps with the tolerance crossings
     bisected to 0.05 nm, excess measured against the unrestricted optimum):
     within 5% of each stock's best residual the cyan peak moves 0.5 nm on
     Fujifilm 200/400, 3–7 nm on seven further stocks and 12–16 nm on Portra
     160, Portra 800 and Pro Image 100; the same three stocks are the ones whose
     peak moves 10–16 nm across the four individual Vision3 stock bases and
     the family average where the other nine move under 3 nm, ambiguity
     across every chart and basis tested, located in neither. The Status M density of the
     reconstructed midscale spreads no more than 0.014 D in red and 0.005 D
     in green or blue across any such set, but that is closure of the
     fitted sum, a range across reconstructed midscales. A different
     statistic, the maximum deviation from the shipped conversion: each
     member's dye set inverted on identical scan densities (216 dye-amount
     triplets pushed forward under the shipped set through the LEDs behind
     the stock's D-min, the per-node solve the Status M engine performs,
     re-implemented on a unity sensor, over the population every member
     inverts validly) deviates in Status M by up to 0.100 D in green on
     unequal-amount inputs for Portra 160, 0.062 Portra 800, 0.056 Superia
     Premium 400, 0.051 Pro Image 100, 0.040 Fujicolor 100, and
     0.004–0.025 D on the other seven; equal-amount inputs, which are not a
     photographed neutral since the curves are independently normalised,
     deviate 0.002–0.051 D. The two figures are not a ratio.
     The split the data cannot fix reaches the conversion on unequal
     mixtures (Invariant 7: the split is the prior's, the sum is the
     data's). The shipped cubes were not rebuilt per member and the print
     path is not propagated. A per-stock bound is inadmissible, since fitting stocks
     ...
     of 0.5809, the highest in the fleet, is the accepted cost of declining
     to damp them with a bound they alone would receive.
   - **A stock that pins more than one bound simultaneously signals
     under-determination of the basis**, and is
     read as evidence about the basis, never as a property of the emulsion.
     Fujifilm 200 and 400 pin the cyan shift and the 0.85 cyan width, and
     take a magenta width of 0.728 where every other stock lies in
     0.84–1.19. Eight of the twelve stocks fit against a bound, Portra 160,
     Portra 800, Pro Image 100 and Pro 400H being free of one: Ektar 100,
     Gold 200, Portra 400, Ultra Max 400, Fujicolor 100 and Superia Premium
     400 pin the 1.15 cyan width. No magenta or yellow width reaches its
     0.5–2.0 bound. That the bound binds this widely under the
   ...
   - Status M red responsivity is truncated at the 700 nm dye-chart edge and
     renormalised, affecting 0.49% of the red area on the 1 nm grid, the same handling class
...
9. **The shipped cubes embed no camera, and naming one is worth a bounded and
   ...
   *The default assumes a unity response; a shared curve cancels only in its
   ...
   illustrative shapes against a flat reference, measured on no
   sensor:
   ...
   *The bound is a function of density.* Substituting each of the forty-four
   ...
   - **Below approximately 1.2 peak dye the camera is a second-order term**,
     ...
     climbs past the 0.027–0.049 D basis-sensitivity band of entry 8. A
   ...
   *A colour filter is not purely a cost.* It band-limits the illuminant's
   ...
   working range (C-41 probe RMSE 0.0004–0.0016 D, maximum 0.007–0.034 D over
   ...
   figures are a property of the unity response. On the 5.0
...
10. **Scan-side stray light is unmodelled.** `engine/` contains no
   ...
   unquantified. The inexpensive route to closing it
...
11. **Reversal interimage is unmodelled, and no structure exists for it.**
   ...
   interimage by one method. It is a caveat: the
   ...
   §4a.
12. **Most of the C-41 fleet fits against a bound.** Eight
   of the twelve stocks rest at least one shape parameter exactly on its
   limit; only Portra 160, Portra 800, Pro Image 100 and Pro 400H are free
   of one. Two pin the cyan shift at +25 nm (both Fujifilm stocks), and all
   eight pin the cyan width: Ektar 100, Ultra Max 400, Gold 200, Portra 400,
   Fujicolor 100 and Superia Premium 400 at the 1.15 ceiling, the two
   Fujifilm stocks at the 0.85 floor. No magenta or yellow width reaches its
   0.5–2.0 bound. A
   ...
   The solve does not depend on where it starts: `least_squares` is a local
   method, and on seven of the twelve stocks a seeded 64-point multistart
   over the same bounds finds a better optimum than a single fixed start, by
   2.0–29.5% in RMSE, Ektar 100 the largest; on the other five the gain is
   under 0.03%. `fit_audit.multistart`
   records the seed, the start count, the single-start RMSE and the
   improvement. The ±25 nm bound is load-bearing for three of the twelve,
   counting Pro Image 100 at 0.0004 nm inside it, and the fitted cyan shift is
   not a per-stock measurement for those stocks; for the other nine the
   shift is a fitted optimum between +15.1 and +23.7 nm; whether the uniform bound should be widened is an open modelling
   question, to be justified against dye chemistry, never against the residual
   it would buy.
...
13. **The digitised spectral curves carry fabricated edges, bounded but real.**
   Where a Kodak spectral trace stops short of the 400–700 nm frame, the
   digitiser flat-holds its last traced value across the gap: 53 samples
   across the seven Kodak stocks, 34 of them on Ektar 100, whose printed
   curves end at 684.9 nm, so 16 samples per curve are held at the red end,
   D-min at 0.2582, far from a negligible tail. Six stocks begin and end
   within 0.6 nm of the frame edges and hold 0 to 2 samples per curve. Pro
   Image 100's midscale trace begins only at 402.97 nm where the printed
   curve is clipped by the top of the plot box, so four samples of midscale
   and two of D-min are held. Replotted onto the printed chart, the traced
   regions hit the ink on 100.0% of samples for every stock and both curves;
   the held edges hit it on 35.3% of samples on Ektar 100, both curves, and
   on 100.0% everywhere else, so on Ektar the held value disagrees with the
   printed curve by more than the chart's line width.

   *Reach.* The decomposition excludes it: `portra_decompose.py` masks its
   objective to each stock's measured support, and all 53 samples fall
   ...
   mask stops at the 400–700 nm grid edge, beyond the stock's measured
   ...
   *Recorded, uncorrected,* because removing the hold means
...
14. **Part of the Vision3 basis is tracker prediction, and the
   ...
   ambiguous ink, with no invention where no ink exists, hence a caveat.
...
17. **The roll anchor subtracts D-min in integrated density, and the engines
   ...
   *The reversal path carries the same term under a surrogate base.* No
   reversal sheet publishes a base spectrum, so each build estimates the
   term with one surrogate tint its own sheet admits: the characteristic curves'
   ...
   D-min triplets are flat to the third decimal. These figures are the
   sensitivity to that one surrogate on a sampled 9×9×9 grid, and they bound
   no other base spectrum: a base whose tint varies within an LED's band and
   leaves the channels equal is invisible to it, and a measured clear base would
   replace it. Every reversal build prints the figure.
   *The missing band as a term.* Truncation at the support edge is exact for
   the truncated weighting and for nothing else: it asserts that the
   uncovered bands transmit like the covered ones, and a perturbation of a
   band the shipped mode never integrates is zero by construction, which
   measures insensitivity to an unused input; the cost of excluding it is
   unmeasured.
   The error budget therefore carries the support policy itself: the paper
   exposure integrated with the negative's edge density held outward against
   the shipped truncation, each with its own re-solved lock, gives 0.30 mean
   / 1.78 max ΔE2000 off-neutral on Portra 400 and 0.62 / 1.93 on Fujifilm
   400, the fourth-largest term on Portra and the second on Fujifilm; clear
   film in those bands, a permissive diagnostic and not a model of the
   missing spectrum, gives 7.9 / 12.6 and 10.0 / 14.7 and is printed beside
   the table. On the scan side the Status M red responsivity omits 0.49% of
   its area beyond 700 nm on the 1 nm grid the transform integrates on (the
   10 nm source table sums to 0.28%); at equal dye 2.2 a clear red tail
   would read 0.091 D lower than the truncation on Portra 400 and 0.047 D on
   Fujifilm 400, the edge held 0.002 D higher. Both stages share the
   truncation convention, so a consistent change to that intermediate can
   cancel through the chain and the two figures do not add.
18. **Part of every fitted C-41 dye set rests on the basis's held blue edge.**
   ...
   published: magenta held at −0.0057 over 400–409 to 426 nm on ten stocks,
   cyan at 0.2164 over 400–465 nm on Fujifilm 200 and 400 and over 400–425 nm
   on Portra 160, yellow at 0.5103 over 400–403 to 411 nm on all twelve, each
   ...
   on the Kodak stocks (Portra 400 0.0077 against 0.0108 D) and a higher one
...
19. **The reversal sheets' neutral series is not visually neutral through the
   ...
   series may not be a visual neutral at D50 by design; the Status A table's
   explicit entries match the public 1995 edition of ISO 5-3, but its tails
   and the 2009 edition are unverified, so a shape error there is narrowed,
   not excluded;
...
computes `-log10` of an already logarithmic quantity where the `10^-D`
inverse is required. `Density to Linear.dctl` performs that step, and NamiColor-style
...
- **Derive a curve's measured support from the data, never from a metadata
  ...
  the five Fujifilm-family C-41 stocks record an `endpoints` upper limit above the
...
- **Never bridge a gap where the curve left the chart.** Gold 200's cyan has
  ...
  implementation.
- **Ektar 100's printed spectral curves stop at 684.9 nm; the 685-700 nm red
  tail is not flat-held.** That band carries 1.7% of Status M red; flat-hold
  against zero-fill moves Ektar's red Status M aggregate by 0.042 D,
...
- **Spectral comparison tools restrict fits to each stock's measured
  support**; on a fixed 400-700 nm grid Ektar silently contributes
  13 fabricated flat-held points.
- **The Vision3 ADX16 route ships one shared-basis cube; the scene
  ...
  route. A per-stock printing-density cube earns its place by independent
  reference performance, never by a lower closure residual on the synthetic
  box.
- **Do not drop the 9-parameter warp or substitute an unwarped shared basis**
  ...
  peak-normalised dyes and dye shapes at developed midscale neutral. The
  parameter count is not the invariant; what the warp absorbs is, and a
  simpler, regularised or better-supported model that reproduces it on
  independent output performance may replace it. A stock-specific prior
  supported by independent information is admissible; a bound chosen per
  stock for the residual it buys is not, and fleet-discrimination claims
  keep the common prior.
- **A dye-amount inversion residual that sits entirely on zero-clipped nodes
  is a constraint violation and no fit error.** The Status M inversion on the
...
- **The gamut projection is confined to the reversal path.** Applying
  ...
  form scattered pockets; projecting a
  ...
  the region on any path. That rejects this method under these measurements,
  not every constrained solver; a successor is scored on the same figures
  and on off-lattice reachable samples whose interpolation uses an altered
  corner, unchanged reachable nodes alone being insufficient (invariant
  12).
...
- **No measured validation.** Every metric the C-41 fleet reports derives
  ...
  spanning the printable window. The marginal cost
...
- **The C-41 fleet cannot distinguish every stock it holds by their modelled
  dye sets.** Basis sensitivity is 0.027–0.049 D against inter-stock
  distances of 0.021–0.219 D, so 16 of the 66 pairs sit inside the ambiguity
  ...
  the orange mask alone, and only away from the
  grey axis.

- **The Vision3 scene tables are a per-stock model estimate, verified only
  against their own machinery, unvalidated off the neutral series, and
  imprecise in deep shadows and extreme highlights.** The 3×3 is fitted on
  measured reflectances (ColorChecker residual 2.16–2.62 dE2000 mean, maxima
  4.72–5.81, a training-set figure: skin withheld from the fit scores
  2.52–3.28 against 1.58–2.10 in it, and the set weights are a stated
  preference), the neutral ramp is exact by construction, and the
  ColorChecker full-chain figure collapses to the matrix residual because
  forward and inverse share code.
  ...
  triplet to within 0.045 D. None of that reaches the off-neutral inverse.
  The sheet's neutral series is one path through a three-dimensional
  exposure response, and the identity coupling the chain assumes is one of
  many rules that reproduce it: a diagnostic coupling of ±0.05, back-solved
  onto the same series, decodes the same neutral within 4 × 10⁻⁴ D and the
  training reflectances 1.6 ΔE2000 mean and 4.9–5.2 maximum apart, single
  layers up to 0.42–0.73 stops apart. That coefficient is arbitrary and is
  not installed; what it shows is that no neutral check can choose the
  rule, and only separation exposures on a measured roll can (the
  validation roll above). The exposure-invariant, additive sensitivity
  kernel is a further assumption of the same kind. The cube stores an
  exponential output on a
  lattice uniform in density, so the tetrahedral lookup departs from the
  exact chain by 1.6–1.9% mean within each stock's published characteristic
  span, 1.5–1.7% in the 80–85% of cells that are clean and up to 20–48% in
  the cells that straddle the top of a layer's published curve, where the
  exact chain itself steepens, and in a ColorChecker exposure sweep by up to
  22–97% four stops under nominal exposure, where a doubling of exposure
  fails to double the output by up to 18–39%; that tail is a reducible
  lattice error sitting on an irreducible model sensitivity (±0.01 D on one
  scanner channel moves a layer's decoded exposure by up to 0.25 stops four
  stops under nominal), and the build declares the operating region (clean
  cells below a hundred times mid-grey) and reports it separately, while the
  cube itself carries no per-pixel mark of that region. The ADX validator
  scores the delivered scene cube, and finds the cube within 0.2–0.3 ΔE2000 mean and 1.4 maximum of
  the exact engine on the reflectance sets. Mid-grey rests on the traced
  midscale neutral, and the sheet's two charts of that neutral disagree:
  the midscale spectrum decoded through the chain lands 1.4–11.3 ΔE2000 off
  the series' neutral at matched luminance (largest on 250D), a colour
  residual the scalar anchor does not remove, and on 500T the trace ends at
  640 nm, so the red channel's anchor is a partial-band density inverted
  against a full-band curve. The sheets' camera-stops zero differs from the
  anchor by 0.067–0.404 logH, a uniform per-stock exposure trim. The output
  is an adapted, scene-referred estimate under the stock's nominal balance
  illuminant, fitted on reflective surfaces; it recovers neither an
  illuminant-independent reflectance nor the unadapted scene radiance, and a
  fixed matrix is not shown optimal under other illuminants. No frame has
  been checked against a known scene.

- **The sheet's two charts disagree about what a neutral negative reads, and
  the chain can follow only one.** A C-41 neutral is not equal image-dye
  Status M, so the neutral axis has to be taken from the sheet. The
  characteristic-curve series, solved through the spectral base, puts Portra
  400's mid-grey 0.10/0.06 D below and 0.16 D above equal channels in R/G/B;
  the sheet's midscale-neutral spectrum, integrated under the same Status M,
  puts it 0.13 D below and 0.12 D above, a different green. Through the print
  each chart's neutral renders about 20 units of a\*/b\* off under the
  other's balance (the spectrum at a\* −20/b\* +14 on Portra 400, −9/−16 on
  Ektar 100, −20/−23 on Fujifilm 400, −7/−15 on Pro 400H under the shipped
  curve-series balance; under an equal-channel balance, which treats equal
  image-dye densities as neutral, the spectrum reads b\* −31, −48, −21 and
  −14). The series ships, being a series
  where the spectrum is one exposure of unstated placement; the error budget
  carries the distance as the "neutral reference" bracket, the same
  negatives balanced on the spectrum rendering 4.9 mean / 15.2 max ΔE2000
  apart on Portra 400 and 9.3 / 26.6 on Fujifilm 400, and a grey card on a
  measured roll is the only thing that can close it. The series' own
  curvature about the constant offset the lock uses is a further 4 to 5
  units of a\*/b\* over the printable band (every print build prints it).

- **That a frame's rendered cast matches its physical illuminant is
  unverified on film.** Hard constraint 5 guarantees only that no stage
  neutralises the cast; whether the fixed nonlinear chain preserves it is
  an empirical claim, and unlike a neutral-axis check it is not guaranteed
  by construction: the
...
- **Every published curve is a manufacturer prior.** Kodak states on the Vision3 sheets that the curves
  represent production coatings under the stated exposure and processing
  and do not apply directly to a particular box or roll; the other sheets
  carry the same caveat by convention. The per-roll anchor and any declared
  reference frame (hard constraint 5) are the only roll-specific inputs the
  chain has.
...
- **Half of every reversal lattice lies outside the dye gamut and holds a
  ...
  The cube is continuous at the boundary and every value in it is the
  colorimetry of a dye solution the scanner model closes on, but the region
  beyond the gamut is a clamp carrying no information: the nearest reached
  lattice node, not the colour the node asks for and not the nearest point
  on a continuous boundary. Convergence is not admissibility: of the reached
  nodes, 13,096 on Velvia 100 (10.0%), 23,073 on Velvia 50 (17.4%), 23,830
  on Provia 100F (21.0%) and 19,819 on Ektachrome E100 (14.4%) close only
  with a negative dye amount, a few dozen of them at the solver's −0.5
  floor, equally unphysical; every build prints the count. Those are left
  untouched, the mapping through them being continuous, since clamping them
  would displace colours near the boundary that the film does produce; a
  constrained inverse is an untested alternative. The negative and
...
- **Interimage and masking effects are unmodelled**, and for the transforms
  that end in a density this is a predictive limitation:
  a developed, scanned film carries its interimage in the measured densities,
  so those cubes reproduce the effect without modelling it. The Vision3 scene
  route inverts the densities to exposure, and there the identity is a
  modelling assumption (Known limitations, the scene-table bullet). What is
  lost is the
...
engine's own Status M inverse cannot reproduce to 0.001 D (79 of 400 on
Portra 400, missing by up to 0.417 D) measure the solver's clipping on
density triples no dye stack reaches, and are reported as their
own group. The reachable off-neutral column is the budget. The neutral
patches lie on the stock's neutral series, which the Status M cube has
placed on the diagonal.

Portra 400, shipped sensor-free configuration, off-neutral, mean and maximum
ΔE2000 over the 321 reachable in-gamut patches spanning the printable window:

| term | mean | max |
|---|---|---|
| basis sensitivity, six refits under different surrogate bases, propagated through the scan inverse and the print | **2.37** | **8.65** |
| the same at fixed Status M, print engine only (comparison only) | 1.26 | 7.45 |
| paper dye digitisation, ±0.00703 D per layer (chart y-axis fit residual) | 0.44 | 1.45 |
| missing-band policy of the paper exposure, edge held against truncated (register #13) | 0.30 | 1.78 |
| dye fit residual, ±0.0107 D per curve, a flat-offset scenario propagated through the scan inverse and the print | 0.32 | 0.95 |
| the same at fixed Status M, print engine only (comparison only) | 0.13 | 0.58 |
| paper H&D digitisation, ±0.00233 D per layer (chart y-axis fit residual) | 0.13 | 0.41 |
| both shipped cubes composed, Status M then print, tetrahedral, per patch | 0.11 | 1.08 |
| the print cube alone, tetrahedral (comparison, inside the row above) | 0.12 | 1.07 |
...
| **combined, root-sum-square** (under independence; no bound) | 2.46 | 9.08 |
| combined, plain sum (aligned perturbations) | 3.68 | 14.37 |

Fujifilm 400 on Fuji Pro Laser, 269 reachable patches: basis 2.91 / 13.66,
fit residual through the scan 0.51 / 1.30 (0.22 / 0.85 at fixed Status M),
missing-band policy 0.62 / 1.93, composed cubes 0.04 / 0.47, paper dye
wavelength axis 0.20 / 0.98.

Four sensitivities sit beside the table, having no
...
band, in both directions, read through the shipped Status M conversion: 0.20
mean / 0.97 max ΔE2000 off-neutral. The enlarger SPD, 3200 K being a nominal
choice: ±100 K gives 0.07 mean / 0.31 max. A Bayer capture, by register #9's
worst case applied at every patch: up to 15.5 ΔE2000 in the mean, a ceiling. The neutral reference: the same physical
negatives balanced on the sheet's midscale spectrum instead of its
characteristic-curve series render 3.23 mean / 11.81 max ΔE2000 apart on
Portra 400 and 9.49 / 27.09 on Fujifilm 400, the largest figure on the page
and a bracket: it is the sheet's own
disagreement about which negative is grey (Known limitations), the roll's
balance, and a grey card on a measured roll closes it where no modelling
can. Rendering the same balanced coordinates under both references, which
compares two different negatives, moves only 0.23 / 1.24 and is printed as
a diagnostic. The balanced encoding's floor loss (below) is printed beside
the table too. Four conclusions follow, the first the one to quote.

- **The figure these documents lead with is among the smallest terms that
  matter.** The dye fit residual contributes 0.32 ΔE2000 where basis
  sensitivity contributes 2.37, a factor of seven; a reader who takes the
  per-stock RMSE as the stock's accuracy is reading the wrong number by
  most of an order of magnitude. The residual is a wavelength-flat offset
  applied as a scenario: an aggregate RMSE over three superposed dyes is
  neither a per-layer pointwise bound nor a shape error, and the basis
  ensemble is what probes shape.
- **The budget is one term, and it is consumed twice.** The dye set enters
  ...
  alternative basis also performs the scan conversion, gives 2.37, and it is
  this figure that the surrogate basis costs. The fit residual is propagated
  the same way, 0.32 through the scan against 0.13 at fixed Status M.
  Everything else is at or below 0.44 ΔE2000 in the mean, the next-largest
  term, the paper's dye digitisation, sitting a factor of five below. The
  budget carries no structural term: it perturbs inputs, and the four
  model-structure errors that once outranked every term but the basis (the
  bare-LED scan side of register #17, the per-channel reading of the paper
  curves and of the Vision3 curves recorded under Invariants, and an
  equal-channel neutral as the lock's target) were found by testing
  the engines' assumptions, where input perturbation cannot reach, and are
  closed in the engines.
- **Two terms are exactly zero for the shipped configuration**, by
  construction, and neither zero is a bound: the fabricated spectral edges
  never reach the render because `neg_support_mode` truncates that band,
  which is why the support policy itself is carried as the missing-band
  term, and a per-layer offset on the paper's log sensitivity is a per-layer
  exposure scale, which the re-solved gray-axis lock absorbs identically at
  every input, the same mechanism that makes the Fuji paper's relative axes
  harmless. Only a wavelength-dependent sensitivity error could reach the
  render, and the chart's y-axis residual carries no shape information to
  bound one with. The dye chart's wavelength error is bounded by its own
  x-axis residual and carried as a term: vector-precise on Endura (±0.036
  nm, the 0.02 in the table) and an order larger on Fuji Pro Laser (±0.483
  nm), where it contributes 0.20 mean / 0.99 max ΔE2000, a shape change the
  ...
  re-solved lock. The two serialised cubes are scored as the composed chain
  they are deployed as, both sampled tetrahedrally (a scalar density of
  0.0005 D applied to every patch gave 0.05 / 0.21 and is
  printed beside the table for comparison).
- **The combined rows are a sensitivity inventory and no uncertainty
  interval.** Each term is the worst of a set of perturbations at a stated
  bound, carries no distribution, and no covariance
